...
\title{Conformal field theory from lattice fermions\\[0.25cm]
\large\normalfont Revised version of Commun.\ Math.\ Phys.\ \textbf{398} (2023) 219--289}
...
\small
\noindent\textbf{About this version.} This is a revised version of T.~J.~Osborne and
A.~Stottmeister, \emph{Conformal field theory from lattice fermions}, Commun.\ Math.\ Phys.\
\textbf{398} (2023) 219--289, \texttt{arXiv:2107.13834}, which is published open access under
CC~BY~4.0; it is not the version of record, and it should be cited as a revision rather than in
its place. No theorem statement has been changed. What has changed is that the regularity
hypotheses, left as ``sufficiently regular'' in the published version, are now quantified once
and for all in Definition \ref{def:regularity}; that several proofs have been corrected or
supplied, among them those of Lemma \ref{lem:KSconv}, Theorem \ref{thm:KSconvc},
Lemma \ref{lem:U1cur1pconv} and Theorem \ref{thm:corapproxvir}, the last of which is now
unconditional by Proposition \ref{hyp:energybound}; and that quantitative statements --- rates,
domains and an explicit error budget for quantum simulation --- have been added throughout. The
individual changes are itemised in \texttt{CHANGES-v4-to-v5.md}, and the findings that prompted
them in \texttt{critical-reread.pdf}, both in the accompanying source repository, whose
\texttt{README} also records how this revision was prepared.
\end{minipage}
\end{center}
...
It is worth pointing out that such a structure is not immediately available for the operator-algebraic renormalization of (free) massless chiral lattice fermions in 1+1-dimensions considered here. The obstruction, however, is not the failure of the finitely renormalized states $\omega^{(N)}_{M}$ to be pure. What the connecting unitaries $\phi^{N}_{M}$ require is that the GNS representations of the $\omega^{(N)}_{M}$ carry a common multiplicity --- this is the precise content of the Stone-von Neumann-type assumption made above --- and for a quasi-free state on a finite lattice that multiplicity equals $2^{m}$, where $m$ is the number of eigenvalues of the symbol lying strictly between $0$ and $1$. What is at issue is therefore the \emph{constancy} of $m$ in $M$, and not its vanishing. On the chiral subalgebra $m$ is constant at every finite $M$, so the connecting unitaries do exist at all finite scales; it collapses only in the limit $M\rightarrow\infty$, and then only for the momentum-cutoff renormalization group of Section \ref{sec:momcut}, the wavelet renormalization group of Section \ref{sec:waveletrg} leaving $m$ constant up to and including the limit. The mechanism behind the mixedness is indeed the entanglement of the chiral halves, and it is quantitative: by \eqref{eq:Fmassless} and $p_{\pm} = \tfrac{1}{2}(\mathds{1}_{2}\pm\sigma_{y})$ one has $\|[P^{(+)}_{\lambda_{N}=0}(k),p_{\mp}]\| = \tfrac{1}{2}|\sin(\tfrac{1}{2}\vep_{N}k)|$, so that the chiral restriction of the lattice vacuum is mixed with an extensive entropy, of density $2\ln2-1$ per site, carried by the modes at the boundary of the Brillouin zone and of order $(\vep_{N}k)^{2}$ on the modes that survive the scaling limit.\footnote{These statements, and the closed forms behind them, are proved in the companion note \texttt{zini-wang-comparison.tex} of the same source directory: Corollary 3.2 there gives the criterion, Theorem 4.1 and Corollary 5.1 the collapse at $M=\infty$, and Corollary 5.2 the entropy density.}
...
Let $s$ be a compactly supported orthonormal Daubechies scaling function which is $\tfrac{3}{2}$-regular in the sense of Definition \ref{def:regularity} --- by Remark \ref{rem:regularityvalues} this holds for $K\geq4$ --- and $\tfrac{5}{2}$-regular for the rate \eqref{eq:latviraconvsmearedrate}, i.e.~$K\geq7$; and let $X\in\fl(\Lambda_{M},\C)$ for some $M\in\N_{0}$. Then, the smeared KS approximants, $L^{(N)}_{\pm}(S^{M}_{N}(X))$, converge strongly to the continuum Virasoro generators, $L_{\pm}(S^{M}_{\infty}(X))$, on the dense domain $\fF^{\alg}_{\ua}(\cD_{\std})\subset\fF_{\ua}(\fh^{(\pm)}_{\infty,{\color{blue}\pm}})$ spanned by anti-symmetric Fock vectors with finitely many one-particle excitations in $\cD_{\std}$ (finite $\cD_{\std}$-particle number):
\begin{align}
