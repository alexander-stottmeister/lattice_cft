...
%DIF DEL free_fermion_cft_v4.tex   Wed Jun 19 21:00:26 2024
%DIF ADD free_fermion_cft_v5.tex   Thu Jul 30 14:42:15 2026
...
%DIF 257a257-258
\newtheorem{hyp}[thm]{Hypothesis} %DIF > 
\newcommand{\Nop}{\mathsf{N}} %DIF > 
%DIF -------
%\newtheorem{conjecture}{Conjecture}
...
\DIFdel{%DIFDELCMD < \label{eq:hilbertschmidt}%%%
\|a^{\dag}G_{+-}a^{\dag}P_{\leq n}\| }& \DIFdel{\leq (n+2)\|G_{+-}\|_{2}, }& \DIFdel{\|aG_{+-}aP_{\leq n}\| }& \DIFdel{\leq n \|G_{+-}\|_{2}.
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:hilbertschmidt}
\|a^{\dag}G_{+-}a^{\dag}P_{\leq n}\| }& \DIFadd{\leq (n+2)\|G_{+-}\|_{2}, }& \DIFadd{\|aG_{-+}aP_{\leq n}\| }& \DIFadd{\leq n \|G_{-+}\|_{2}.
}\end{align}\DIFaddend 
...
\DIFaddbegin \begin{rem}[Re-centring: an alternative comparison, carried along]
\label{rem:recentring}
\DIFadd{The Daubechies scaling functions are markedly asymmetric --- $\supp(\!\ _{K}s) = [0,2K-1]$ --- and their first moment
}\begin{align}
\DIFadd{\label{eq:comdef}
\mu(s) }& \DIFadd{:= \int_{\R}x\!\ s(x)\!\ dx = \tfrac{1}{\sqrt{2}}\sum_{n\in\Z}n\!\ h_{n}, }& \DIFadd{\hat{s}(\xi) }& \DIFadd{= 1-i\mu(s)\xi+\cO(\xi^{2}),
}\end{align}
\DIFadd{is correspondingly far from zero: $\mu(\!\ _{4}s)\approx5.995$ and $\mu(\!\ _{10}s)\approx16.869$. Since $R^{N}_{\infty}$ sends the degree of freedom at the lattice site $x\in\Lambda_{N}$ to $s^{(\vep_{N})}(\!\ .\!\ -x)$, whose centre of mass sits at $x+\vep_{N}\mu(s)$ rather than at $x$, the wavelet renormalization group carries a built-in offset of size $\vep_{N}\mu(s)$ between the lattice and the continuum.
}

\DIFadd{This offset is invisible in every convergence statement below --- it tends to $0$ --- but it is the leading source of error in every }\emph{\DIFadd{rate}} \DIFadd{obtained along the wavelet route, see \eqref{eq:comphase} and Remarks \ref{rem:KSconvrate} and \ref{rem:errorrates}. One may remove it by comparing the renormalized lattice quantities not with the continuum objects themselves but with their translates by $a_{N}:=\vep_{N}\mu(s)$. Writing $(T_{a}\xi)(y):=\xi(y-a)$, so that $T_{a}e_{m}=e^{-ima}e_{m}$, one has
}\begin{align}
\DIFadd{\label{eq:recentringphase}
T_{a}\!\ \ell_{\pm,k}\!\ T_{a}^{*} }& \DIFadd{= e^{\pm ika}\!\ \ell_{\pm,k}, }& \DIFadd{T_{a}C }& \DIFadd{= CT_{a}, }& \DIFadd{T_{a}^{*}T_{a} }& \DIFadd{= \1,
}\end{align}
\DIFadd{so that on the $k$-th mode the re-centring amounts to the single phase $e^{\pm i\mu(s)\vep_{N}k}\rightarrow1$. It costs nothing and gains one order in $\vep_{N}$ throughout.
}

\DIFadd{We deliberately do }\emph{\DIFadd{not}} \DIFadd{absorb this into Definition \ref{def:waveletrg}, for two reasons. First, the re-centred maps $T_{-a_{N}}R^{N}_{\infty}$ are no longer asymptotically compatible in the sense of Proposition \ref{prop:waveletrg}\!\ (4): $T_{-a_{N+1}}R^{N+1}_{\infty}\circ R^{N}_{N+1} = T_{-a_{N+1}}R^{N}_{\infty}\neq T_{-a_{N}}R^{N}_{\infty}$ because $a_{N+1}=\tfrac{1}{2}a_{N}$, so they do not define an inductive system and the whole scaling-limit construction of Section \ref{sec:ffscaling} would have to be redone. Second, one cannot instead re-centre the scaling function: $s(\!\ .\!\ +\mu(s))$ is again orthonormal, but it satisfies a scaling equation with the non-integer shifts $n-\mu(s)$ and is therefore not the scaling function of a dyadic multiresolution analysis. Re-centring is thus a }\emph{\DIFadd{comparison convention}}\DIFadd{, not a redefinition, and we carry it along as an option, invoking it only where rates are stated.
}

\DIFadd{Finally, note that the momentum-cutoff renormalization group of Definition \ref{def:momrg} needs no such convention: no scaling function enters its symbol, the multiplier $\chi_{\Gamma_{N}}$ contributing no first moment, and correspondingly the error \eqref{eq:momrgestimates} is $\cO(\vep_{N}^{2})$ from the outset. This is one further, and quite concrete, advantage of that group over the wavelet one, alongside the invariance of $\cD_{\std}$ under the Hardy projections noted in Remark \ref{rem:moeberror}.
}\end{rem}

\DIFaddend Clearly, the difference between $R^{N}_{\infty}$ and $S^{N}_{\infty}$ is the geometrical scaling factor $\vep_{N}^{\frac{1}{2}}$ respectively $\vep_{N}$ which reflects the fact that fermions are half densities while generators are densities.
...
\DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:convergence}%%%
\left|\chi_{[-2^{M}\pi,2^{M}\pi)}(l)\!\prod_{n=1}^{M}\!{}_{K}m_{0}(2^{-n}l)\right| }& \DIFdel{\!=\!\chi_{[-2^{M}\pi,2^{M}\pi)}(l)\max\{e^{C}\!,\!\pi^{-\cK_{j}}\}\!\!\left(\tfrac{|\sin(\frac{1}{2}l)|}{|l|}\right)^{\!K}\!(1+|l|)^{\cK_{j}},
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:convergence}
\left|\chi_{[-2^{M}\pi,2^{M}\pi)}(l)\!\prod_{n=1}^{M}\!{}_{K}m_{0}(2^{-n}l)\right| }& \DIFadd{\!\leq\!\chi_{[-2^{M}\pi,2^{M}\pi)}(l)\!\ e^{C}\!\!\left(\tfrac{|\sin(\frac{1}{2}l)|}{|l|}\right)^{\!K}\!(1+|l|)^{\cK_{j}},
}\end{align}\DIFaddend 
...
\begin{itemize}
	...
	\DIFdelbegin \begin{align*}
	\DIFdel{\prod_{m=0}^{j^{-1}M-1}|}{}\DIFdel{_{K}\cL_{j}(2^{-(mj+1)}l)| }& \DIFdel{\leq q_{j}^{j^{-1}M} = 2^{M\cK_{j}}\leq \pi^{-\cK_{j}}(1+|l|)^{\cK_{j}}.
	}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align*}
	\DIFadd{\prod_{m=0}^{j^{-1}M-1}|}{}\DIFadd{_{K}\cL_{j}(2^{-(mj+1)}l)| }& \DIFadd{\leq q_{j}^{j^{-1}M} = 2^{M\cK_{j}}\leq (1+|l|)^{\cK_{j}}\leq e^{C}(1+|l|)^{\cK_{j}},
	}\end{align*}\DIFaddend 
\end{itemize}
\DIFaddbegin \DIFadd{where we used $|l|>2^{M}$ in the second and $C>0$ in the third step.
}\DIFaddend Thus, we have \DIFdelbegin \DIFdel{$\prod_{n=1}^{M}|{}_{K}\cL(2^{-n}l)| = \prod_{m=0}^{j^{-1}M-1}|{}_{K}\cL_{j}(2^{-(mj+1)}l)| \leq \max\{e^{C},\pi^{-\cK_{j}}\}(1+|l|)^{\cK_{j}}$ }\DIFdelend \DIFaddbegin \DIFadd{$\prod_{n=1}^{M}|{}_{K}\cL(2^{-n}l)| = \prod_{m=0}^{j^{-1}M-1}|{}_{K}\cL_{j}(2^{-(mj+1)}l)| \leq e^{C}(1+|l|)^{\cK_{j}}$ }\DIFaddend for $|l|\leq 2^{M}\pi$. In combination with the formula $\prod_{n=1}^{M}\Big|\tfrac{1-e^{-i2^{-n}l}}{2}\Big| = \prod_{n=1}^{M}|\cos(\tfrac{1}{2}2^{-n}l)| = \Big|\tfrac{2\sin(\frac{1}{2}l)}{2^{M+1}\sin(2^{-(M+1)l})}\Big|$ the result follows.
\end{proof}
\DIFaddbegin \DIFadd{For later use we record the form in which \eqref{eq:convergence} will be applied. Since $|\sin(\tfrac{1}{2}l)|/|l|\leq\min\{\tfrac{1}{2},|l|^{-1}\}\leq2(1+|l|)^{-1}$, and since $m_{0}$ is $2\pi$-periodic, Lemma \ref{lem:convergence} yields:
}\begin{cor}
\label{cor:convergence}
\DIFadd{Let $s=\!\ _{K}s$ be as in Lemma \ref{lem:convergence} and put $P_{J}(l):=\prod_{n=1}^{J}|{}_{K}m_{0}(2^{-n}l)|$, $J\in\N_{0}$. Then $P_{J}\leq1$, $P_{J}$ is $2^{J+1}\pi$-periodic, and there is $C_{K}>0$, }\emph{\DIFadd{independent of $J$}}\DIFadd{, with:
}\begin{align}
\DIFadd{\label{eq:convergencecor}
P_{J}(l) }& \DIFadd{\leq C_{K}(1+|l|)^{-K+\cK_{j}}, }& \DIFadd{|l| }& \DIFadd{\leq2^{J}\pi.
}\end{align}
\DIFadd{Moreover, $|\hat{s}(2^{-J}l)|\!\ P_{J}(l) = |\hat{s}(l)|$ for all $l\in\R$ by \eqref{eq:infprod}.
}\end{cor}
\DIFadd{We stress that the restriction $|l|\leq2^{J}\pi$ in \eqref{eq:convergencecor} is essential: outside this fundamental domain $P_{J}$ merely repeats periodically and does not decay at all. For this reason the dominated-convergence arguments in Section \ref{sec:KSconvwav} are run over the Brillouin zone $\Gamma_{N,{\color{blue}\pm}}$ and not over all of $\Gamma_{\infty,{\color{blue}\pm}}$.
}\DIFaddend It is known that $\cK_{2} \leq (K-1)(2-\tfrac{3}{4}\log_{2}(3)$ \cite{DaubechiesTenLecturesOn} implying:
\begin{align}
\label{eq:decay}
...
\DIFaddbegin \DIFadd{Throughout the article the phrase ``sufficiently regular scaling function'' refers to the following quantitative hypothesis, which we make explicit once and for all because the various convergence statements below require different amounts of regularity.
}

\begin{defn}[Regularity index]
\label{def:regularity}
\DIFadd{For a compactly supported orthonormal Daubechies scaling function $s=\!\ _{K}s$ we write
}\begin{align}
\DIFadd{\label{eq:sobolevindex}
\sigma_{K} }& \DIFadd{:= \sup\{\sigma\geq0\!\ :\!\ s\in H^{\sigma}(\R)\} = \sup\Big\{\sigma\geq0\!\ :\!\ \int_{\R}\!(1+l^{2})^{\sigma}|\hat{s}(l)|^{2}dl<\infty\Big\}
}\end{align}
\DIFadd{for its Sobolev exponent. We say that $s$ is }\emph{\DIFadd{$\rho$-regular}} \DIFadd{if $\sigma_{K}>\rho$. By Lemma \ref{lem:decay} one always has $\sigma_{K}\geq K-\cK-\tfrac{1}{2}$, but $\sigma_{K}$ is the sharper quantity and it is the one that controls all the $\ltwo$-sums appearing below.
}\end{defn}

\begin{rem}[Which $K$ is needed]
\label{rem:regularityvalues}
\DIFadd{The Sobolev exponents of the Daubechies family are classical (see \mbox{%DIFAUXCMD
\cite[Sec.~7.1]{DaubechiesTenLecturesOn}}\hskip0pt%DIFAUXCMD
) and grow asymptotically like $\sim0.2\!\ K$; numerically,
}\begin{center}
\begin{tabular}{l|cccccccccc}
\DIFadd{$K$ }& \DIFadd{2 }& \DIFadd{3 }& \DIFadd{4 }& \DIFadd{5 }& \DIFadd{6 }& \DIFadd{7 }& \DIFadd{8 }& \DIFadd{9 }& \DIFadd{10 }& \DIFadd{11 }\\ \hline
\DIFadd{$\sigma_{K}$ }& \DIFadd{$1.000$ }& \DIFadd{$1.415$ }& \DIFadd{$1.776$ }& \DIFadd{$2.098$ }& \DIFadd{$2.390$ }& \DIFadd{$2.661$ }& \DIFadd{$2.917$ }& \DIFadd{$3.165$ }& \DIFadd{$3.406$ }& \DIFadd{$3.641$
}\end{tabular}
\end{center}
\DIFadd{The thresholds that occur in Section \ref{sec:approxconf} are the following. }\emph{\DIFadd{(i)}} \DIFadd{$1$-regularity, i.e.~$s\in h^{1}$, is needed for $\cD_{W}$ to be contained in the domain of $\ell_{\pm,k}$, hence for Lemma \ref{lem:KSconv} to have content; note that $_{2}s$ is exactly borderline ($\sigma_{2}=1$), so that $K\geq3$ is the honest minimum. }\emph{\DIFadd{(ii)}} \DIFadd{$\tfrac{3}{2}$-regularity, i.e.~$K\geq4$, is needed in Lemma \ref{lem:KSconvsmeared}. }\emph{\DIFadd{(iii)}} \DIFadd{$(1+\delta)$-regularity is needed for the error estimate \eqref{eq:errorscaling}, both for the finiteness of the Sobolev norm $\|\hat{s}(\vep_{M}\!\ .\!\ )\|_{h^{1+\delta}}$ occurring in \eqref{eq:uniformestimateFock} and for the supremum defining $\textup{Error}^{2}(\delta,L,k,N)$ to decay at the rate $2^{-2\delta N}$: the value $\delta=1$ requires $\sigma_{K}>2$, i.e.~$K\geq5$, and the value $\delta=2$ requires $\sigma_{K}>3$, i.e.~$K\geq9$. We have refrained from optimising these thresholds. Two further thresholds are added by Section \ref{sec:KSsmeared}: $\rho>\tfrac{5}{2}$ with $K\beta>\tfrac{3}{2}$ in Theorem \ref{thm:KSreconstruction}, i.e.~$K\geq9$, and $K\beta\geq\tfrac{5}{2}$ for its rate, i.e.~$K\geq10$.
}

\DIFadd{It is worth separating the }\emph{\DIFadd{two}} \DIFadd{decay exponents that are in play, because they are not interchangeable and each has its own natural r\^ole. The }\emph{\DIFadd{pointwise}} \DIFadd{exponent is the largest $\rho$ with $|\hat{s}(l)|\leq C(1+|l|)^{-\rho}$, which controls $|\hat{s}|$ at a single momentum and which Lemma \ref{lem:decay} bounds below by $K-\cK$ via \eqref{eq:decaysf}; the }\emph{\DIFadd{$L^{2}$}} \DIFadd{exponent is $\sigma_{K}$ of \eqref{eq:sobolevindex}, controlling weighted sums $\sum_{l}\langle l\rangle^{2\sigma}|\hat{s}(l)|^{2}$. Numerically they are close, with $\rho$ slightly the larger:
}\begin{center}
\begin{tabular}{l|cccccccccc}
\DIFadd{$K$ }& \DIFadd{2 }& \DIFadd{3 }& \DIFadd{4 }& \DIFadd{5 }& \DIFadd{6 }& \DIFadd{7 }& \DIFadd{8 }& \DIFadd{9 }& \DIFadd{10 }& \DIFadd{11 }\\ \hline
\DIFadd{$\sigma_{K}$ }& \DIFadd{$1.000$ }& \DIFadd{$1.415$ }& \DIFadd{$1.776$ }& \DIFadd{$2.098$ }& \DIFadd{$2.390$ }& \DIFadd{$2.661$ }& \DIFadd{$2.917$ }& \DIFadd{$3.165$ }& \DIFadd{$3.406$ }& \DIFadd{$3.641$ }\\
\DIFadd{$\rho$ (measured) }& \DIFadd{$1.336$ }& \DIFadd{$1.634$ }& \DIFadd{$1.910$ }& \DIFadd{$2.174$ }& \DIFadd{$2.431$ }& \DIFadd{$2.681$ }& \DIFadd{$2.927$ }& \DIFadd{$3.170$ }& \DIFadd{$3.409$ }& \DIFadd{$3.646$
}\end{tabular}
\end{center}
\DIFadd{so the implication $\sigma_{K}\geq\rho-\tfrac{1}{2}$ of Definition \ref{def:regularity} gives away half a derivative that is not actually lost --- a discrepancy which matters only at small $K$. As a rule: $\rho$ is the right tool wherever a single value of $\hat{s}$ must be bounded, as in the uniform majorant of Lemma \ref{lem:unifmaj}, in the Brillouin-zone/tail split of \eqref{eq:convergencecor}, and in the aliasing estimate \eqref{eq:aliasingmass}; $\sigma_{K}$ is the right tool wherever a sum against a Sobolev weight must converge, as in every statement of the form ``$s$ is $\rho$-regular'' listed above. Where a result needs both --- Lemma \ref{lem:loopjackson} is the clearest case --- the two enter through different factors and cannot be traded for one another.
}\end{rem}

\DIFadd{The following lemma is the technical heart of the convergence proof in Section \ref{sec:KSconvwav}. It provides a majorant for the products $\hat{s}(2^{-J}l-h)P_{J}(l)$ that is }\emph{\DIFadd{uniform in $J$}} \DIFadd{--- something that neither factor supplies on its own: $\hat{s}(2^{-J}\cdot)$ decays only on the scale $2^{J}$, and $P_{J}$ decays only inside its fundamental domain. The point is that the two decays interlock, because $P_{J}$ can be of order one outside $[-2^{J}\pi,2^{J}\pi)$ only near the multiples of $2^{J+1}\pi$, and there $\hat{s}(2^{-J}l-h)$ sits close to a zero of order $K$ of $\hat{s}$.
}

\begin{lemma}[Uniform majorant]
\label{lem:unifmaj}
\DIFadd{Let $s=\!\ _{K}s$ be a compactly supported orthonormal Daubechies scaling function, let $j\in\N$ and put $\rho:=K-\cK_{j}>0$. There is a constant $C=C(K,j)$ such that
}\begin{align}
\DIFadd{\label{eq:unifmaj}
|\hat{s}(2^{-J}l-h)|\!\ P_{J}(l) }& \DIFadd{\leq C\big(1+2^{J\rho}|h|^{K}\big)(1+|l|)^{-\rho},
}\end{align}
\DIFadd{for all $J\in\N_{0}$, all $l\in\R$ and all $h\in\R$ with $|h|\leq\tfrac{\pi}{2}$.
}\end{lemma}
\begin{proof}
\DIFadd{We use two consequences of Lemma \ref{lem:convergence}. First, \eqref{eq:convergencecor}, i.e.
}\begin{align}
\DIFadd{\label{eq:unifmajP}
P_{J}(l) }& \DIFadd{\leq C_{1}(1+|l|)^{-\rho}, }& \DIFadd{|l| }& \DIFadd{\leq2^{J}\pi,
}\end{align}
\DIFadd{together with $P_{J}\leq1$ and the $2^{J+1}\pi$-periodicity of $P_{J}$ (Corollary \ref{cor:convergence}). Second, inserting $_{K}m_{0}(\xi)=(\tfrac{1+e^{-i\xi}}{2})^{K}{}_{K}\cL(\xi)$ into $\hat{s}=\prod_{n\geq1}{}_{K}m_{0}(2^{-n}\!\ \cdot\!\ )$ and using $\prod_{n\geq1}\tfrac{1+e^{-i2^{-n}\xi}}{2} = e^{-i\frac{\xi}{2}}\tfrac{\sin(\frac{\xi}{2})}{\frac{\xi}{2}}$ gives the factorisation
}\begin{align}
\DIFadd{\label{eq:shatfactor}
\hat{s}(\xi) }& \DIFadd{= \Big(e^{-i\frac{\xi}{2}}\tfrac{\sin(\frac{\xi}{2})}{\frac{\xi}{2}}\Big)^{K}\Lambda(\xi), }& \DIFadd{\Lambda(\xi) }& \DIFadd{:= \prod_{n\geq1}{}_{K}\cL(2^{-n}\xi),
}\end{align}
\DIFadd{and letting $M\rightarrow\infty$ in the estimate $\prod_{n=1}^{M}|{}_{K}\cL(2^{-n}l)|\leq e^{C}(1+|l|)^{\cK_{j}}$, $|l|\leq2^{M}\pi$, established in the proof of Lemma \ref{lem:convergence}, yields $|\Lambda(\xi)|\leq e^{C}(1+|\xi|)^{\cK_{j}}$ for all $\xi\in\R$. In particular $\hat{s}$ vanishes to order exactly $K$ at every $2\pi\nu$, $\nu\neq0$, and, since $|\sin(\pi\nu+\tfrac{u}{2})|=|\sin(\tfrac{u}{2})|\leq\min\{1,\tfrac{|u|}{2}\}$,
}\begin{align}
\DIFadd{\label{eq:shatzero}
|\hat{s}(2\pi\nu+u)| }& \DIFadd{\leq e^{C}\tfrac{\min\{1,\frac{|u|}{2}\}^{K}}{|\pi\nu+\frac{u}{2}|^{K}}(1+2\pi|\nu|+|u|)^{\cK_{j}}, }& \DIFadd{\nu }& \DIFadd{\in\Z,\ u\in\R.
}\end{align}

\emph{\DIFadd{Region (a): $|l|\leq2^{J}\pi$.}} \DIFadd{Then \eqref{eq:unifmajP} and $|\hat{s}|\leq\|s\|_{L^{1}}=1$ give \eqref{eq:unifmaj} with $C=C_{1}$, the bracket on the right being $\geq1$.
}

\emph{\DIFadd{Region (b): $|l|>2^{J}\pi$.}} \DIFadd{Let $\nu\in\Z$ be the nearest integer to $l/(2^{J+1}\pi)$ and put $l':=l-2^{J+1}\pi\nu$, so that $|l'|\leq2^{J}\pi$ and, because $|l|>2^{J}\pi$, $\nu\neq0$. Setting $v:=2^{-J}l'$ and $u:=v-h$ we have $|v|\leq\pi$, $|u|\leq\tfrac{3\pi}{2}$ and
}\begin{align*}
\DIFadd{2^{-J}l }& \DIFadd{= 2\pi\nu+v, }& \DIFadd{2^{-J}l-h }& \DIFadd{= 2\pi\nu+u.
}\end{align*}
\DIFadd{By periodicity and \eqref{eq:unifmajP}, $P_{J}(l)=P_{J}(l')\leq C_{1}(1+|l'|)^{-\rho}$. Since $|u|\leq\tfrac{3\pi}{2}$ we have $|\pi\nu+\tfrac{u}{2}|\geq\pi|\nu|-\tfrac{3\pi}{4}\geq\tfrac{\pi}{4}|\nu|$ for $|\nu|\geq1$, so \eqref{eq:shatzero} gives
}\begin{align*}
\DIFadd{|\hat{s}(2^{-J}l-h)| }& \DIFadd{\leq C_{2}\min\{1,\tfrac{|u|}{2}\}^{K}|\nu|^{-K+\cK_{j}} = C_{2}\min\{1,\tfrac{|u|}{2}\}^{K}|\nu|^{-\rho}.
}\end{align*}
\DIFadd{Moreover $|l'|\leq2^{J}\pi$ implies $2^{J}\pi|\nu|\leq|l|\leq3\cdot2^{J}\pi|\nu|$ and hence $(1+|l|)^{-\rho}\geq(1+3\pi)^{-\rho}(2^{J}|\nu|)^{-\rho}$. It therefore suffices to bound
}\begin{align*}
\DIFadd{\cR }& \DIFadd{:= \min\{1,\tfrac{|u|}{2}\}^{K}\Big(\tfrac{2^{J}}{1+|l'|}\Big)^{\rho}
}\end{align*}
\DIFadd{by a multiple of $1+2^{J\rho}|h|^{K}$, and we distinguish two cases.
}

\emph{\DIFadd{(b1) $|v|\geq2|h|$.}} \DIFadd{Then $\tfrac{1}{2}|v|\leq|u|\leq\tfrac{3}{2}|v|$, so $\min\{1,\tfrac{|u|}{2}\}^{K}\leq(\tfrac{3}{4}|v|)^{K}$. If $2^{J}|v|\geq1$, then $|l'|=2^{J}|v|$ gives $\big(\tfrac{2^{J}}{1+|l'|}\big)^{\rho}\leq|v|^{-\rho}$ and hence $\cR\leq(\tfrac{3}{4})^{K}|v|^{K-\rho}=(\tfrac{3}{4})^{K}|v|^{\cK_{j}}\leq(\tfrac{3}{4})^{K}\pi^{\cK_{j}}$, using $|v|\leq\pi$ and $\cK_{j}\geq0$. If $2^{J}|v|<1$, then $\big(\tfrac{2^{J}}{1+|l'|}\big)^{\rho}\leq2^{J\rho}$ and $|v|^{K}<2^{-JK}$, whence $\cR\leq(\tfrac{3}{4})^{K}2^{-J(K-\rho)}=(\tfrac{3}{4})^{K}2^{-J\cK_{j}}\leq(\tfrac{3}{4})^{K}$.
}

\emph{\DIFadd{(b2) $|v|<2|h|$.}} \DIFadd{Then $|u|\leq|v|+|h|\leq3|h|$ and, using $1+|l'|\geq1$,
}\begin{align*}
\DIFadd{\cR }& \DIFadd{\leq \big(\tfrac{3|h|}{2}\big)^{K}2^{J\rho} \leq \big(\tfrac{3}{2}\big)^{K}2^{J\rho}|h|^{K}.
}\end{align*}
\DIFadd{Collecting the three cases proves \eqref{eq:unifmaj}.
}\end{proof}

\begin{cor}
\label{cor:unifmaj}
\DIFadd{Let $s$ be as in Lemma \ref{lem:unifmaj} and let $M\in\N_{0}$, $k\in\tfrac{\pi}{L}\Z$. Then, for all $N>M$ with $\vep_{N}|k|\leq\tfrac{\pi}{2}$ and all $m\in\Gamma_{\infty,{\color{blue}\pm}}$,
}\begin{align}
\DIFadd{\label{eq:unifmajf}
|\hat{s}(\vep_{N}m)|\!\ P_{N-M}(\vep_{M}(m\!+\!k)) }& \DIFadd{\leq C\big(1+(L|k|)^{K}2^{-M\rho}\big)(1+\vep_{M}|m\!+\!k|)^{-\rho},
}\end{align}
\DIFadd{with $C=C(K,j)$ as in \eqref{eq:unifmaj}. In particular the function
}\begin{align}
\DIFadd{\label{eq:majorant}
g_{k}(M,m) }& \DIFadd{:= C\big(1+(L|k|)^{K}2^{-M\rho}\big)\!\ |m+\tfrac{k}{2}|\!\ (1+\vep_{M}|m+k|)^{-\rho}
}\end{align}
\DIFadd{satisfies $|f^{(N)}_{k}(M,m)|\leq g_{k}(M,m)$ for all $N>M$, and $g_{k}(M,\!\ .\!\ )\in\ltwo(\Gamma_{\infty,{\color{blue}\pm}},(2L)^{-1})$ whenever $\rho>\tfrac{3}{2}$, i.e.~whenever $s$ is $1$-regular in the sense of Definition \ref{def:regularity}.
}\end{cor}
\begin{proof}
\DIFadd{Apply Lemma \ref{lem:unifmaj} with $l=\vep_{M}(m+k)$, $h=\vep_{N}k$ and $J=N-M$, so that $2^{-J}l-h=\vep_{N}m$; then $2^{J\rho}|h|^{K}=(L|k|)^{K}2^{(N-M)\rho-NK}=(L|k|)^{K}2^{-N\cK_{j}-M\rho}\leq(L|k|)^{K}2^{-M\rho}$ because $K-\rho=\cK_{j}\geq0$. The bound on $f^{(N)}_{k}$ follows from $|\cos(\tfrac{1}{4}\vep_{N}k)^{2}\sin(\vep_{N}(m+\tfrac{k}{2}))/\vep_{N}|\leq|m+\tfrac{k}{2}|$, and the square-summability of $g_{k}(M,\!\ .\!\ )$ from $\sum_{m}|m|^{2}(1+\vep_{M}|m|)^{-2\rho}<\infty\Leftrightarrow2\rho-2>1$.
}\end{proof}

\DIFaddend \subsection{Momentum-cutoff renormalization group}
\label{sec:momcut}
...
\begin{lemma}
\label{lem:stateconv}
Let $\{\omega^{(N)}\}_{N\in\N_{0}}$ be a family of quasi-free lattice states, each defined by an operator $0\leq S^{(N)}\leq1$ on $\fh_{N,{\color{blue}\pm}}$ ($CS^{(N)} = (1-S^{(N)})C$ in the self-dual case) \DIFdelbegin \DIFdel{. Assume that for all $N\in\N_{0}$ the kernel of $S^{(N)}$ }\DIFdelend \DIFaddbegin \DIFadd{whose kernel }\DIFaddend is diagonal in momentum space\DIFdelbegin \DIFdel{and converges }\DIFdelend \DIFaddbegin \DIFadd{, $\langle e_{k,i}, S^{(N)}e_{l,j}\rangle_{N} = 2L_{N}\delta_{k,l}S^{(N)}(k)_{ij}$. Assume that the periodic extensions of these kernels converge }\DIFaddend point-wise\DIFaddbegin \DIFadd{,
}\begin{align}
\DIFadd{\label{eq:kernelpointlim}
\lim_{N\rightarrow\infty}S^{(N)}_{\per}(k)_{ij} }& \DIFadd{= S(k)_{ij}, }& \DIFadd{k }& \DIFadd{\in\Gamma_{\infty,{\color{blue}\pm}},\ i,j=1,2,
}\end{align}
\DIFaddend to the kernel of an operator \DIFdelbegin \DIFdel{$0\leq S\leq 1$ }\DIFdelend \DIFaddbegin \DIFadd{$0\leq S\leq1$ on $\fh_{\infty,{\color{blue}\pm}}$ }\DIFaddend ($CS = (1-S)C$), \DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:kernelpointlim}%%%
\lim_{N\rightarrow\infty}S^{(N)}(k)_{ij} }& \DIFdel{= S(k)_{ij}, }& \DIFdel{k\in\Gamma_{N,{\color{blue}\pm}}, i,j=1,2,
}\end{align*}%DIFAUXCMD
\DIFdel{where $\langle e_{k,i}, S^{(N)}e_{l,j}\rangle_{N} = 2L_{N}\delta_{k,l}S^{(N)}(k)_{ij}$ and $\langle e_{k,i}, S e_{l,j}\rangle_{\infty}= 2L\delta_{k,l}S(k)_{ij}$}\DIFdelend \DIFaddbegin \DIFadd{where $\langle e_{k,i}, Se_{l,j}\rangle_{\infty}= 2L\delta_{k,l}S(k)_{ij}$}\DIFaddend . Then, \DIFdelbegin \DIFdel{the renormalization group }\DIFdelend \DIFaddbegin \DIFadd{for the wavelet as well as for the momentum-cutoff renormalization group, and }\emph{\DIFadd{without any regularity assumption on the scaling function $s$}}\DIFadd{, the renormalization group }\DIFaddend flow \eqref{eq:rgflow} of $\omega^{(N)}$ converges \DIFaddbegin \DIFadd{in norm }\DIFaddend to $\omega^{(N)}_{S} = \omega_{S}\circ\alpha^{N}_{\infty}$:
\DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:2pconvnorm}%%%
\lim_{M\rightarrow\infty}\|\omega^{(N)}_{M}-\omega^{(N)}_{S}\| }& \DIFdel{= 0.
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:2pconvnorm}
\lim_{M\rightarrow\infty}\|\omega^{(N)}_{M}-\omega^{(N)}_{S}\| }& \DIFadd{= 0, }& \DIFadd{N }& \DIFadd{\in\N_{0}.
}\end{align}\DIFaddend 
\DIFaddbegin \DIFadd{More precisely, the renormalized symbols
}\begin{align}
\DIFadd{\label{eq:pulledbacksymbols}
\hat{S}^{(N)}_{M} }& \DIFadd{:= R^{N\!\ *}_{N+M}S^{(N+M)}R^{N}_{N+M}, }& \DIFadd{\hat{S}^{(N)}_{\infty} }& \DIFadd{:= R^{N\!\ *}_{\infty}SR^{N}_{\infty},
}\end{align}
\DIFadd{are again momentum-diagonal, and with
}\begin{align}
\DIFadd{\label{eq:symboldist}
\delta_{M}(N) }& \DIFadd{:= \max_{l\in\Gamma_{N,{\color{blue}\pm}}}\big\|\hat{S}^{(N)}_{M}(l)-\hat{S}^{(N)}_{\infty}(l)\big\|
}\end{align}
\DIFadd{one has $\lim_{M\rightarrow\infty}\delta_{M}(N) = 0$ together with the explicit estimates
}\begin{align}
\DIFadd{\label{eq:2pconvmono}
\big|(\omega^{(N)}_{M}-\omega^{(N)}_{S})(a(\xi_{1})\dots a(\xi_{n})a^{\dag}(\eta_{n+1})\dots a^{\dag}(\eta_{2n}))\big| }& \DIFadd{\leq n\!\ \delta_{M}(N)\prod_{q=n+1}^{2n}\|\eta_{q}\|_{N}, }\\
\DIFadd{\label{eq:2pconvnormquant}
\|\omega^{(N)}_{M}-\omega^{(N)}_{S}\| }& \DIFadd{\leq 2L_{N}\!\ \varpi\big(\delta_{M}(N)\big),
}\end{align}
\DIFadd{for all orthonormal $\xi_{1},\dots,\xi_{n}\in\fh_{N,{\color{blue}\pm}}$ and all $\eta_{n+1},\dots,\eta_{2n}\in\fh_{N,{\color{blue}\pm}}$, where $\varpi$ denotes a modulus of continuity for the map $T\mapsto\omega_{T}$ on the compact set $\{T\in M_{2}(\C)\!\ :\!\ 0\leq T\leq1\}$.
}\DIFaddend \end{lemma}
\begin{proof}
\DIFdelbegin \DIFdel{It is sufficient to prove the statement for the two-point functions because the states are quasi-free and the algebras $\fA_{N,{\color{blue}\pm}}$ and $\fB_{N,{\color{blue}\pm}}$ are generated (in norm)the by algebraic span of annihilation and creation operators respectively the Majorana operators. We state the proof only for the complex fermions $\fA_{N,{\color{blue}\pm}}$ as it is completely analogous for the Majorana fermions $\fB_{N,{\color{blue}\pm}}$. By the Cauchy-Schwarz inequality and the properties of the renormalization group we have:
}\begin{align*}
\DIFdel{|(\omega^{(N)}_{M}-\omega^{(N)}_{S})(a(\xi)a^{\dag}(\eta)) }& \DIFdel{= \langle R^{N}_{\infty}(\xi), (R^{N+M}_{\infty}\!S^{(N+M)}\!R^{N}_{N+M}-SR^{N}_{\infty})(\eta)\rangle_{\infty} }\\
& \DIFdel{\leq \|\xi\|_{N}\|(R^{N+M}_{\infty}\!S^{(N+M)}\!R^{N}_{N+M}-SR^{N}_{\infty})(\eta)\|_{\infty} }\\
& \DIFdel{\leq \|\xi\|_{N} \tfrac{1}{2L_{N}}\!\!\!\sum_{l\in\Gamma_{N,{\color{blue}\pm}}}\sum_{j=1,2}|\hat{\eta}^{(j)}_{l}| \|(R^{N+M}_{\infty}\!S^{(N+M)}\!R^{N}_{N+M}\!-\!SR^{N}_{\infty})(e_{l,j})\|_{\infty} }\\
& \DIFdel{\leq \|\xi\|_{N} \|\eta\|_{N} \Big(\tfrac{1}{2L_{N}}\!\!\!\sum_{l\in\Gamma_{N,{\color{blue}\pm}}}\sum_{j=1,2}\!\!\|(R^{N+M}_{\infty}\!S^{(N+M)}\!R^{N}_{N+M}\!-\!SR^{N}_{\infty})(e_{l,j})\|_{\infty}^{2}\Big)^{\!\frac{1}{2}}.
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \emph{\DIFadd{Step 1: the renormalized symbols are convex combinations over aliasing classes.}} \DIFadd{Both renormalization groups act in momentum space by multiplication with a symbol followed by periodic extension, cf.~\eqref{eq:waveletrg1pF} and \eqref{eq:momrg1p}:
}\begin{align*}
\DIFadd{\widehat{R^{N}_{N+M}(\xi)}(k) }& \DIFadd{= \sigma_{M}(k)\hat{\xi}_{\per}(k), }& \DIFadd{\widehat{R^{N}_{\infty}(\xi)}(k) }& \DIFadd{= \sigma_{\infty}(k)\hat{\xi}_{\per}(k),
}\end{align*}\DIFaddend 
\DIFdelbegin \DIFdel{Next, we evaluate the last factor in the last line explicitly:
}\begin{align*}
 & \DIFdel{\tfrac{1}{2L_{N}}\!\!\!\sum_{l\in\Gamma_{N,{\color{blue}\pm}}}\sum_{j=1,2}\!\!\|(R^{N+M}_{\infty}\!S^{(N+M)}\!R^{N}_{N+M}\!-\!SR^{N}_{\infty})(e_{l,j})\|_{\infty}^{2} }\\
 & \DIFdel{= \tfrac{1}{2L_{N}}\!\!\!\sum_{l\in\Gamma_{N,{\color{blue}\pm}}}\sum_{j=1,2}\tfrac{1}{2L}\!\!\!\sum_{k\in\Gamma_{\infty,{\color{blue}\pm}}}\sum_{i=1,2}\!\!|\langle e_{k,i},(R^{N+M}_{\infty}\!S^{(N+M)}\!R^{N}_{N+M}\!-\!SR^{N}_{\infty})(e_{l,j})\rangle_{\infty}|^{2} }\\
 & \DIFdel{= \sum_{l\in\Gamma_{N,{\color{blue}\pm}}}\sum_{k\in\Gamma_{\infty,{\color{blue}\pm}}}\sum_{i,j=1,2}|\hat{s}(\vep_{N}k)|^{2}|(S^{(N+M)}_{\per})(k)_{ij}-S(k)_{ij}|^{2}\delta_{0,\frac{\pi}{L}(k-l)\!\!\!\mod 2L_{N}},
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{with $\sigma_{M}(k) = 2^{\frac{M}{2}}\prod_{m=1}^{M}m_{0}(\vep_{N+m}k)$ and $\sigma_{\infty}(k) = \vep_{N}^{\frac{1}{2}}\hat{s}(\vep_{N}k)$ in the wavelet case, respectively $\sigma_{M}(k) = 2^{\frac{M}{2}}\chi_{\Gamma_{N}}(k)$ and $\sigma_{\infty}(k) = \vep_{N}^{\frac{1}{2}}\chi_{\Gamma_{N}}(k)$ in the momentum-cutoff case. Equivalently, in terms of the plane-wave basis,
}\begin{align}
\DIFadd{\label{eq:waveletrginner}
\langle e_{k,i},R^{N}_{M}(e_{l,j})\rangle_{M} }& \DIFadd{= \delta_{ij}2^{\frac{M-N}{2}}\Big(\prod_{m=1}^{M-N}m_{0}(\vep_{N+m}k)\Big)\sum_{x\in\Lambda_{N}}e^{-i(k-l)x} }\\ \DIFadd{\nonumber
}& \DIFadd{= 2L_{N}\delta_{ij}2^{\frac{M-N}{2}}\Big(\prod_{m=1}^{M-N}m_{0}(\vep_{N+m}k)\Big)\delta_{0,\frac{L}{\pi}(k-l)\!\!\!\!\mod 2L_{N}}, }\\ \DIFadd{\nonumber
\langle e_{k,i},R^{N}_{\infty}(e_{l,j})\rangle_{\infty} }& \DIFadd{= \delta_{ij}\vep_{N}^{\frac{1}{2}}\hat{s}(\vep_{N}k)\sum_{x\in\Lambda_{N}}e^{-i(k-l)x} }\\ \DIFadd{\nonumber
}& \DIFadd{= 2L_{N}\delta_{ij}\vep_{N}^{\frac{1}{2}}\hat{s}(\vep_{N}k)\delta_{0,\frac{L}{\pi}(k-l)\!\!\!\!\mod 2L_{N}},
}\end{align}\DIFaddend 
\DIFdelbegin \DIFdel{where the last line follows from the }\DIFdelend \DIFaddbegin \DIFadd{for all $k\in\Gamma_{\infty,{\color{blue}\pm}}$ and $l\in\Gamma_{N,{\color{blue}\pm}}$, with $\chi_{\Gamma_{N}}(k)$ in place of the products and of $\hat{s}(\vep_{N}k)$ in the momentum-cutoff case. Writing $k\sim l$ for $\tfrac{L}{\pi}(k-l)\equiv0\!\!\mod 2L_{N}$, i.e.~for $k$ in the aliasing class of $l\in\Gamma_{N,{\color{blue}\pm}}$, a direct computation using $L_{N}\vep_{N}=L$ and $L_{N+M}=2^{M}L_{N}$ gives that $\hat{S}^{(N)}_{M}$ and $\hat{S}^{(N)}_{\infty}$ are momentum-diagonal with
}\begin{align}
\DIFadd{\label{eq:convexsymb}
\hat{S}^{(N)}_{M}(l) }& \DIFadd{= \!\!\sum_{k\sim l,\!\ k\in\Gamma_{N+M,{\color{blue}\pm}}}\!\!\!\!\!\!w_{M}(k)\!\ S^{(N+M)}_{\per}(k), }& \DIFadd{\hat{S}^{(N)}_{\infty}(l) }& \DIFadd{= \!\!\sum_{k\sim l,\!\ k\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!\!\!\!\!w_{\infty}(k)\!\ S(k),
}\end{align}
\DIFadd{where $w_{M}(k) := |\prod_{m=1}^{M}m_{0}(\vep_{N+m}k)|^{2}$ and $w_{\infty}(k) := |\hat{s}(\vep_{N}k)|^{2}$ (respectively $w_{M}=w_{\infty}=\chi_{\Gamma_{N}}$). The decisive point is that these weights are }\emph{\DIFadd{probability weights}} \DIFadd{on each aliasing class,
}\begin{align}
\DIFadd{\label{eq:aliasprob}
\sum_{k\sim l}w_{M}(k) }& \DIFadd{= 1 = \sum_{k\sim l}w_{\infty}(k), }& \DIFadd{l }& \DIFadd{\in\Gamma_{N,{\color{blue}\pm}}.
}\end{align}
\DIFadd{Indeed, for $M=1$ the two aliases of $l$ in $\Gamma_{N+1,{\color{blue}\pm}}$ have arguments $\vep_{N+1}k$ and $\vep_{N+1}k+\pi$, so that \eqref{eq:aliasprob} is the defining relation $|m_{0}(\xi)|^{2}+|m_{0}(\xi+\pi)|^{2}=1$ of a low-pass filter, and the general case follows by iteration; for $w_{\infty}$ it is the orthonormality relation $\sum_{n\in\Z}|\hat{s}(\xi+2\pi n)|^{2}=1$ of the scaling function; and in the momentum-cutoff case it merely says that each aliasing class meets the Brillouin zone $\Gamma_{N,{\color{blue}\pm}}$ exactly once. In particular $0\leq\hat{S}^{(N)}_{M},\hat{S}^{(N)}_{\infty}\leq1$ automatically, and }\emph{\DIFadd{no decay estimate on $\hat{s}$ is required at any point of the argument.}}

\emph{\DIFadd{Step 2: $\delta_{M}(N)\rightarrow0$.}} \DIFadd{Fix $l\in\Gamma_{N,{\color{blue}\pm}}$ and denote by $\mu^{l}_{M}$, $\mu^{l}_{\infty}$ the probability measures on the aliasing class of $l$ defined by $w_{M}$, $w_{\infty}$. By the }\DIFaddend scaling equation in momentum space \eqref{eq:infprod}, \DIFdelbegin \DIFdel{the inner products, }\begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:waveletrginner}%%%
\langle e_{k,i},R^{N}_{M}(e_{l,j})\rangle_{M} }& \DIFdel{= \delta_{ij}2^{\frac{M-N}{2}}\Big(\prod_{m=1}^{M-N}m_{0}(\vep_{N+m}k)\Big)\sum_{x\in\Lambda_{N}}e^{-i(k-l)x} }\\ \DIFdel{\nonumber
}& \DIFdel{= 2L_{N}\delta_{ij}2^{\frac{M-N}{2}}\Big(\prod_{m=1}^{M-N}m_{0}(\vep_{N+m}k)\Big)\delta_{0,\frac{L}{\pi}(k-l)\!\!\!\!\mod 2L_{N}}, }\\ \DIFdel{\nonumber
\langle e_{k,i},R^{N}_{\infty}(e_{l,j})\rangle_{\infty} }& \DIFdel{= \delta_{ij}\vep_{N}^{\frac{1}{2}}\hat{s}(\vep_{N}k)\sum_{x\in\Lambda_{N}}e^{-i(k-l)x} }\\ \DIFdel{\nonumber
}& \DIFdel{= 2L_{N}\delta_{ij}\vep_{N}^{\frac{1}{2}}\hat{s}(\vep_{N}k)\delta_{0,\frac{L}{\pi}(k-l)\!\!\!\!\mod 2L_{N}},
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{$w_{M}(k)\rightarrow w_{\infty}(k)$ point-wise as $M\rightarrow\infty$, and since both are probability weights, Scheffé's lemma gives convergence in total variation, $\|\mu^{l}_{M}-\mu^{l}_{\infty}\|_{TV}\rightarrow0$. Writing $\Delta_{M}(k) := S^{(N+M)}_{\per}(k)-S(k)$, which satisfies $\|\Delta_{M}(k)\|\leq2$ and $\Delta_{M}(k)\rightarrow0$ by \eqref{eq:kernelpointlim}, we obtain from \eqref{eq:convexsymb}
}\begin{align*}
\DIFadd{\big\|\hat{S}^{(N)}_{M}(l)-\hat{S}^{(N)}_{\infty}(l)\big\| }& \DIFadd{\leq \sum_{k\sim l}w_{M}(k)\|\Delta_{M}(k)\| + \sum_{k\sim l}|w_{M}(k)-w_{\infty}(k)|\!\ \|S(k)\| }\\
& \DIFadd{\leq \sum_{k\sim l}w_{\infty}(k)\|\Delta_{M}(k)\| + 3\|\mu^{l}_{M}-\mu^{l}_{\infty}\|_{TV},
}\end{align*}\DIFaddend 
\DIFdelbegin \DIFdel{for all $k\in\Gamma_{\infty,{\color{blue}\pm}}$ and $l\in\Gamma_{N,{\color{blue}\pm}}$, }\DIFdelend \DIFaddbegin \DIFadd{where we used $\|S(k)\|\leq1$ }\DIFaddend and \DIFdelbegin \DIFdel{the periodic extension of $S^{(N+M)}(k)_{ij}$ to $k\in\Gamma_{\infty,{\color{blue}\pm}}$. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{As a consequence of the decay estimate on $\hat{s}$ in Lemma \ref{lem:decay}, Lebesgue's dominated convergence theorem,
}\DIFdelend \DIFaddbegin \DIFadd{$\|\Delta_{M}\|\leq2$. The first term tends to $0$ by dominated convergence with respect to the }\emph{\DIFadd{fixed}} \DIFadd{probability measure $\mu^{l}_{\infty}$, with constant majorant $2$; the second tends to $0$ by Scheffé. As $\Gamma_{N,{\color{blue}\pm}}$ is finite, $\delta_{M}(N) = \max_{l}\|\cdot\|\rightarrow0$.
}

\emph{\DIFadd{Step 3: the estimate \eqref{eq:2pconvmono}.}} \DIFadd{By \eqref{eq:carqfs} the two sides are determinants, $\det\hat{M}_{r}$ with $(\hat{M}_{1})_{pq} = \langle\xi_{p},(1-\hat{S}^{(N)}_{M})\eta_{n+q}\rangle_{N}$ }\DIFaddend and \DIFdelbegin \DIFdel{the fact that the states }\DIFdelend \DIFaddbegin \DIFadd{$(\hat{M}_{2})_{pq} = \langle\xi_{p},(1-\hat{S}^{(N)}_{\infty})\eta_{n+q}\rangle_{N}$. Telescoping over the columns and applying Hadamard's inequality to each of the $n$ resulting determinants,
}\begin{align*}
\DIFadd{|\det\hat{M}_{1}-\det\hat{M}_{2}| }& \DIFadd{\leq \sum_{p=1}^{n}\big\|(\hat{M}_{1}-\hat{M}_{2})_{\cdot p}\big\|_{2}\prod_{q<p}\big\|(\hat{M}_{1})_{\cdot q}\big\|_{2}\prod_{q>p}\big\|(\hat{M}_{2})_{\cdot q}\big\|_{2},
}\end{align*}
\DIFadd{and, the $\xi_{p}$ being orthonormal, $\|(\hat{M}_{r})_{\cdot q}\|_{2}\leq\|(1-\hat{S})\eta_{n+q}\|_{N}\leq\|\eta_{n+q}\|_{N}$ as well as $\|(\hat{M}_{1}-\hat{M}_{2})_{\cdot p}\|_{2}\leq\|\hat{S}^{(N)}_{M}-\hat{S}^{(N)}_{\infty}\|\!\ \|\eta_{n+p}\|_{N} = \delta_{M}(N)\|\eta_{n+p}\|_{N}$, the last equality because both symbols are momentum-diagonal. This gives \eqref{eq:2pconvmono}. We stress that the bound is independent of $L_{N}$, i.e.~of the number of lattice sites.
}

\emph{\DIFadd{Step 4: the norm estimate \eqref{eq:2pconvnormquant}.}} \DIFadd{Since $\hat{S}^{(N)}_{M}$ and $\hat{S}^{(N)}_{\infty}$ }\DIFaddend are \DIFaddbegin \DIFadd{momentum-diagonal, both states factorize over the momentum modes: under the isomorphism $\fA_{N,{\color{blue}\pm}}\cong\bigotimes_{l\in\Gamma_{N,{\color{blue}\pm}}}M_{4}(\C)$ of Section \ref{sec:car} they are product states with factors the }\DIFaddend quasi-free \DIFdelbegin \DIFdel{\eqref{eq:carqfs}, we know that $\omega^{(N)}$ converges to $\omega^{(N)}_{S}$ in the weak$^{*}$ sense. Since the algebra $\fA_{N,{\color{blue}\pm}}$ is finite dimensional, we know that this is equivalent to strong convergence, hence the result follows. }\DIFdelend \DIFaddbegin \DIFadd{states of the $2\times2$ symbols $\hat{S}(l)$. The trace distance is subadditive on product states, so
}\begin{align*}
\DIFadd{\|\omega^{(N)}_{M}-\omega^{(N)}_{S}\| }& \DIFadd{\leq \sum_{l\in\Gamma_{N,{\color{blue}\pm}}}\big\|\omega_{\hat{S}^{(N)}_{M}(l)}-\omega_{\hat{S}^{(N)}_{\infty}(l)}\big\|.
}\end{align*}
\DIFadd{On the }\emph{\DIFadd{fixed}} \DIFadd{finite-dimensional algebra $\fA_{\CAR}(\C^{2})\cong M_{4}(\C)$ the map $T\mapsto\omega_{T}$ is continuous, because by \eqref{eq:carqfs} all moments of $\omega_{T}$ are polynomials in the entries of $T$ and a functional on a finite-dimensional algebra depends linearly on finitely many of them. Being continuous on the compact set $\{0\leq T\leq1\}$, it is uniformly continuous with some modulus $\varpi$, and \eqref{eq:2pconvnormquant} follows. Together with Step 2 this proves \eqref{eq:2pconvnorm}.
}\DIFaddend \end{proof}

\begin{cor}
\label{cor:stateconv}
\DIFdelbegin \DIFdel{Assuming }\DIFdelend \DIFaddbegin \DIFadd{Assume }\DIFaddend the renormalization condition \eqref{eq:rgcond}, \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{and, in the Ramond sector with $m=0$, the convention $\sign(0)=0$ of \eqref{eq:Fmassless}. Then the }\DIFaddend renormalization group flow of the family of lattice vacua $\omega^{(N)}_{0,{\color{blue}\pm}}$ converges to $\omega^{(N)}_{\infty,{\color{blue}\pm}}$ (in norm) at any scale $N$. \DIFaddbegin \DIFadd{In the massless case one has, moreover, the exact identity
}\begin{align}
\DIFadd{\label{eq:vacsymbolerror}
\big\|S^{(N)}_{0}(k)-S(k)\big\| }& \DIFadd{= \big|\sin(\tfrac{1}{4}\vep_{N}k)\big|, }& \DIFadd{k }& \DIFadd{\in\Gamma_{\infty,{\color{blue}\pm}},
}\end{align}
\DIFadd{and consequently, with $\delta_{M}(N)$ as in \eqref{eq:symboldist},
}\begin{align}
\DIFadd{\label{eq:vacrate}
\delta_{M}(N) }& \DIFadd{\leq \tfrac{C_{s}}{\pi}\!\ 2^{-M} + 3\max_{l\in\Gamma_{N,{\color{blue}\pm}}}\|\mu^{l}_{M}-\mu^{l}_{\infty}\|_{TV}, }& \DIFadd{C_{s} }& \DIFadd{:= \sup_{u\in[-\pi,\pi)}\sum_{n\in\Z}|\hat{s}(u+2\pi n)|^{2}|u+2\pi n| ,
}\end{align}
\DIFadd{where $C_{s}<\infty$ if and only if $\int_{\R}|\xi||\hat{s}(\xi)|^{2}d\xi<\infty$, i.e.~if and only if $s\in H^{\frac{1}{2}}(\R)$. Both bounds in \eqref{eq:vacrate} are independent of the scale $N$.
}\DIFaddend \end{cor}
\begin{proof}
\DIFdelbegin \DIFdel{The renormalization condition implies }\DIFdelend \DIFaddbegin \DIFadd{For $k\in\Gamma_{\infty,{\color{blue}\pm}}$ with $(m,k)\neq(0,0)$ the renormalization condition gives $n_{\lambda_{N}}(k) = \vep_{N}n_{m}(k)+o(\vep_{N})$ and $\omega_{\lambda_{N}}(k) = \vep_{N}\omega_{m}(k)+o(\vep_{N})$ with $\omega_{m}(k)>0$, whence }\DIFaddend the point-wise convergence \DIFaddbegin \DIFadd{$S^{(N)}_{0}(k) = \1-P^{(+)}_{\lambda_{N}}(k)\rightarrow\1-P^{(+)}_{m}(k) = S(k)$ required in \eqref{eq:kernelpointlim}; for $m=0$ and $k=0$ this is the convention $\sign(0)=0$, see the discussion below. Lemma \ref{lem:stateconv} then applies. For \eqref{eq:vacsymbolerror}, subtract \eqref{eq:masslesslimproj} from \eqref{eq:Fmassless}:
}\begin{align*}
\DIFadd{S^{(N)}_{0}(k)-S(k) }& \DIFadd{= \tfrac{\sign(k)}{2}\big(\sin(\tfrac{1}{2}\vep_{N}k)\sigma_{x}-(\cos(\tfrac{1}{2}\vep_{N}k)-1)\sigma_{y}\big),
}\end{align*}
\DIFadd{whose operator norm is $\tfrac{1}{2}(\sin^{2}(\tfrac{1}{2}\vep_{N}k)+(\cos(\tfrac{1}{2}\vep_{N}k)-1)^{2})^{\frac{1}{2}} = |\sin(\tfrac{1}{4}\vep_{N}k)|$. Since $0\leq S^{(N)}_{0},S\leq1$ forces $\|S^{(N)}_{0}(k)-S(k)\|\leq1$, and since $|\sin(x)|\leq|x|$, \eqref{eq:vacsymbolerror} yields the scale-free bound $\|S^{(N+M)}_{0,\per}(k)-S(k)\|\leq\tfrac{1}{\pi}\vep_{N+M}|k|$, valid both inside and outside the Brillouin zone of $\Gamma_{N+M,{\color{blue}\pm}}$. Inserting this into the first term of Step 2 of the proof of Lemma \ref{lem:stateconv} and substituting $u=\vep_{N}l$, $\vep_{N}k = u+2\pi n$ gives
}\begin{align*}
\DIFadd{\sum_{k\sim l}w_{\infty}(k)\|S^{(N+M)}_{0,\per}(k)-S(k)\| }& \DIFadd{\leq \tfrac{\vep_{N+M}}{\pi}\sum_{k\sim l}|\hat{s}(\vep_{N}k)|^{2}|k| = \tfrac{\vep_{N+M}}{\pi\vep_{N}}\sum_{n\in\Z}|\hat{s}(u+2\pi n)|^{2}|u+2\pi n| \leq \tfrac{C_{s}}{\pi}2^{-M},
}\end{align*}
\DIFadd{which is \eqref{eq:vacrate}. The characterisation of $C_{s}<\infty$ is immediate from $\Gamma_{\infty,{\color{blue}\pm}}$-periodisation }\DIFaddend of \DIFdelbegin \DIFdel{the kernel $S^{(N)}_{0}(k) = \1-P^{(+)}_{\lambda_{N}}(k) \rightarrow \1 - P^{(+)}_{m}$ for $N\rightarrow\infty$. }\DIFdelend \DIFaddbegin \DIFadd{$|\hat{s}|^{2}|\!\ \cdot\!\ |$.
}\DIFaddend \end{proof}

\DIFaddbegin \begin{rem}[The zero mode in the Ramond sector]
\label{rem:zeromode}
\DIFadd{For $m>0$, and for every $k\neq0$, the limit $S(k)$ in Corollary \ref{cor:stateconv} is determined by the renormalization condition \eqref{eq:rgcond} alone. This fails at the single point $(m,k)=(0,0)$, which occurs only in the Ramond sector, $0\in\Gamma_{\infty,+}$, and which is precisely the zero-mode degeneracy noted below \eqref{eq:Fmassless}: $\omega_{m}(0)=0$, so $P^{(+)}_{m=0}(0)$ is not defined by \eqref{eq:scalinglim}, and the massless limit of $P^{(+)}_{\lambda_{N}}(0)$ is direction dependent. The convention $\sign(0)=0$, i.e.~$\lambda_{N}\equiv0$, gives $S(0)=\tfrac{1}{2}\1_{2}$; taking instead $\lambda_{N}>0$ with $\vep_{N}^{-1}\lambda_{N}\rightarrow0$ gives $P^{(+)}_{\lambda_{N}}(0) = \tfrac{1}{2}(\1_{2}+\sigma_{z})$ for }\emph{\DIFadd{every}} \DIFadd{$N$ and hence $S(0)=\tfrac{1}{2}(\1_{2}-\sigma_{z})$, a different scaling-limit state.
}

\DIFadd{This is not a harmless ambiguity that could be averaged away, and it is worth recording why. Because $\hat{s}(0)=1$ and $\hat{s}(2\pi n)=0$ for $n\neq0$, the aliasing class of $l=0$ carries }\emph{\DIFadd{all}} \DIFadd{of its weight at $k=0$: $w_{\infty}(0)=1$ and $w_{\infty}(k)=0$ for the remaining aliases, and likewise $w_{M}(0)=1$ since $m_{0}(\pi)=0$. Hence \eqref{eq:convexsymb} degenerates at the zero mode to
}\begin{align*}
\DIFadd{\hat{S}^{(N)}_{M}(0) }& \DIFadd{= S^{(N+M)}(0), }& \DIFadd{\hat{S}^{(N)}_{\infty}(0) }& \DIFadd{= S(0),
}\end{align*}
\DIFadd{so that a discrepancy at $k=0$ enters $\delta_{M}(N)$ undamped, and by \eqref{eq:2pconvmono} it is visible already in the two-point function. The convention $\sign(0)=0$ is the one consistent with the rest of the article: the doubled projection \eqref{eq:doubleprojR} has the block %
\mbox{%DIFAUXCMD
$\big(\begin{smallmatrix}\frac{1}{2} & \pm\frac{1}{2}\\ \pm\frac{1}{2} & \frac{1}{2}\end{smallmatrix}\big)$
}%DIFAUXCMD
at $k=0$, which is precisely the purification \eqref{eq:doubleproj} of $S(0)=\tfrac{1}{2}\1_{2}$, and it is this zero mode that is responsible for the non-purity of $\omega_{{\color{blue}+}}$ and for the conformal weights $h=\tfrac{1}{8}$ respectively $h=\tfrac{1}{16}$ in \eqref{eq:zeromodeR} and \eqref{eq:scalelimviramajR}.
}\end{rem}

\DIFaddend \begin{rem}
\label{rem:stateconv}
\DIFdelbegin \DIFdel{The statement }\DIFdelend \DIFaddbegin \DIFadd{Two features }\DIFaddend of Lemma \ref{lem:stateconv} \DIFdelbegin \DIFdel{is remains valid if we replace the wavelet renormalization group by the }\DIFdelend \DIFaddbegin \DIFadd{deserve emphasis. First, both renormalization groups are covered by one and the same argument, the only difference being the alias weights: $w_{\infty}=|\hat{s}(\vep_{N}\!\ .\!\ )|^{2}$ in the wavelet case and $w_{\infty}=\chi_{\Gamma_{N}}$ --- a Dirac measure on each aliasing class --- in the }\DIFaddend momentum-cutoff \DIFdelbegin \DIFdel{renormalization group. To see, this we observe that :
}\begin{align*}
\DIFdel{|\langle e_{k,i},(R^{N+M}_{\infty}\!S^{(N+M)}\!R^{N}_{N+M}\!-\!SR^{N}_{\infty})(e_{l,j})\rangle_{\infty}|^{2} }& \DIFdel{= 2L_{N}\vep_{N}^{\frac{1}{2}}\delta_{k,l}|(S^{(N+M)})(l)_{ij}-S(l)_{ij}|^{2},
}\end{align*}%DIFAUXCMD
\DIFdel{for all $k\in\Gamma_{\infty,{\color{blue}\pm}}$ and $l\in\Gamma_{N,{\color{blue}\pm}}$ because: }\begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:momrginner}%%%
\langle e_{k,i},R^{N}_{M}(e_{l,j})\rangle_{M} }& \DIFdel{= 2L_{N}\delta_{ij}2^{\frac{1}{2}(M-N)}\delta_{k,l}, }\\ \DIFdel{\nonumber
\langle e_{k,i},R^{N}_{\infty}(e_{l,j})\rangle_{\infty} }& \DIFdel{= 2L_{N}\delta_{ij}\vep_{N}^{\frac{1}{2}}\delta_{k,l}.
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{case, for which \eqref{eq:convexsymb} degenerates to $\hat{S}^{(N)}_{M}(l)=S^{(N+M)}(l)$ and $\hat{S}^{(N)}_{\infty}(l)=S(l)$, so that $\delta_{M}(N)=\max_{l\in\Gamma_{N,{\color{blue}\pm}}}\|S^{(N+M)}_{\per}(l)-S(l)\|$ and \eqref{eq:2pconvmono} becomes an equality-driven estimate with the bare symbol deviation. Second, and in contrast with all convergence statements of Section \ref{sec:approxconf}, }\emph{\DIFadd{no regularity of the scaling function enters}}\DIFadd{: the argument uses only $|m_{0}(\xi)|^{2}+|m_{0}(\xi+\pi)|^{2}=1$ and $\sum_{n\in\Z}|\hat{s}(\xi+2\pi n)|^{2}=1$, both of which are equivalent to orthonormality of $\{s(\!\ .\!\ -n)\}_{n\in\Z}$. In particular Lemma \ref{lem:stateconv} holds for the Haar scaling function $_{1}s=\chi_{[0,1)}$, for which $\hat{s}$ decays only like $|\xi|^{-1}$ and the decay estimate \eqref{eq:decaysf} is unavailable. This is as it should be: the convergence of the lattice vacua is a statement about states on the fixed finite-dimensional algebra $\fA_{N,{\color{blue}\pm}}$, and it is only the convergence of the }\emph{\DIFadd{unbounded}} \DIFadd{generators in Section \ref{sec:approxconf} that is sensitive to the regularity of $s$.
}\DIFaddend \end{rem}

\DIFaddbegin \begin{rem}[Where the rate comes from]
\label{rem:staterate}
\DIFadd{Step 2 of the proof isolates the two mechanisms that limit the rate at which the lattice vacua converge: the $\mu^{l}_{\infty}$-average $\sum_{k\sim l}w_{\infty}(k)\|S^{(N+M)}_{\per}(k)-S(k)\|$ of the symbol deviation, and the total variation $\|\mu^{l}_{M}-\mu^{l}_{\infty}\|_{TV}$ of the alias weights, i.e.~the convergence of the finite products $\prod_{m=1}^{M}m_{0}(\vep_{N+m}\!\ .\!\ )$ to $\hat{s}(\vep_{N}\!\ .\!\ )$. Note that a supremum over the aliasing class would }\emph{\DIFadd{not}} \DIFadd{be small in the first term: $S(k)$ is the continuum symbol and does not become stationary in $k$, so it is essential that $\mu^{l}_{\infty}$ is a fixed probability measure and that dominated convergence is applied with respect to it. This is precisely where the localisation of $\hat{s}$ enters the }\emph{\DIFadd{rate}} \DIFadd{--- and only the rate, never the validity of \eqref{eq:2pconvnorm}. In the momentum-cutoff case $\mu^{l}_{\infty}=\delta_{l}$ and the first term collapses to $\|S^{(N+M)}(l)-S(l)\|$, which for the lattice vacua of Corollary \ref{cor:stateconv} is controlled directly by the renormalization condition \eqref{eq:rgcond}.
}\end{rem}

\DIFaddend \section{Approximation of conformal symmetries}
...
It is evident from these formulas that we recover the original Koo-Saleur formulas for $N\rightarrow\infty$ because $\chi_{\Gamma_{\infty}}\equiv1$, and $H^{(N)}_{k}(0)$ agrees with the unmodified $H^{(N)}_{k}$ (and similarly all the other modified quantities). \DIFaddbegin \DIFadd{The lattice Virasoro relations \eqref{eq:latviraCRchi} and the $\cO(\vep_{N}^{2})$ remainders \eqref{eq:latviraR} were computed for the }\emph{\DIFadd{unmodified}} \DIFadd{approximants; their fate under the modification is settled by the following remark. Likewise, $\overline{\widehat{\chi_{\Gamma_{N}}(\cdot+k)}}$ is a Dirichlet kernel and decays only like $|x|^{-1}$, so the modified approximants are quasi-local at scale $N$ only up to such tails. Both points are immaterial for the convergence statements, since the modification affects only momenta of order $\pi/\vep_{N}$, but they should be kept in mind when the finite-scale operators are used as they stand, e.g.~in a quantum simulation.
}\begin{rem}[The finite-scale algebra of the modified approximants]
\label{rem:modalgebra}
\DIFadd{By \eqref{eq:KSmod1p} the modified one-particle approximant is the }\emph{\DIFadd{truncated}} \DIFadd{weighted shift $\ell^{(N)}_{k}: e_{l}\mapsto\ell^{(N)}_{k}(l)\chi_{\Gamma_{N}}(l+k)\!\ e_{l+k}$, where the unmodified one lets the shift wrap around the periodic momentum lattice $\Gamma_{N}$. Composing two of them,
}\begin{align}
\DIFadd{\label{eq:modcommutator}
}[\DIFadd{\ell^{(N)}_{k},\ell^{(N)}_{k'}}]\DIFadd{e_{l} }& \DIFadd{= \chi_{\Gamma_{N}}(l\!+\!k\!+\!k')\big[\ell^{(N)}_{k'}(l)\ell^{(N)}_{k}(l\!+\!k')\chi_{\Gamma_{N}}(l\!+\!k')-\ell^{(N)}_{k}(l)\ell^{(N)}_{k'}(l\!+\!k)\chi_{\Gamma_{N}}(l\!+\!k)\big]e_{l+k+k'},
}\end{align}
\DIFadd{so the modified commutator differs from the truncation of the unmodified one by the term $D^{(N)}_{k,k'}$ obtained by replacing the two inner multipliers by $\chi_{\Gamma_{N}}(\!\ \cdot\!\ )-1$. Two observations follow.
}\DIFaddend 

\DIFaddbegin \DIFadd{First, $D^{(N)}_{k,k'}=0$ }\emph{\DIFadd{identically}} \DIFadd{whenever $k$ and $k'$ have the same sign: both partial shifts $l+k$ and $l+k'$ then lie between $l$ and $l+k+k'$, so if the total shift stays inside $\Gamma_{N}$ so does each partial one. The case $k'=0$ is trivial for the same reason, $\chi_{\Gamma_{N}}(l)\equiv1$ on $\Gamma_{N}$; in particular the pair $(k,0)$ governing the energy bound of Lemma \ref{lem:energybound1k} is unaffected. For all such pairs the modified approximants satisfy \eqref{eq:latviraCRchi} and \eqref{eq:latviraR} verbatim.
}

\DIFadd{Second, for opposite signs $D^{(N)}_{k,k'}$ is supported on those $l$ with $l+k+k'\in\Gamma_{N}$ but $l+k\notin\Gamma_{N}$ or $l+k'\notin\Gamma_{N}$, a set of at most $|k|+|k'|$ momenta all lying within $|k|+|k'|$ of the boundary of $\Gamma_{N}$. It is thus of finite rank, uniformly in $N$, but it is }\emph{\DIFadd{not}} \DIFadd{small in operator norm: the symbol $\vep_{N}^{-1}\sin(\vep_{N}(l+\tfrac{k}{2}))$ is of order $|k|$ at the zone boundary, so $\|D^{(N)}_{k,k'}\|=\cO(1)$. What is small is its restriction to any fixed Sobolev scale: since $|l|\geq\pi/\vep_{N}-(|k|+|k'|)$ on its support,
}\begin{align}
\DIFadd{\label{eq:modalgebrarate}
\big\|D^{(N)}_{k,k'}\big\|_{h^{\sigma}\rightarrow h^{0}} }& \DIFadd{\leq C_{k,k'}\!\ \vep_{N}^{\sigma}, }& \DIFadd{\sigma }& \DIFadd{\geq0,
}\end{align}
\DIFadd{which for $\sigma>2$ is smaller than the $\cO(\vep_{N}^{2})$ remainder \eqref{eq:latviraR} it accompanies. So the modification does not spoil the finite-scale algebra on the domains on which the convergence statements are formulated, and \eqref{eq:modalgebrarate} quantifies the ``affects only momenta of order $\pi/\vep_{N}$'' of the preceding paragraph.
}\end{rem}

\DIFaddend \subsubsection{The formal scaling limit}
\label{sec:formallim}
...
\DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:contvira1p}%%%
\ell_{\pm,k} }& \DIFdel{= \tfrac{\pm1}{2\pi}\sum_{l\in\Gamma_{\infty,{\color{blue}\pm}}}(l\mp\tfrac{k}{2})e_{l\mp k}\otimes \overline{e}_{l},
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:contvira1p}
\ell_{\pm,k} }& \DIFadd{= \tfrac{\pm1}{2L}\sum_{l\in\Gamma_{\infty,{\color{blue}\pm}}}(l\mp\tfrac{k}{2})e_{l\mp k}\otimes \overline{e}_{l},
}\end{align}\DIFaddend 
...
\DIFadd{The prefactor in \eqref{eq:contvira1p} is fixed by $\langle e_{l},e_{l}\rangle_{\infty}=2L$, so that
}\begin{align}
\DIFadd{\label{eq:normalisationaction}
\ell_{\pm,k}e_{m} }& \DIFadd{= \pm(m\mp\tfrac{k}{2})e_{m\mp k}, }& \DIFadd{(\ell_{\pm,k})_{mn} }& \DIFadd{= \pm2L\delta_{m,n\mp k}(n\mp\tfrac{k}{2}).
}\end{align}
\DIFadd{This is exactly the normalisation of the symbols $\ell_{k}(l)$ in \eqref{eq:scalelim1pvira}, so that the bridge to the second-quantised generators is $L_{\pm,k} = \tfrac{L}{\pi}dF_{S}(\ell_{\pm,k})$ throughout, consistently with \eqref{eq:scalelimvira}, \eqref{eq:viraCRcxrep} and \eqref{eq:KShermiteanparts}. Since $\ell_{\pm,k}$ and $\ell_{\pm,-k}$ have Hilbert-Schmidt off-diagonal parts supported on the finite strip $\{n\!\ :\!\ \pm n>0,\ \mp(n\mp k)>0\}$, the Schwinger cocycle \eqref{eq:schwinger} is the finite sum $c_{P^{\mp}}(\ell_{\pm,k},\ell_{\pm,-k}) = (\tfrac{\pi}{L})^{2}\sum_{n}(n\mp\tfrac{k}{2})^{2} = (\tfrac{\pi}{L})^{2}\tfrac{1}{12}\tfrac{L}{\pi}k((\tfrac{L}{\pi}k)^{2}-1)$, which together with the prefactor $(\tfrac{L}{\pi})^{2}$ in \eqref{eq:viraCRcxrep} reproduces $c=1$ (respectively $c=\tfrac{1}{2}$ in the Majorana case) for every $L>0$. We emphasise this because the two normalisations differ by a factor $\tfrac{L}{\pi}$ and agree only for $L=\pi$.
}\end{rem}

\DIFaddend We note that \eqref{eq:contvira1pad} implies that $\ell_{\pm,k}$ is closable. In view of this definition, we use the following notation for the one-particle KS approximants \eqref{eq:latvirasubchi},
\DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:latvira1p}%%%
\ell^{(N)}_{\pm,k} }& \DIFdel{= \tfrac{\pm1}{2\pi}\sum_{l\in\Gamma_{N,{\color{blue}\pm}}}\cos(\tfrac{1}{4}\vep_{N}k)^{2}\sin(\vep_{N}(l\mp\tfrac{k}{2}))e_{l\mp k}\otimes \overline{e}_{l},
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:latvira1p}
\ell^{(N)}_{\pm,k} }& \DIFadd{= \tfrac{\pm1}{2L}\sum_{l\in\Gamma_{N,{\color{blue}\pm}}}\cos(\tfrac{1}{4}\vep_{N}k)^{2}\sin(\vep_{N}(l\mp\tfrac{k}{2}))e_{l\mp k}\otimes \overline{e}_{l},
}\end{align}\DIFaddend 
...
\DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:analyticvector}%%%
\|\ell_{\pm,k}^{j}e_{m}\|_{\infty}^{2} }& \DIFdel{\leq 2L(\tfrac{L}{\pi})^{2j}\left\{\begin{matrix} |m|^{2j} & k=0 \\ |k|^{2j}\left(\tfrac{\Gamma(|\frac{m}{k}|+\frac{1}{2}+j)}{\Gamma(|\frac{m}{k}|+\frac{1}{2})}\right)^{2} & k\neq0 \end{matrix}\right.,}\\ \DIFdel{\nonumber
\|r_{\pm,k}^{j}e_{m}\|_{\infty}^{2} }& \DIFdel{\leq 2L(\tfrac{L}{2\pi})^{2j}\left\{\begin{matrix} |2 m|^{2j} & k=0 \\ \binom{2j}{j}|k|^{2j}\left(\tfrac{\Gamma(|\frac{m}{k}|+\frac{1}{2}+j)}{\Gamma(|\frac{m}{k}|+\frac{1}{2})}\right)^{2} & k\neq0 \end{matrix}\right.,
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:analyticvector}
\|\ell_{\pm,k}^{j}e_{m}\|_{\infty}^{2} }& \DIFadd{\leq 2L\left\{\begin{matrix} |m|^{2j} & k=0 \\ |k|^{2j}\left(\tfrac{\Gamma(|\frac{m}{k}|+\frac{1}{2}+j)}{\Gamma(|\frac{m}{k}|+\frac{1}{2})}\right)^{2} & k\neq0 \end{matrix}\right.,}\\ \DIFadd{\nonumber
\|r_{\pm,k}^{j}e_{m}\|_{\infty}^{2} }& \DIFadd{\leq 2L(\tfrac{1}{2})^{2j}\left\{\begin{matrix} |2 m|^{2j} & k=0 \\ \binom{2j}{j}|k|^{2j}\left(\tfrac{\Gamma(|\frac{m}{k}|+\frac{1}{2}+j)}{\Gamma(|\frac{m}{k}|+\frac{1}{2})}\right)^{2} & k\neq0 \end{matrix}\right.,
}\end{align}\DIFaddend 
with an analogous bound for $\|\iota_{\pm,k}^{j}e_{m}\|_{\infty}^{2}$. Here, $\Gamma(x+1) = x\Gamma(x)$ denotes the Gamma function. These bounds \DIFdelbegin \DIFdel{imply that each }\DIFdelend \DIFaddbegin \DIFadd{are summable with an explicit radius which, for $k\neq0$, does not depend on $m$. Indeed, with $a := |\tfrac{m}{k}|+\tfrac{1}{2}$ one has $\tfrac{\Gamma(a+j)}{\Gamma(a)j!} = \binom{a+j-1}{j}$, so that the binomial series gives }\begin{align}
\DIFadd{\label{eq:analyticradius}
\sum_{j=0}^{\infty}\tfrac{t^{j}}{j!}\|\ell_{\pm,k}^{j}e_{m}\|_{\infty} }& \DIFadd{\leq \sqrt{2L}\sum_{j=0}^{\infty}\binom{a+j-1}{j}(|k|t)^{j} = \sqrt{2L}\!\ (1-|k|t)^{-a}, }& \DIFadd{|k|t }& \DIFadd{<1,
}\end{align}
\DIFadd{and the identical bound holds for $r_{\pm,k}$ and $\iota_{\pm,k}$, because $(\tfrac{1}{2})^{j}\binom{2j}{j}^{\frac{1}{2}}\leq1$ by $\binom{2j}{j}\leq4^{j}$. Each }\DIFaddend plane wave $e_{m}$ is \DIFaddbegin \DIFadd{therefore }\DIFaddend an analytic vector for $\ell_{\pm,k}$, $r_{\pm,k}$ and $\iota_{\pm,k}$\DIFdelbegin \DIFdel{because the series, }\begin{align*}
\DIFdel{\sum_{j=0}^{\infty}\tfrac{t^{j}}{j!}\|\ell_{\pm,k}^{j}e_{m}\|_{\infty} }& \DIFdel{< \infty, }& \DIFdel{\sum_{j=0}^{\infty}\tfrac{t^{j}}{j!}\|r_{\pm,k}^{j}e_{m}\|_{\infty} }& \DIFdel{< \infty, }& \DIFdel{\sum_{j=0}^{\infty}\tfrac{t^{j}}{j!}\|\iota_{\pm,k}^{j}e_{m}\|_{\infty} }& \DIFdel{< \infty,
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{, with radius of convergence
}\begin{align}
\DIFadd{\label{eq:analyticradiusvalue}
t }& \DIFadd{< \tfrac{1}{|k|}\quad\text{for }\ell_{\pm,k},r_{\pm,k},\iota_{\pm,k}, }& \DIFadd{t }& \DIFadd{< \tfrac{\pi}{L|k|}\quad\text{for }\tfrac{L}{\pi}\ell_{\pm,k},\tfrac{L}{\pi}r_{\pm,k},\tfrac{L}{\pi}\iota_{\pm,k},
}\end{align}\DIFaddend 
\DIFdelbegin \DIFdel{are convergent for some $t>0$. }\DIFdelend \DIFaddbegin \DIFadd{the second being the normalisation of Remark \ref{rem:normalisation} in which the one-particle operators enter the second-quantized Virasoro generators. What matters below is that this radius is }\emph{\DIFadd{uniform in}} \DIFadd{$m$. For $k=0$ the first line of \eqref{eq:analyticvector} yields only the $m$-dependent radius $\tfrac{1}{|m|}$; this is immaterial, since $\ell_{\pm,0}$ is diagonal in $\{e_{m}\}_{m\in\Gamma_{\infty,{\color{blue}\pm}}}$ and hence self-adjoint on $h^{1}(\Gamma_{\infty,{\color{blue}\pm}})$ to begin with. }\DIFaddend By Nelson’s analytic vector theorem \cite{NelsonAnalyticVectors, ReedMethodsOfModern2} we have:
\begin{cor}
\label{cor:viraesa}
...
\DIFdelbegin \begin{align*}
\DIFdel{\|\xi\|_{\ell_{\pm,k}}^{2} }& \DIFdel{= \|\xi\|_{\infty}^{2}+\|\ell_{\pm,k}\xi\|_{\infty}^{2} \leq \max\{1+(\tfrac{L k}{2\pi})^{2},(\tfrac{L}{\pi})^{2}\}\|\xi\|_{h^{1}(\Gamma_{\infty,{\color{blue}\pm}})}^{2},
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align*}
\DIFadd{\|\xi\|_{\ell_{\pm,k}}^{2} }& \DIFadd{= \|\xi\|_{\infty}^{2}+\|\ell_{\pm,k}\xi\|_{\infty}^{2} \leq C_{k,L}\|\xi\|_{h^{1}(\Gamma_{\infty,{\color{blue}\pm}})}^{2},
}\end{align*}\DIFaddend 
\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{for a constant $C_{k,L}$ depending only on $k$ and $L$, and }\DIFaddend similarly for $\|\xi\|_{r_{\pm,k}}$ and $\|\xi\|_{\iota_{\pm,k}}$. \DIFdelbegin \DIFdel{But, it is known that }\DIFdelend \DIFaddbegin \DIFadd{Now }\DIFaddend $\cD_{\std}$ \DIFdelbegin \DIFdel{and $\cD_{W}$ (provided the scaling function $s$ is sufficiently regular)are dense in }\DIFdelend \DIFaddbegin \DIFadd{is dense in }\DIFaddend $h^{1}(\Gamma_{\infty,{\color{blue}\pm}})$\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{WASA}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{, and so is $\cD_{W}$ }\emph{\DIFadd{provided $s$ is $1$-regular in the sense of Definition \ref{def:regularity}}}\DIFadd{, i.e.~$s\in H^{1}(\R)$ \mbox{%DIFAUXCMD
\cite{WASA}}\hskip0pt%DIFAUXCMD
. We stress that this hypothesis is not automatic: by Remark \ref{rem:regularityvalues} the Daubechies function $_{2}s$ is exactly borderline ($\sigma_{2}=1$), so that $\cD_{W}\not\subset\cD(\ell_{\pm,k})$ in that case and $K\geq3$ is required}\DIFaddend . It follows that the graph-norm closures of $\cD_{\std}$ and $\cD_{W}$ agree, and we conclude that $\cD_{W}$ is a core for $\ell_{\pm,k}$, $r_{\pm,k}$ and $\iota_{\pm,k}$ \cite{ReedMethodsOfModern1}.
...
\label{lem:KSconv}
Let $\xi\in D_{W}$ and \DIFdelbegin \DIFdel{$s\in C^{\alpha}(\R)$ a sufficiently regular, }\DIFdelend \DIFaddbegin \DIFadd{let $s=\!\ _{K}s$ be a }\DIFaddend compactly supported orthonormal \DIFdelbegin \DIFdel{scaling function}\DIFdelend \DIFaddbegin \DIFadd{Daubechies scaling function which is $1$-regular in the sense of Definition \ref{def:regularity}, i.e.~$\sigma_{K}>1$ (equivalently: $K-\cK_{j}>\tfrac{3}{2}$ for some $j$; by Remark \ref{rem:regularityvalues} this holds for $K\geq3$)}\DIFaddend . Then:
\begin{align*}
\lim_{N\rightarrow\infty}\|(\tilde{\ell}^{(N)}_{\pm,k} - \ell_{\pm,k})\xi\|_{\infty} & = 0,
\end{align*}
...
\DIFdelbegin \begin{align*}
\DIFdel{f^{(N)}_{k,\pm}(M,m,l) }& \DIFdel{= \tfrac{\pm1}{2\pi}\sum_{n\in\Gamma_{N,{\color{blue}\pm}}}\cos(\tfrac{1}{4}\vep_{N}k)^{2}\sin(\vep_{N}(n\mp\tfrac{k}{2}))\langle e_{m},R^{N}_{\infty}(e_{n\mp k})\rangle_{\infty}\langle e_{n},R^{M}_{N}(e_{l})\rangle_{N}, }\\
\DIFdel{f_{k,\pm}(M,m,l) }& \DIFdel{= \tfrac{\pm1}{2\pi}\sum_{n\in\Gamma_{\infty,{\color{blue}\pm}}}(n\mp\tfrac{k}{2})\underbrace{\langle e_{m},e_{n\mp k}\rangle_{\infty}}_{=2L\delta_{m,n\mp k}}\langle e_{n},R^{M}_{\infty}(e_{l})\rangle_{\infty}.
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align*}
\DIFadd{f^{(N)}_{k,\pm}(M,m,l) }& \DIFadd{= \tfrac{\pm1}{2L}\sum_{n\in\Gamma_{N,{\color{blue}\pm}}}\cos(\tfrac{1}{4}\vep_{N}k)^{2}\sin(\vep_{N}(n\mp\tfrac{k}{2}))\langle e_{m},R^{N}_{\infty}(e_{n\mp k})\rangle_{\infty}\langle e_{n},R^{M}_{N}(e_{l})\rangle_{N}, }\\
\DIFadd{f_{k,\pm}(M,m,l) }& \DIFadd{= \tfrac{\pm1}{2L}\sum_{n\in\Gamma_{\infty,{\color{blue}\pm}}}(n\mp\tfrac{k}{2})\underbrace{\langle e_{m},e_{n\mp k}\rangle_{\infty}}_{=2L\delta_{m,n\mp k}}\langle e_{n},R^{M}_{\infty}(e_{l})\rangle_{\infty}.
}\end{align*}\DIFaddend 
Explicitly evaluating the inner product as in \eqref{eq:waveletrginner}, this leads to:
\DIFdelbegin \begin{align*}
\DIFdel{f^{(N)}_{k,\pm}\!(M,m,l) }&\DIFdel{\!=\!\tfrac{\pm1}{4\pi}\vep_{M}^{\frac{1}{2}} \cos(\tfrac{1}{4}\vep_{N}k)^{2}\hat{s}(\vep_{N}m)\!\!\sum_{n\in\Gamma_{N,{\color{blue}\pm}}}\!\!2L_{N}\sin(\vep_{N}(n\mp\tfrac{k}{2}))\delta_{0,\frac{L}{\pi}(m-(n\mp k))\!\!\!\!\mod 2L_{N}} }\\
& \DIFdel{\hspace{5cm} \times 2L_{M}\delta_{0,\frac{L}{\pi}(n-l)\!\!\!\!\mod 2L_{M}}\prod_{j=1}^{N-M}m_{0}(\vep_{M+j}n) }\\
&\DIFdel{\!=\!\tfrac{\pm1}{4\pi}\vep_{M}^{\frac{1}{2}} \cos(\tfrac{1}{4}\vep_{N}k)^{2}\hat{s}(\vep_{N}m)\!\!\sum_{n\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!2L_{N}\sin(\vep_{N}(n\mp\tfrac{k}{2}))\delta_{0,\frac{L}{\pi}(m-(n\mp k))\!\!\!\!\mod 2L_{N}} }\\
& \DIFdel{\hspace{5cm} \times 2L_{M}\delta_{0,\frac{L}{\pi}(n-l)\!\!\!\!\mod 2L_{M}}\chi_{\Gamma_{N}}(n)\prod_{j=1}^{N-M}\!\!\!m_{0}(\vep_{M+j}n) }\\
&\DIFdel{\!=\!\tfrac{\pm L}{2\pi}2L_{M}\vep_{M}^{\frac{1}{2}} \cos(\tfrac{1}{4}\vep_{N}k)^{2}\hat{s}(\vep_{N}m)\sum_{n\in\Z}\tfrac{\sin(\vep_{N}(m\pm\frac{k}{2})\!)}{\vep_{N}} \delta_{0,\frac{L}{\pi}(m\pm k+\frac{2\pi}{\vep_{N}}n-l)\!\!\!\!\mod 2L_{M}}}\\
& \DIFdel{\hspace{5.5cm} \times\!\!\chi_{\Gamma_{N}}\!(m\!\pm\!k\!+\!\tfrac{2\pi}{\vep_{N}}n)\!\!\!\prod_{j=1}^{N-M}\!\!\!m_{0}(\vep_{M+j}(m\!\pm\!k\!+\!\tfrac{2\pi}{\vep_{N}}n)\!), }\\
\DIFdel{f_{k,\pm}\!(M,m,l) }&\DIFdel{\!=\!\tfrac{\pm L}{2\pi}2L_{M}\vep_{M}^{\frac{1}{2}}\hat{s}(\vep_{M}(m\!\pm\!k))(m\!\pm\!\tfrac{k}{2})\delta_{0,\frac{L}{\pi}(m\pm k-l)\!\!\!\!\mod 2L_{M}},
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align*}
\DIFadd{f^{(N)}_{k,\pm}\!(M,m,l) }&\DIFadd{\!=\!\tfrac{\pm1}{4\pi}\vep_{M}^{\frac{1}{2}} \cos(\tfrac{1}{4}\vep_{N}k)^{2}\hat{s}(\vep_{N}m)\!\!\sum_{n\in\Gamma_{N,{\color{blue}\pm}}}\!\!2L_{N}\sin(\vep_{N}(n\mp\tfrac{k}{2}))\delta_{0,\frac{L}{\pi}(m-(n\mp k))\!\!\!\!\mod 2L_{N}} }\\
& \DIFadd{\hspace{5cm} \times 2L_{M}\delta_{0,\frac{L}{\pi}(n-l)\!\!\!\!\mod 2L_{M}}\prod_{j=1}^{N-M}m_{0}(\vep_{M+j}n) }\\
&\DIFadd{\!=\!\tfrac{\pm1}{4\pi}\vep_{M}^{\frac{1}{2}} \cos(\tfrac{1}{4}\vep_{N}k)^{2}\hat{s}(\vep_{N}m)\!\!\sum_{n\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!2L_{N}\sin(\vep_{N}(n\mp\tfrac{k}{2}))\delta_{0,\frac{L}{\pi}(m-(n\mp k))\!\!\!\!\mod 2L_{N}} }\\
& \DIFadd{\hspace{5cm} \times 2L_{M}\delta_{0,\frac{L}{\pi}(n-l)\!\!\!\!\mod 2L_{M}}\chi_{\Gamma_{N}}(n)\prod_{j=1}^{N-M}\!\!\!m_{0}(\vep_{M+j}n) }\\
&\DIFadd{\!=\!\tfrac{\pm1}{2}2L_{M}\vep_{M}^{\frac{1}{2}} \cos(\tfrac{1}{4}\vep_{N}k)^{2}\hat{s}(\vep_{N}m)\sum_{n\in\Z}\tfrac{\sin(\vep_{N}(m\pm\frac{k}{2})\!)}{\vep_{N}} \delta_{0,\frac{L}{\pi}(m\pm k+\frac{2\pi}{\vep_{N}}n-l)\!\!\!\!\mod 2L_{M}}}\\
& \DIFadd{\hspace{5.5cm} \times\!\!\chi_{\Gamma_{N}}\!(m\!\pm\!k\!+\!\tfrac{2\pi}{\vep_{N}}n)\!\!\!\prod_{j=1}^{N-M}\!\!\!m_{0}(\vep_{M+j}(m\!\pm\!k\!+\!\tfrac{2\pi}{\vep_{N}}n)\!), }\\
\DIFadd{f_{k,\pm}\!(M,m,l) }&\DIFadd{\!=\!\tfrac{\pm1}{2}2L_{M}\vep_{M}^{\frac{1}{2}}\hat{s}(\vep_{M}(m\!\pm\!k))(m\!\pm\!\tfrac{k}{2})\delta_{0,\frac{L}{\pi}(m\pm k-l)\!\!\!\!\mod 2L_{M}},
}\end{align*}\DIFaddend 
...
Thus, we arrive at:
\DIFdelbegin \begin{align*}
\DIFdel{f^{(N)}_{k,\pm}\!(M,m,l) }&\DIFdel{\!=\!\tfrac{\pm L}{2\pi}2L_{M}\vep_{M}^{\frac{1}{2}} \cos(\tfrac{1}{4}\vep_{N}k)^{2}\hat{s}(\vep_{N}m)\sum_{n\in\Z}\tfrac{\sin(\vep_{N}(m\pm\frac{k}{2}))}{\vep_{N}} \delta_{0,\frac{L}{\pi}(m\pm k-l)\!\!\!\!\mod 2L_{M}}}\\
& \DIFdel{\hspace{5.5cm} \times \chi_{\Gamma_{N}}\!(m\!\pm\!k\!+\!\tfrac{2\pi}{\vep_{N}}n)\!\!\prod_{j=1}^{N-M}\!\!\!m_{0}(\vep_{M+j}(m\!\pm\!k)\!) }\\
&\DIFdel{\!=\!\tfrac{\pm L}{2\pi}2L_{M}\vep_{M}^{\frac{1}{2}} \cos(\tfrac{1}{4}\vep_{N}k)^{2}\hat{s}(\vep_{N}m)\tfrac{\sin(\vep_{N}(m\pm\frac{k}{2})\!)}{\vep_{N}} \delta_{0,\frac{L}{\pi}(m\pm k-l)\!\!\!\!\mod 2L_{M}}\tfrac{\hat{s}(\vep_{M}(m\pm k)\!)}{\hat{s}(\vep_{N}(m\pm k)\!)},
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align*}
\DIFadd{f^{(N)}_{k,\pm}\!(M,m,l) }&\DIFadd{\!=\!\tfrac{\pm1}{2}2L_{M}\vep_{M}^{\frac{1}{2}} \cos(\tfrac{1}{4}\vep_{N}k)^{2}\hat{s}(\vep_{N}m)\sum_{n\in\Z}\tfrac{\sin(\vep_{N}(m\pm\frac{k}{2}))}{\vep_{N}} \delta_{0,\frac{L}{\pi}(m\pm k-l)\!\!\!\!\mod 2L_{M}}}\\
& \DIFadd{\hspace{5.5cm} \times \chi_{\Gamma_{N}}\!(m\!\pm\!k\!+\!\tfrac{2\pi}{\vep_{N}}n)\!\!\prod_{j=1}^{N-M}\!\!\!m_{0}(\vep_{M+j}(m\!\pm\!k)\!) }\\
&\DIFadd{\!=\!\tfrac{\pm1}{2}2L_{M}\vep_{M}^{\frac{1}{2}} \cos(\tfrac{1}{4}\vep_{N}k)^{2}\hat{s}(\vep_{N}m)\tfrac{\sin(\vep_{N}(m\pm\frac{k}{2})\!)}{\vep_{N}} \delta_{0,\frac{L}{\pi}(m\pm k-l)\!\!\!\!\mod 2L_{M}}\!\!\prod_{j=1}^{N-M}\!\!\!m_{0}(\vep_{M+j}(m\!\pm\!k)\!),
}\end{align*}\DIFaddend 
because $\sum_{n\in\Z}\chi_{\Gamma_{N}}(m\pm k+\tfrac{2\pi}{\vep_{N}}n) = 1$ for any fixed $m\pm k\in\Gamma_{\infty,{\color{blue}\pm}}$. We \DIFdelbegin \DIFdel{also used \eqref{eq:infprod}}\DIFdelend \DIFaddbegin \DIFadd{stress that this finite product must }\emph{\DIFadd{not}} \DIFadd{be rewritten as the quotient $\hat{s}(\vep_{M}(m\pm k))/\hat{s}(\vep_{N}(m\pm k))$ suggested by \eqref{eq:infprod}: orthonormality of $s$ forces $\sum_{n\in\Z}|\hat{s}(l+2\pi n)|^{2}=1$ and hence $\hat{s}(2\pi n)=0$ for every $n\neq0$, so that in the Ramond sector the denominator vanishes identically at the admissible momenta $m\pm k = \tfrac{2\pi n}{\vep_{N}}\in\Gamma_{\infty,+}$, $n\neq0$, while the numerator does not}\DIFaddend . Next, we write:
\DIFdelbegin \begin{align*}
\DIFdel{f^{(N)}_{k,\pm}(M,m,l) }& \DIFdel{= \tfrac{\pm L}{2\pi}2L_{M}\vep_{M}^{\frac{1}{2}}f^{(N)}_{\pm k}(M,m)\delta_{0,\frac{L}{\pi}(m\pm k-l)\!\!\!\!\mod 2L_{M}}, }\\
\DIFdel{f_{k,\pm}(M,m,l) }& \DIFdel{=  \tfrac{\pm L}{2\pi}2L_{M}\vep_{M}^{\frac{1}{2}}f_{\pm k}(M,m)\delta_{0,\frac{L}{\pi}(m\pm k-l)\!\!\!\!\mod 2L_{M}}.
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align*}
\DIFadd{f^{(N)}_{k,\pm}(M,m,l) }& \DIFadd{= \tfrac{\pm1}{2}2L_{M}\vep_{M}^{\frac{1}{2}}f^{(N)}_{\pm k}(M,m)\delta_{0,\frac{L}{\pi}(m\pm k-l)\!\!\!\!\mod 2L_{M}}, }\\
\DIFadd{f_{k,\pm}(M,m,l) }& \DIFadd{=  \tfrac{\pm1}{2}2L_{M}\vep_{M}^{\frac{1}{2}}f_{\pm k}(M,m)\delta_{0,\frac{L}{\pi}(m\pm k-l)\!\!\!\!\mod 2L_{M}}.
}\end{align*}\DIFaddend 
where we defined:
\DIFdelbegin \begin{align*}
\DIFdel{f^{(N)}_{k}(M,m) }& \DIFdel{:= \hat{s}(\vep_{M}(m+k)\!)\cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(m+\frac{k}{2})\!)}{\vep_{N}}\tfrac{\hat{s}(\vep_{N}m)}{\hat{s}(\vep_{N}(m+k)\!)}, }\\
\DIFdel{f_{k}(M,m) }& \DIFdel{:= \hat{s}(\vep_{M}(m+k)\!)(m\!+\!\tfrac{k}{2}).
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align*}
\DIFadd{f^{(N)}_{k}(M,m) }& \DIFadd{:= \hat{s}(\vep_{N}m)\cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(m+\frac{k}{2})\!)}{\vep_{N}}\!\!\prod_{j=1}^{N-M}\!\!\!m_{0}(\vep_{M+j}(m\!+\!k)\!), }\\
\DIFadd{f_{k}(M,m) }& \DIFadd{:= \hat{s}(\vep_{M}(m+k)\!)(m\!+\!\tfrac{k}{2}).
}\end{align*}\DIFaddend 
...
\end{align*}
for arbitrary $l\in\Gamma_{M,{\color{blue}\pm}}$. It remains to show that $\lim_{N\rightarrow\infty}f^{(N)}_{k,\pm}(M,\!\ .\!\ ,l) = f_{k,\pm}(M,\!\ .\!\ ,l)$ in $\ltwo(\Gamma_{\infty,{\color{blue}\pm}},(2L)^{-1})$, which follows from the dominated convergence theorem \DIFdelbegin \DIFdel{if we provide a $g_{k}(M,\!\ .\!\ )\in\ltwo(\Gamma_{\infty,{\color{blue}\pm}}, (2L)^{-1})$ such that: }\begin{align*}
\DIFdel{|f^{(N)}_{k}(M,m)| }& \DIFdel{\leq g_{k}(M,m).
}\end{align*}%DIFAUXCMD
\DIFdel{The existence of such a $g_{k}(N,\!\ .\!\ )$ is implied by }\DIFdelend \DIFaddbegin \DIFadd{once a majorant $g_{k}(M,\!\ .\!\ )\in\ltwo(\Gamma_{\infty,{\color{blue}\pm}},(2L)^{-1})$ with $|f^{(N)}_{k}(M,m)|\leq g_{k}(M,m)$ for all $N>M$ is provided. This is exactly the content of Corollary \ref{cor:unifmaj}: for $N\geq N_{0}(k)$, i.e.~$\vep_{N}|k|\leq\tfrac{\pi}{2}$}\DIFaddend ,
\DIFdelbegin \begin{align*}
\DIFdel{|f^{(N)}_{k}(M,m)| }& \DIFdel{\leq |(m+\tfrac{k}{2})\hat{s}(\vep_{M}(m+k))|\Big|\tfrac{\hat{s}(\vep_{N}m)}{\hat{s}(\vep_{N}(m+k)\!)}\Big| \leq C_{k}|(m+\tfrac{k}{2})\hat{s}(\vep_{M}(m+k))|,
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:KSconvmaj}
|f^{(N)}_{k}(M,m)| }& \DIFadd{\leq |\hat{s}(\vep_{N}m)|\!\ P_{N-M}(\vep_{M}(m+k))\!\ |m+\tfrac{k}{2}| \leq g_{k}(M,m),
}\end{align}\DIFaddend 
\DIFdelbegin \DIFdel{for some $C_{k}>0$, because of the uniform continuity of $\hat{s}$, }\DIFdelend \DIFaddbegin \DIFadd{with $g_{k}(M,\!\ .\!\ )$ as in \eqref{eq:majorant}, which lies in $\ltwo(\Gamma_{\infty,{\color{blue}\pm}},(2L)^{-1})$ because $s$ is $1$-regular. The finitely many scales $M<N<N_{0}(k)$ are irrelevant for the limit.
}

\DIFadd{Let us emphasise where the difficulty in \eqref{eq:KSconvmaj} lies, since it is easy to overlook. Rewriting the finite product as $\hat{s}(\vep_{M}(m+k))/\hat{s}(\vep_{N}(m+k))$ and bounding the quotient is not an option, for the reason given above. Bounding $P_{N-M}$ by \eqref{eq:convergencecor} alone is not an option either, because that estimate is available only on the fundamental domain $|\vep_{M}(m+k)|\leq2^{N-M}\pi$, i.e.~only for momenta in the $N$-th Brillouin zone, while $m$ ranges over all of $\Gamma_{\infty,{\color{blue}\pm}}$; outside, $P_{N-M}$ merely repeats periodically and does not decay. What makes \eqref{eq:KSconvmaj} true is that the two factors decay in an interlocked fashion: $P_{N-M}$ is of order one outside the Brillouin zone only near the multiples of $2^{N-M+1}\pi$, and precisely there $\hat{s}(\vep_{N}m)$ is close to a zero of order $K$ of $\hat{s}$. This is what Lemma \ref{lem:unifmaj} makes quantitative.
}

\emph{\DIFadd{Extension to the full domain.}} \DIFadd{The convergence in fact holds on all of $h^{1}(\Gamma_{\infty,{\color{blue}\pm}})$, not merely on $\cD_{W}$, because the operators involved are uniformly bounded from $h^{1}$ to $\fh^{(\pm)}_{\infty,{\color{blue}\pm}}$. Indeed, by \eqref{eq:aliasprob} and the Cauchy-Schwarz inequality,
}\begin{align}
\DIFadd{\label{eq:coisometrysobolev}
\big|\widehat{(R^{N}_{\infty})^{*}\xi}(l)\big|^{2} }& \DIFadd{= \vep_{N}^{-1}\Big|\sum_{k\sim l}\overline{\hat{s}(\vep_{N}k)}\hat{\xi}(k)\Big|^{2} \leq \vep_{N}^{-1}\!\!\sum_{k\sim l}|\hat{\xi}(k)|^{2},
}\end{align}
\DIFaddend and \DIFdelbegin \DIFdel{the decay estimate \eqref{eq:decaysf} for sufficiently regular $s\in C^{\alpha}(\R)$. $\hat{s}$ is uniformly continuous since $\lim_{|k|\rightarrow\infty}|\hat{s}(k)|=0$ and $\hat{s}(k) = \prod_{j=1}^{\infty}m_{0}(2^{-j}k)$ is locally uniformly convergent.
}\DIFdelend \DIFaddbegin \DIFadd{$|l|\leq|k|$ for every $k$ in the aliasing class of $l\in\Gamma_{N,{\color{blue}\pm}}$, whence $\|(R^{N}_{\infty})^{*}\xi\|_{h^{\sigma}(\Gamma_{N,{\color{blue}\pm}})}\leq\|\xi\|_{h^{\sigma}(\Gamma_{\infty,{\color{blue}\pm}})}$ for every $\sigma\geq0$. Together with $|\!\cos(\tfrac{1}{4}\vep_{N}k)^{2}\sin(\vep_{N}(l\mp\tfrac{k}{2}))/\vep_{N}|\leq|l\mp\tfrac{k}{2}|$ and the isometry of $R^{N}_{\infty}$ this gives
}\begin{align}
\DIFadd{\label{eq:unifh1bound}
\|\tilde{\ell}^{(N)}_{\pm,k}\xi\|_{\infty} }& \DIFadd{\leq C_{k,L}\|\xi\|_{h^{1}(\Gamma_{\infty,{\color{blue}\pm}})}, }& \DIFadd{\|\ell_{\pm,k}\xi\|_{\infty} }& \DIFadd{\leq C_{k,L}\|\xi\|_{h^{1}(\Gamma_{\infty,{\color{blue}\pm}})},
}\end{align}
\DIFadd{with $C_{k,L}$ independent of $N$. Since $\cD_{W}$ is $h^{1}$-dense (see the discussion preceding this paragraph) and $h^{1}(\Gamma_{\infty,{\color{blue}\pm}}) = \cD(\ell_{\pm,k})$, an $\tfrac{\vep}{3}$-argument extends the convergence from $\cD_{W}$ to the whole domain of $\ell_{\pm,k}$.
}\DIFaddend \end{proof}

\DIFaddbegin \begin{rem}[Sharp rate in the Sobolev scale]
\label{rem:KSconvrate}
\DIFadd{Estimate \eqref{eq:unifh1bound} cannot be improved to norm convergence at $\sigma=1$: the operators $\tilde{\ell}^{(N)}_{\pm,k}-\ell_{\pm,k}$ do }\emph{\DIFadd{not}} \DIFadd{converge to $0$ in $\cB(h^{1},\fh^{(\pm)}_{\infty,{\color{blue}\pm}})$. Taking for $\xi$ the plane wave $e_{n}$ with $n\in\Gamma_{\infty,{\color{blue}\pm}}$ closest to $\tfrac{\pi}{\vep_{N}}$, one has $\vep_{N}n = \pi+O(\vep_{N})$ and therefore $\|\ell_{\pm,k}e_{n}\|_{\infty} = |n\mp\tfrac{k}{2}|\!\ \|e_{n}\|_{\infty}\sim\tfrac{\pi}{\vep_{N}}\|e_{n}\|_{\infty}$, whereas the symbol of $\ell^{(N)}_{\pm,k}$ satisfies $|\!\sin(\vep_{N}(n\mp\tfrac{k}{2}))|/\vep_{N} = |\!\sin(\pi+O(\vep_{N}))|/\vep_{N} = O(1)$, so that $\|\tilde{\ell}^{(N)}_{\pm,k}e_{n}\|_{\infty} = O(\|e_{n}\|_{\infty})$ by \eqref{eq:coisometrysobolev}, and hence
}\begin{align}
\DIFadd{\label{eq:KSratelower}
\big\|\tilde{\ell}^{(N)}_{\pm,k}-\ell_{\pm,k}\big\|_{h^{1+\delta}\rightarrow h^{0}} }& \DIFadd{\geq c_{k,L}\!\ \vep_{N}^{\delta}, }& \DIFadd{\delta }& \DIFadd{\geq0.
}\end{align}
\DIFadd{This is the Brillouin-zone edge, i.e.~the same mechanism that the prefactor $\tfrac{\pi}{2L_{N}}|\!\sin(\tfrac{1}{2}\vep_{N}k)|^{-1}$ of Definition \ref{def:KSapprox} was designed to control. Conversely, for $0<1+\delta<\sigma_{K}$ one has
}\begin{align}
\DIFadd{\label{eq:KSrateupper}
\big\|\tilde{\ell}^{(N)}_{\pm,k}-\ell_{\pm,k}\big\|_{h^{1+\delta}\rightarrow h^{0}} }& \DIFadd{\leq C_{k,L,\delta}\!\ \vep_{N}^{\min\{\delta,1\}},
}\end{align}
\DIFadd{which follows from the splitting $\tilde{\ell}^{(N)}_{\pm,k}-\ell_{\pm,k} = R^{N}_{\infty}\big[\ell^{(N)}_{\pm,k}-(R^{N}_{\infty})^{*}\ell_{\pm,k}R^{N}_{\infty}\big](R^{N}_{\infty})^{*}+\big[P_{N}\ell_{\pm,k}P_{N}-\ell_{\pm,k}\big]$, $P_{N}:=R^{N}_{\infty}(R^{N}_{\infty})^{*}$ being the orthogonal projection onto the multiresolution space $\im R^{N}_{\infty}$: the second bracket is $O(\vep_{N}^{\delta})$ by the Jackson estimate $\|(1-P_{N})\|_{h^{1+\delta}\rightarrow h^{1}}\leq C\vep_{N}^{\delta}$ together with the $h^{1+\delta}$-stability of $P_{N}$ \mbox{%DIFAUXCMD
\cite{DaubechiesTenLecturesOn}}\hskip0pt%DIFAUXCMD
, while the first bracket is a Fourier multiplier on $\Gamma_{N,{\color{blue}\pm}}$ whose symbol deviates from $\pm(l\mp\tfrac{k}{2})$ by $O(\vep_{N}(|l|+|k|)^{2})$, the leading contribution being once more the centre-of-mass phase \eqref{eq:comphase}. Consequently the exponents in \eqref{eq:KSratelower} and \eqref{eq:KSrateupper} match for $0<\delta\leq1$, and under the re-centred comparison of Remark \ref{rem:recentring} the upper exponent improves to $\min\{\delta,2\}$. Numerically (with $L=\pi$, $k=2$, $_{4}s$) the operator norm is $1.365, 1.824, 1.107, 1.036, 1.035$ for $N=3,\dots,7$ at $\delta=0$, confirming that no convergence takes place there, while the measured rates after re-centring are $1.00$, $1.50$ and $2.00$ at $\delta=1,\tfrac{3}{2},\tfrac{5}{2}$, i.e.~exactly $\min\{\delta,2\}$; the constant in \eqref{eq:unifh1bound} is $C_{k,L}=1$ for $L=\pi$, approached from below.
}\end{rem}

\DIFaddend The lemma implies the convergence of the KS approximants in the standard Fock space representation ($c=0$).
\begin{thm}[Covergence of the KS approximants]
\label{thm:KSconv}
Let \DIFdelbegin \DIFdel{$s\in C^{\alpha}(\R)$ be a sufficiently regular, }\DIFdelend \DIFaddbegin \DIFadd{$s$ be a }\DIFaddend compactly supported orthonormal Daubechies scaling function \DIFaddbegin \DIFadd{which is $1$-regular in the sense of Definition \ref{def:regularity}}\DIFaddend . The KS approximants, $L^{(N)}_{\pm,k}$, converge strongly to the continuum Virasoro generators, $L_{\pm,k}$, on the dense, common core $\fF^{\alg}_{\ua}(\cD_{W})\subset\fF_{\ua}(\fh^{(\pm)}_{\infty,{\color{blue}\pm}})$ spanned by anti-symmetric Fock vectors with finitely many one-particle excitations in $\cD_{W}$ (finite $\cD_{W}$-particle number), i.e.:
\begin{align}
...
\end{align}
for all $n\in\N_{0}$ and $\xi_{1},\dots,\xi_{n}\in\cD_{W}$. \DIFaddbegin \DIFadd{In fact the convergence holds for all $\xi_{1},\dots,\xi_{n}\in h^{1}(\Gamma_{\infty,{\color{blue}\pm}}) = \cD(\ell_{\pm,k})$, and for $0<1+\delta<\sigma_{K}$ with the rate
}\begin{align}
\DIFadd{\label{eq:latviraconvrate}
\big\|(\alpha^{N}_{\infty}(L^{(N)}_{\pm,k}) - L_{\pm,k})\Phi\big\| }& \DIFadd{\leq \tfrac{L}{\pi}C_{k,L,\delta}\!\ \vep_{N}^{\min\{\delta,1\}}\big\|d\Gamma(\langle\!\ \cdot\!\ \rangle^{1+\delta})\Phi\big\|, }& \DIFadd{\langle m\rangle }& \DIFadd{:= 1+\tfrac{L}{\pi}|m|,
}\end{align}
\DIFadd{for all $\Phi$ in the domain of the second quantization $d\Gamma(\langle\!\ \cdot\!\ \rangle^{1+\delta})$ of the Sobolev weight. In particular \eqref{eq:latviraconv} holds on all of $\cD(d\Gamma(\langle\!\ \cdot\!\ \rangle))$, a domain which, unlike $\fF^{\alg}_{\ua}(\cD_{W})$, does not depend on the choice of scaling function.
}\DIFaddend \end{thm}
\begin{proof}
...
\DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:KShermiteanparts}%%%
dF_{S}(\tilde{\ell}^{(N)}_{k,\pm}) }& \DIFdel{=\ :\pi_{S}(\alpha^{N}_{\infty}(dF_{0}(\ell^{(N)}_{k,\pm}))):\ =\ :\pi_{S}(\alpha^{N}_{\infty}(L^{(N)}_{k,\pm})):. 
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:KShermiteanparts}
\tfrac{L}{\pi}dF_{S}(\tilde{\ell}^{(N)}_{k,\pm}) }& \DIFadd{=\ :\pi_{S}(\alpha^{N}_{\infty}(\tfrac{L}{\pi}dF_{0}(\ell^{(N)}_{k,\pm}))):\ =\ :\pi_{S}(\alpha^{N}_{\infty}(L^{(N)}_{k,\pm})):. 
}\end{align}\DIFaddend 
...
\DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:latvirachi1pmommod}%%%
\ell^{(N)}_{k,\pm} }& \DIFdel{= \tfrac{\pm1}{2\pi}\sum_{l\in\Gamma_{N,{\color{blue}\pm}}} \cos(\tfrac{1}{4}\vep_{N}k)^{2}\sin(\vep_{N}(l\mp\tfrac{k}{2}))\chi_{\Gamma_{N}}(l\mp k)e_{l\mp k}\otimes \overline{e}_{l}\;,
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:latvirachi1pmommod}
\ell^{(N)}_{k,\pm} }& \DIFadd{= \tfrac{\pm1}{2L}\sum_{l\in\Gamma_{N,{\color{blue}\pm}}} \cos(\tfrac{1}{4}\vep_{N}k)^{2}\sin(\vep_{N}(l\mp\tfrac{k}{2}))\chi_{\Gamma_{N}}(l\mp k)e_{l\mp k}\otimes \overline{e}_{l}\;,
}\end{align}\DIFaddend 
...
\DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:latvirachi1pmom}%%%
\tilde{\ell}^{(N)}_{k,\pm} }& \DIFdel{= \tfrac{\pm1}{2\pi}\sum_{l\in\Gamma_{N,{\color{blue}\pm}}} \cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(l\mp\frac{k}{2}))}{\vep_{N}}\chi_{\Gamma_{N}}(l\mp k)e_{l\mp k}\otimes \overline{e}_{l}\;.
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:latvirachi1pmom}
\tilde{\ell}^{(N)}_{k,\pm} }& \DIFadd{= \tfrac{\pm1}{2L}\sum_{l\in\Gamma_{N,{\color{blue}\pm}}} \cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(l\mp\frac{k}{2}))}{\vep_{N}}\chi_{\Gamma_{N}}(l\mp k)e_{l\mp k}\otimes \overline{e}_{l}\;.
}\end{align}\DIFaddend 
...
\DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:latvirachi1pmommat}%%%
(\tilde{\ell}^{(N)}_{\pm,k})_{mn} }&\DIFdel{\!=\!\pm\tfrac{L}{\pi}2L\delta_{m,n\mp k}\cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(n\mp\frac{k}{2}))}{\vep_{N}}\chi_{\Gamma_{N}(n\!\mp\!k)}\chi_{\Gamma_{N}}(n), }\\ \DIFdel{\nonumber
(\ell_{\pm,k})_{mn} }&\DIFdel{\!=\!\pm\tfrac{L}{\pi}2L\delta_{m,n\mp k}(n\!\mp\!\tfrac{k}{2}).
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:latvirachi1pmommat}
(\tilde{\ell}^{(N)}_{\pm,k})_{mn} }&\DIFadd{\!=\!\pm2L\delta_{m,n\mp k}\cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(n\mp\frac{k}{2}))}{\vep_{N}}\chi_{\Gamma_{N}(n\!\mp\!k)}\chi_{\Gamma_{N}}(n), }\\ \DIFadd{\nonumber
(\ell_{\pm,k})_{mn} }&\DIFadd{\!=\!\pm2L\delta_{m,n\mp k}(n\!\mp\!\tfrac{k}{2}).
}\end{align}\DIFaddend 
...
\DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:momrgestimates}%%%
}& \DIFdel{\|((\tilde{\ell}^{(N)}_{k,\pm}-\ell_{k,\pm})_{\pm\pm})^{(t)}R^{M}_{\infty}(\eta)\|_{\infty}^{2} }\\ \DIFdel{\nonumber
}& \DIFdel{\leq \|\eta\|_{\fh_{M}}^{2}(\tfrac{L}{\pi})^{2}\!\!\!\sum_{n\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!\!\chi_{\Gamma_{M}}(n)\theta(\pm(n\!\mp\!k))\theta(\pm n)\Big|\!\cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(n\mp\frac{k}{2}))}{\vep_{N}}\chi_{\Gamma_{N}(n\!\mp\!k)}\!-\!(n\!\mp\!\tfrac{k}{2})\Big|^{2}, }\\ \DIFdel{\nonumber
}& \DIFdel{\|(\tilde{\ell}^{(N)}_{k,\pm}-\ell_{k,\pm})_{\pm\mp}\|_{2}^{2} }\\ \DIFdel{\nonumber
}& \DIFdel{\leq (\tfrac{L}{\pi})^{2}\!\!\!\sum_{n\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!\theta(\pm(n\!\mp\!k))\theta(\mp n)\Big|\cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(n\mp\frac{k}{2}))}{\vep_{N}}\chi_{\Gamma_{N}(n\!\mp\!k)}\chi_{\Gamma_{N}}(n)\!-\!(n\!\mp\!\tfrac{k}{2})\Big|^{2}.
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:momrgestimates}
}& \DIFadd{\|((\tilde{\ell}^{(N)}_{k,\pm}-\ell_{k,\pm})_{\pm\pm})^{(t)}R^{M}_{\infty}(\eta)\|_{\infty}^{2} }\\ \DIFadd{\nonumber
}& \DIFadd{\leq \|\eta\|_{\fh_{M}}^{2}\!\!\!\sum_{n\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!\!\chi_{\Gamma_{M}}(n)\theta(\pm(n\!\mp\!k))\theta(\pm n)\Big|\!\cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(n\mp\frac{k}{2}))}{\vep_{N}}\chi_{\Gamma_{N}(n\!\mp\!k)}\!-\!(n\!\mp\!\tfrac{k}{2})\Big|^{2}, }\\ \DIFadd{\nonumber
}& \DIFadd{\|(\tilde{\ell}^{(N)}_{k,\pm}-\ell_{k,\pm})_{\pm\mp}\|_{2}^{2} }\\ \DIFadd{\nonumber
}& \DIFadd{\leq \!\!\!\sum_{n\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!\theta(\pm(n\!\mp\!k))\theta(\mp n)\Big|\cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(n\mp\frac{k}{2}))}{\vep_{N}}\chi_{\Gamma_{N}(n\!\mp\!k)}\chi_{\Gamma_{N}}(n)\!-\!(n\!\mp\!\tfrac{k}{2})\Big|^{2}.
}\end{align}\DIFaddend 
...
\end{align*} 
\DIFaddbegin \DIFadd{Moreover, all three statements hold for every $\xi\in h^{1}(\Gamma_{\infty,{\color{blue}\pm}}) = \cD(\ell_{\pm,k})$, and with the sharp rate
}\begin{align}
\DIFadd{\label{eq:momrgrate}
\big\|\tilde{\ell}^{(N)}_{\pm,k}-\ell_{\pm,k}\big\|_{h^{1+\delta}\rightarrow h^{0}} }& \DIFadd{\asymp \vep_{N}^{\min\{\delta,2\}}, }& \DIFadd{\big\|(\tilde{\ell}^{(N)}_{\pm,k}-\ell_{\pm,k})_{\pm\mp}\big\|_{2} }& \DIFadd{\leq \sqrt{C_{k}}\!\ \vep_{N}^{2},
}\end{align}
\DIFadd{for every $\delta>0$, with $C_{k}$ the explicit finite sum in \eqref{eq:momrgestimates}.
}\DIFaddend \end{lemma}
\DIFaddbegin \begin{proof}
\DIFadd{All assertions are immediate from \eqref{eq:latvirachi1pmommat}, because for the momentum-cutoff renormalization group $\tilde{\ell}^{(N)}_{\pm,k}-\ell_{\pm,k}$ is the Fourier multiplier with shift
}\begin{align*}
\DIFadd{d^{(N)}_{k}(n) }& \DIFadd{= \pm\Big(\cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(n\mp\frac{k}{2}))}{\vep_{N}}\chi_{\Gamma_{N}}(n\!\mp\!k)\chi_{\Gamma_{N}}(n) - (n\mp\tfrac{k}{2})\Big),
}\end{align*}
\DIFadd{so that $\|\tilde{\ell}^{(N)}_{\pm,k}-\ell_{\pm,k}\|_{h^{1+\delta}\rightarrow h^{0}} = \sup_{n}|d^{(N)}_{k}(n)|\langle n\rangle^{-(1+\delta)}$ exactly. Inside the Brillouin zone $|d^{(N)}_{k}(n)| = |n\mp\tfrac{k}{2}|\!\ |\!\cos(\tfrac{1}{4}\vep_{N}k)^{2}\sinc(\vep_{N}(n\mp\tfrac{k}{2}))-1| \leq |n\mp\tfrac{k}{2}|(\tfrac{\vep_{N}^{2}(n\mp\frac{k}{2})^{2}}{6}+\tfrac{(\vep_{N}k)^{2}}{16})$, which gives $\vep_{N}^{2}$ for $|n| = \cO(1)$ and $\vep_{N}^{\delta}$ at the zone boundary $|n|\sim\tfrac{\pi}{\vep_{N}}$; outside it $d^{(N)}_{k}(n) = \mp(n\mp\tfrac{k}{2})$ and the quotient is $\cO(|n|^{-\delta})\leq\cO(\vep_{N}^{\delta})$. The lower bound of the same order is the zone-boundary vector of Remark \ref{rem:KSconvrate}. The Hilbert-Schmidt estimate is \eqref{eq:momrgestimates}, whose index set $\{n:\pm n>0,\ \mp(n\mp k)>0\}$ is finite and contained in $|n|<|k|$, so that the same Taylor bound applies termwise. Finally $\|\tilde{\ell}^{(N)}_{\pm,k}\|_{h^{1}\rightarrow h^{0}}\leq\sup_{n}|n\mp\tfrac{k}{2}|\langle n\rangle^{-1}\leq C_{k,L}$ uniformly in $N$, and $\cD_{\std}$ is $h^{1}$-dense, which gives the extension to $h^{1}$ as in the proof of Lemma \ref{lem:KSconv}.
}\end{proof}
\DIFadd{We stress two points of contrast with the wavelet case. No regularity of the scaling function is assumed --- indeed no scaling function occurs in \eqref{eq:latvirachi1pmommat} at all --- and the exponent in \eqref{eq:momrgrate} is $\min\{\delta,2\}$ rather than $\min\{\delta,1\}$, because the momentum cutoff carries no centre-of-mass defect, cf.~Remark \ref{rem:recentring}. Numerically, for $L=\pi$ and $k=2$ the measured rates of $\|\tilde{\ell}^{(N)}_{\pm,k}-\ell_{\pm,k}\|_{h^{1+\delta}\rightarrow h^{0}}$ are $0.00, 0.50, 1.00, 2.00, 2.00$ at $\delta = 0,\tfrac{1}{2},1,2,3$, the value at $\delta=0$ being exactly $1$ for every $N$; the Hilbert-Schmidt norms decay at rate $2.00$ for $k=2,4,8$.
}

\DIFaddend Because of Remark \ref{rem:impcond}, similar estimates as \eqref{eq:noquantconv} and \eqref{eq:momrgestimates} also hold for $dQ_{S}$ in Majorana setting. Applying the lemma, we have:
...
\end{align}
for all $n\in\N_{0}$ and $\xi_{1},\dots,\xi_{n}\in\cD_{\std}$. \DIFaddbegin \DIFadd{Moreover, for every $\delta>0$ the convergence holds on all of $\cD(d\Gamma(\langle\!\ .\!\ \rangle^{1+\delta}))$ with the rate
}\begin{align}
\DIFadd{\label{eq:latviraconvcrate}
\big\|(:\!\pi_{{\color{blue}\pm}}(\alpha^{N}_{\infty}(L^{(N)}_{\pm,k})\!)\!: - L_{\pm,k})\Phi\big\| }& \DIFadd{\leq \tfrac{L}{\pi}\Big(C_{k,L,\delta}\!\ \vep_{N}^{\min\{\delta,2\}}\big\|d\Gamma(\langle\!\ .\!\ \rangle^{1+\delta})\Phi\big\| + \sqrt{C_{k}}\!\ \vep_{N}^{2}\big\|(\Nop+2)\Phi\big\|\Big).
}\end{align}
\DIFaddend \end{thm}
\DIFdelbegin \DIFdel{By Corollary \ref{cor:viraesa} and \eqref{eq:noquantconv}}\DIFdelend \DIFaddbegin \begin{proof}
\DIFadd{By \eqref{eq:KShermiteanparts}, $:\!\pi_{{\color{blue}\pm}}(\alpha^{N}_{\infty}(L^{(N)}_{\pm,k}))\!:-L_{\pm,k} = \tfrac{L}{\pi}dF_{{\color{blue}\pm}}(\tilde{\ell}^{(N)}_{\pm,k}-\ell_{\pm,k})$, so it suffices to insert $\tilde{o}^{(N)} = \tilde{\ell}^{(N)}_{\pm,k}$ and $o = \ell_{\pm,k}$ into \eqref{eq:noquantconv}. The two diagonal terms on the right-hand side of \eqref{eq:noquantconv} tend to $0$ by the second assertion of Lemma \ref{lem:KSconvc}, and the two Hilbert-Schmidt terms by its third assertion; this gives \eqref{eq:latviraconvc}. For \eqref{eq:latviraconvcrate} one inserts \eqref{eq:momrgrate} into \eqref{eq:noquantconv} instead, bounds $\sum_{p}\|\eta_{p}\|_{h^{1+\delta}}\prod_{q\neq p}\|\eta_{q}\|$ by $\|d\Gamma(\langle\!\ .\!\ \rangle^{1+\delta})\Phi\|$ and $(n+2)$ by $\Nop+2$ exactly as in the proof of Theorem \ref{thm:KSconv}, and extends from $\fF^{\alg}_{\ua}(\cD_{\std})$ by linearity and density.
}\end{proof}

\DIFadd{Before turning to the associated unitary groups we record an energy bound which is uniform in the scale and which will be used repeatedly; for the smeared approximants it is extended in Proposition \ref{hyp:energybound} below. Recall the one-particle Sobolev weight $A:=\langle\!\ .\!\ \rangle$}\DIFaddend , \DIFaddbegin \DIFadd{$\langle m\rangle = 1+\tfrac{L}{\pi}|m|$, of \eqref{eq:latviraconvrate}.
}

\begin{lemma}[Uniform energy bound at fixed $k$]
\label{lem:energybound1k}
\DIFadd{Let $G$ be any of $\tilde{\ell}^{(N)}_{\pm,k}$, $\ell_{\pm,k}$ and their real and imaginary parts \eqref{eq:contvira1pRI}, \eqref{eq:latvira1pRI}. Then
}\begin{align}
\DIFadd{\label{eq:relbound1k}
\big\|A^{-\frac{1}{2}}GA^{-\frac{1}{2}}\big\| }& \DIFadd{\leq C\langle k\rangle^{\frac{1}{2}}, }& \DIFadd{\big\|G_{+-}\big\|_{2}+\big\|G_{-+}\big\|_{2} }& \DIFadd{\leq \gamma_{k} < \infty,
}\end{align}
\DIFadd{with $C$ and $\gamma_{k}$ independent of $N$, and consequently
}\begin{align}
\DIFadd{\label{eq:energybound1k}
\big\|\tfrac{L}{\pi}dF_{{\color{blue}\pm}}(G)\Psi\big\| }& \DIFadd{\leq C_{k}\big\|(\1+L_{\pm,0})\Psi\big\|, }& \DIFadd{\pm\tfrac{L}{\pi}dF_{{\color{blue}\pm}}(\Re\!\ G) }& \DIFadd{\leq C_{k}(\1+L_{\pm,0}),
}\end{align}
\DIFadd{for all $\Psi\in\cD(L_{\pm,0})$ and all $N$, and likewise for $\Im\!\ G$. No regularity of the scaling function is required.
}\end{lemma}
\begin{proof}
\DIFadd{By \eqref{eq:latvirachi1pmommat} the matrix elements of $\tilde{\ell}^{(N)}_{\pm,k}$ and of $\ell_{\pm,k}$ are supported on $m=n\mp k$ and bounded in modulus by $|n\mp\tfrac{k}{2}|$, so that
}\begin{align*}
\DIFadd{\big\|A^{-\frac{1}{2}}GA^{-\frac{1}{2}}\big\| }& \DIFadd{\leq \sup_{n\in\Gamma_{\infty,{\color{blue}\pm}}}\tfrac{|n\mp\frac{k}{2}|}{\langle n\mp k\rangle^{\frac{1}{2}}\langle n\rangle^{\frac{1}{2}}} \leq C\langle k\rangle^{\frac{1}{2}}:
}\end{align*}
\DIFadd{the quotient tends to $1$ as $|n|\rightarrow\infty$, vanishes at $n=\pm\tfrac{k}{2}$, and is largest --- of order $|k|^{\frac{1}{2}}$ --- near $n=0$ and $n=\pm k$, as follows from $|n\mp\tfrac{k}{2}|\leq\tfrac{1}{2}(|n|+|n\mp k|)$ and $|n|\leq|n\mp k|+|k|$. The second estimate in \eqref{eq:relbound1k} holds because $\|(\ell_{\pm,k})_{\pm\mp}\|_{2}^{2}$ is the finite sum in \eqref{eq:momrgestimates}, the index set $\{n:\pm n>0,\ \mp(n\mp k)>0\}$ being contained in $|n|<|k|$, while $\|(\tilde{\ell}^{(N)}_{\pm,k})_{\pm\mp}\|_{2}$ converges to it by Lemma \ref{lem:KSconvc}, hence is bounded uniformly in $N$. The passage to real and imaginary parts is immediate from $\ell^{*}_{\pm,k}=\ell_{\pm,-k}$ and $\|A^{-\frac{1}{2}}G^{*}A^{-\frac{1}{2}}\| = \|A^{-\frac{1}{2}}GA^{-\frac{1}{2}}\|$.
}

\DIFadd{For \eqref{eq:energybound1k} we split $dF_{{\color{blue}\pm}}(G)$ as in \eqref{eq:secondquantarakino}. Since $A$ commutes with the conjugation $J$ and compressions preserve the operator order, the first estimate in \eqref{eq:relbound1k} gives $\pm(\Re\!\ G)_{++}\leq C\langle k\rangle^{\frac{1}{2}}A$ and likewise for $(\!\ .\!\ )^{t}_{--}$, whence by monotonicity of $d\Gamma$
}\begin{align*}
\DIFadd{\big\|\big(a^{\dag}G_{++}a-a^{\dag}G^{t}_{--}a\big)\Psi\big\| }& \DIFadd{\leq 4\!\ C\langle k\rangle^{\frac{1}{2}}\big\|d\Gamma(A)\Psi\big\|,
}\end{align*}
\DIFadd{and separately $\pm(\Re\!\ G)_{++}\leq C\langle k\rangle^{\frac{1}{2}}A$ yields the stated form bound. The pair terms are controlled by \eqref{eq:hilbertschmidt}, $\|(a^{\dag}G_{+-}a^{\dag}+aG_{-+}a)\Psi\|\leq2\gamma_{k}\|(\Nop+2)\Psi\|$ with $\Nop$ the number operator. Finally the representations $\pi_{{\color{blue}\pm}}$ have positive energy: by \eqref{eq:viraCRcxrep} respectively \eqref{eq:viraCRcxrepR}, $L_{\pm,0}=\tfrac{L}{\pi}dF_{{\color{blue}\pm}}(\ell_{\pm,0})$ is the second quantization of $|m|$ relative to the particle/hole decomposition determined by $S$, so that $d\Gamma(A)=\Nop+L_{\pm,0}$. In the Neveu-Schwarz sector $|m|\geq\tfrac{\pi}{2L}$ on $\Gamma_{\infty,{\color{blue}-}}$, whence $\Nop\leq2L_{\pm,0}$ and $d\Gamma(A)\leq3L_{\pm,0}$; in the Ramond sector the zero mode spans a two-dimensional subspace of the doubled one-particle space \eqref{eq:doubleprojR} and can be occupied at most twice by the Pauli principle, so that $\Nop\leq2\!\ \1+L_{\pm,0}$ and $d\Gamma(A)\leq2\!\ \1+2L_{\pm,0}$. Collecting the three estimates gives \eqref{eq:energybound1k}.
}\end{proof}

\DIFadd{For the passage to the associated unitary groups one needs the real and imaginary parts of $L_{\pm,k}$ to be essentially self-adjoint on }\DIFaddend $\fF^{\alg}_{\ua}(\cD_{\std})$\DIFdelbegin \DIFdel{contains a total set of analytic vectors for $L_{\pm,k}$, and by }\DIFdelend \DIFaddbegin \DIFadd{. Corollary \ref{cor:viraesa} provides this at the one-particle level, but in $\pi_{{\color{blue}\pm}}$ the pair terms $a^{\dag}G_{+-}a^{\dag}$, $aG_{-+}a$ of \eqref{eq:secondquantarakino} raise the particle number, and iterating \eqref{eq:hilbertschmidt} together with \eqref{eq:analyticvector} produces a factor $(j!)^{2}$ after $j$ steps, so that the plane-wave Fock vectors cannot be shown to be }\emph{\DIFadd{analytic}} \DIFadd{vectors in this way. We therefore replace }\DIFaddend Nelson's analytic vector theorem \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{NelsonAnalyticVectors} }\hskip0pt%DIFAUXCMD
the real and imaginary parts of $L_{\pm,k}$ are }\DIFdelend \DIFaddbegin \DIFadd{by his commutator theorem \mbox{%DIFAUXCMD
\cite[Theorem X.37]{ReedMethodsOfModern2}}\hskip0pt%DIFAUXCMD
, which is available because of Lemma \ref{lem:energybound1k}.
}

\begin{cor}[Essential self-adjointness in $\pi_{{\color{blue}\pm}}$]
\label{cor:viraesac}
\DIFadd{Let $T$ be the closure of $\tfrac{L}{\pi}dF_{{\color{blue}\pm}}(r_{\pm,k})$ or of $\tfrac{L}{\pi}dF_{{\color{blue}\pm}}(\iota_{\pm,k})$, and put $\cN:=\1+L_{\pm,0}$. Then $T$ is }\DIFaddend essentially self-adjoint on \DIFaddbegin \DIFadd{every core for $\cN$, in particular on }\DIFaddend $\fF^{\alg}_{\ua}(\cD_{\std})$ \DIFdelbegin \DIFdel{. This }\DIFdelend \DIFaddbegin \DIFadd{and on $\cD(L_{\pm,0})$.
}\end{cor}
\begin{proof}
\DIFadd{$\cN$ is self-adjoint with $\cN\geq\1$, since the representations $\pi_{{\color{blue}\pm}}$ have positive energy, and $\fF^{\alg}_{\ua}(\cD_{\std})$ is a core for $\cN$ because $L_{\pm,0}$ is diagonal in the plane-wave occupation basis. Condition (i) of \mbox{%DIFAUXCMD
\cite[Theorem X.37]{ReedMethodsOfModern2}}\hskip0pt%DIFAUXCMD
, $\|T\Phi\|\leq c\|\cN\Phi\|$, is the first estimate of \eqref{eq:energybound1k}. For condition (ii) we use the Virasoro relation \eqref{eq:viraCRcxrep}, $[L_{\pm,0},L_{\pm,k}] = -\tfrac{L}{\pi}k\!\ L_{\pm,k}$, which gives $[\cN,\tfrac{L}{\pi}dF_{{\color{blue}\pm}}(r_{\pm,k})] = -i\tfrac{L}{\pi}k\cdot\tfrac{L}{\pi}dF_{{\color{blue}\pm}}(\iota_{\pm,k})$ and the analogous identity with $r$ and $\iota$ interchanged. Hence
}\begin{align*}
\DIFadd{\big|\langle T\Phi,\cN\Phi\rangle-\langle\cN\Phi,T\Phi\rangle\big| }& \DIFadd{= \big|\langle\Phi,}[\DIFadd{\cN,T}]\DIFadd{\Phi\rangle\big| \leq \tfrac{L}{\pi}|k|\!\ \big|\langle\Phi,T'\Phi\rangle\big| \leq 3\!\ c\!\ \tfrac{L}{\pi}|k|\!\ \big\|\cN^{\frac{1}{2}}\Phi\big\|^{2},
}\end{align*}
\DIFadd{where $T'$ is the partner operator and the last step is the }\emph{\DIFadd{form}} \DIFadd{bound $\pm T'\leq C_{k}(\1+L_{\pm,0}) = C_{k}\cN$ supplied by the second estimate of \eqref{eq:energybound1k}.
}\end{proof}
\DIFadd{Corollary \ref{cor:viraesac} together with Theorem \ref{thm:KSconvc} }\DIFaddend implies strong resolvent convergence of the KS approximants and, therefore \cite{ReedMethodsOfModern1}\DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{, the convergence of the associated unitary groups. That argument is however not quantitative. Since the energy bound of Lemma \ref{lem:energybound1k} is at our disposal, we can instead obtain the convergence of the unitary groups --- and with it of all the Bogoliubov transformations of this section --- with an explicit rate and an explicit dependence on $t$, valid for all $t\in\R$ rather than only for small $t$ as in \eqref{eq:conformalerror}. The mechanism is Duhamel's formula combined with the fact that the energy grows at most exponentially along the flow.
}

\begin{lemma}[Energy growth and Duhamel]
\label{lem:duhamel}
\DIFadd{Let $T$, $T_{N}$ be the closures of $\tfrac{L}{\pi}dF_{{\color{blue}\pm}}(o)$ and $\tfrac{L}{\pi}dF_{{\color{blue}\pm}}(\tilde{o}^{(N)})$ for $o=r_{\pm,k},\iota_{\pm,k}$ and $\tilde{o}^{(N)}=\tilde{r}^{(N)}_{\pm,k},\tilde{\iota}^{(N)}_{\pm,k}$, let $\cN:=\1+L_{\pm,0}$ and let $p\in\N$. Then:
}\begin{enumerate}
\item[\DIFadd{(i)}] \DIFadd{there is $c_{k}\geq0$, with $c_{0}=0$, such that
}\begin{align}
\DIFadd{\label{eq:energygrowth}
\big\|\cN^{p}e^{i\tau T}\Phi\big\| }& \DIFadd{\leq e^{p\!\ c_{k}|\tau|}\big\|\cN^{p}\Phi\big\|, }& \DIFadd{\Phi }& \DIFadd{\in\cD(\cN^{p}),\ \tau\in\R;
}\end{align}
\item[\DIFadd{(ii)}] \DIFadd{if $\|(T_{N}-T)\Psi\|\leq\varkappa_{N}\|\cN^{p}\Psi\|$ for all $\Psi\in\cD(\cN^{p})$, then
}\begin{align}
\DIFadd{\label{eq:duhamel}
\big\|(e^{itT_{N}}-e^{itT})\Phi\big\| }& \DIFadd{\leq \varkappa_{N}\!\ \Lambda_{k,p}(t)\!\ \big\|\cN^{p}\Phi\big\|, }& \DIFadd{\Lambda_{k,p}(t) }& \DIFadd{:= \tfrac{e^{p\!\ c_{k}|t|}-1}{p\!\ c_{k}} \ \big(=|t|\textup{ for }k=0\big),
}\end{align}
\DIFadd{for all $\Phi\in\cD(\cN^{p})$ and all $t\in\R$.
}\end{enumerate}
\end{lemma}
\begin{proof}
\DIFadd{(i) By the Virasoro relations \eqref{eq:viraCRcxrep}, $[L_{\pm,0},L_{\pm,k}]=-\tfrac{L}{\pi}k L_{\pm,k}$, so $i[T,\cN]$ is $\tfrac{L}{\pi}k$ times the partner operator $T'$ ($T'=\iota$-part if $T=r$-part and conversely), and $\pm T'\leq C_{k}\cN$ in the form sense by the second estimate of \eqref{eq:energybound1k}. Hence $\pm i[T,\cN]\leq c_{k}\cN$ with $c_{k}:=\tfrac{L}{\pi}|k|C_{k}$, and $c_{0}=0$ because $L_{\pm,0}$ commutes with itself. Writing $\Phi_{\tau}:=e^{i\tau T}\Phi$ and $f(\tau):=\|\cN^{\frac{p}{2}}\Phi_{\tau}\|^{2}$ we get $|f'(\tau)| = |\langle\Phi_{\tau},i[T,\cN^{p}]\Phi_{\tau}\rangle|\leq p\!\ c_{k}f(\tau)$, and \eqref{eq:energygrowth} follows by Gr\"onwall's inequality after replacing $p$ by $2p$.
(ii) By the energy bound $\cD(\cN^{p})\subset\cD(T_{N})\cap\cD(T)$, and by (i) $e^{i\tau T}$ leaves $\cD(\cN^{p})$ invariant, so Duhamel's formula
}\begin{align*}
\DIFadd{e^{itT_{N}}-e^{itT} }& \DIFadd{= i\int_{0}^{t}e^{i(t-\tau)T_{N}}(T_{N}-T)e^{i\tau T}\!\ d\tau
}\end{align*}
\DIFadd{holds strongly on $\cD(\cN^{p})$. Estimating the integrand with the hypothesis and \eqref{eq:energygrowth} gives $\|(e^{itT_{N}}-e^{itT})\Phi\|\leq\varkappa_{N}\int_{0}^{|t|}e^{p\!\ c_{k}\tau}d\tau\!\ \|\cN^{p}\Phi\|$, which is \eqref{eq:duhamel}.
}\end{proof}
\DIFadd{Combining Lemma \ref{lem:duhamel} with the rates of Theorem \ref{thm:KSconvc} --- note that $d\Gamma(\langle\!\ .\!\ \rangle^{1+\delta})\leq d\Gamma(\langle\!\ .\!\ \rangle)^{1+\delta}\leq C\cN^{p}$ for $p\geq1+\delta$ by Step 3 of the proof of Lemma \ref{lem:energybound1k} --- we obtain the quantitative form of the convergence of the unitary groups.
}\DIFaddend \begin{cor}[Convergence of KS unitaries]
...
\end{align*}
for all $\phi\in\fF_{\ua}(\fh^{(\pm)}_{\infty,{\color{blue}\pm}})$ uniformly on compact intervals in $t\in\R$. \DIFaddbegin \DIFadd{Quantitatively, for every $\delta>0$ and every integer $p\geq1+\delta$,
}\begin{align}
\DIFadd{\label{eq:KSunitariescrate}
\big\|(U^{(N)}_{t}-U_{t})\Phi\big\| }& \DIFadd{\leq \tfrac{L}{\pi}\big(C_{k,L,\delta}\!\ \vep_{N}^{\min\{\delta,2\}}+\sqrt{C_{k}}\!\ \vep_{N}^{2}\big)\!\ \Lambda_{k,p}(t)\!\ \big\|(\1+L_{\pm,0})^{p}\Phi\big\|
}\end{align}
\DIFadd{for all $\Phi\in\cD((\1+L_{\pm,0})^{p})$ and all $t\in\R$, with $\Lambda_{k,p}$ as in \eqref{eq:duhamel}; in particular $\Lambda_{0,p}(t)=|t|$ for the chiral time evolution, so that the error grows only linearly in $t$ there.
}\DIFaddend \end{cor}
\DIFaddbegin \DIFadd{The same argument, with \eqref{eq:latviraconvsmearedrate} in place of \eqref{eq:latviraconvcrate} and Proposition \ref{hyp:energybound} in place of Lemma \ref{lem:energybound1k}, gives the corresponding explicit bounds for the smeared unitaries of Corollary \ref{cor:KSconvsmeared}. The Bogoliubov transformations of Corollaries \ref{cor:KSconvbog} and \ref{cor:KSconvbogc} are one-particle statements by \eqref{eq:KSmultiestimate} and have already been made quantitative in \eqref{eq:KSconvbograte} --- in the present, momentum-cutoff case the exponent $\min\{\delta,1\}$ there improves to $\min\{\delta,2\}$ by \eqref{eq:momrgrate} --- and the smeared case is Remark \ref{rem:duhamel1psmeared}. For the derivations of Corollary \ref{cor:KSderivationconv} no $t$-dependence occurs at all: \eqref{eq:KSmultiestimatederivation} together with \eqref{eq:momrgrate} gives directly
}\begin{align}
\DIFadd{\label{eq:KSconvbogderrate}
\big\|}[\DIFadd{:\!dF_{S}(\tilde{o}^{(N)})\!:,A}]\DIFadd{-}[\DIFadd{:\!dF_{S}(o)\!:,A}]\DIFadd{\big\| }& \DIFadd{\leq C_{k,L,\delta}\!\ \vep_{N}^{\min\{\delta,2\}}\Big(\prod_{l=1}^{n+m}\|\xi_{l}\|_{\infty}\Big)\sum_{q=1}^{n+m}\tfrac{\|\xi_{q}\|_{h^{1+\delta}}}{\|\xi_{q}\|_{\infty}}
}\end{align}
\DIFadd{for $A=a^{\dagger}(\xi_{1})\dots a(\xi_{n+m})$.
}\DIFaddend As in the previous section, Lemma \ref{lem:KSconvc} implies the following convergence statement for the Bogoliubov transformations \eqref{eq:KSconvgroup}:
...
Let \DIFdelbegin \DIFdel{$s\in C^{\alpha}(\R)$ be a sufficiently regular, }\DIFdelend \DIFaddbegin \DIFadd{$s$ be a }\DIFaddend compactly supported orthonormal Daubechies scaling function \DIFaddbegin \DIFadd{which is $\tfrac{3}{2}$-regular in the sense of Definition \ref{def:regularity}, and $\tfrac{5}{2}$-regular for the rate \eqref{eq:smearedrate}}\DIFaddend .  For $M\in\N_{0}$ and $X\in\fl(\Lambda_{M},\C)$, we consider,
\begin{align*}
...
\DIFdelbegin \begin{align*}
\DIFdel{\|(\tilde{\ell}^{(N)}_{\pm,k})_{\pm\mp}\|_{2}^{2} }& \DIFdel{= (\tfrac{L}{\pi})^{2}\!\!\!\!\sum_{n\in\Gamma_{N,{\color{blue}\pm}}}\!\!\!\!\theta(\pm n\!-\!k)\theta(\mp n)\cos(\tfrac{1}{4}\vep_{N}k)^{4}\Big(\tfrac{\sin(\vep_{N}(n\mp\frac{k}{2}))}{\vep_{N}}\Big)^{2}\chi_{\Gamma_{N}(n\!\mp\!k)}, }\\
\DIFdel{\|(\ell_{\pm,k})_{\pm\mp}\|_{2}^{2} }& \DIFdel{= (\tfrac{L}{\pi})^{2}\!\!\!\!\sum_{n\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!\!\!\theta(\pm n\!-\!k)\theta(\mp n)(n\!\mp\!\tfrac{k}{2})^{2}.
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align*}
\DIFadd{\|(\tilde{\ell}^{(N)}_{\pm,k})_{\pm\mp}\|_{2}^{2} }& \DIFadd{= \!\!\!\!\sum_{n\in\Gamma_{N,{\color{blue}\pm}}}\!\!\!\!\theta(\pm n\!-\!k)\theta(\mp n)\cos(\tfrac{1}{4}\vep_{N}k)^{4}\Big(\tfrac{\sin(\vep_{N}(n\mp\frac{k}{2}))}{\vep_{N}}\Big)^{2}\chi_{\Gamma_{N}(n\!\mp\!k)}, }\\
\DIFadd{\|(\ell_{\pm,k})_{\pm\mp}\|_{2}^{2} }& \DIFadd{= \!\!\!\!\sum_{n\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!\!\!\theta(\pm n\!-\!k)\theta(\mp n)(n\!\mp\!\tfrac{k}{2})^{2}.
}\end{align*}\DIFaddend 
\DIFaddbegin 

\emph{\DIFadd{Structure of the two errors, uniform bounds, and rates.}} \DIFadd{It is worth isolating what the two brackets above actually contain. Using $\tfrac{1}{2L_{N}}2^{N-M}=\tfrac{1}{2L_{M}}=\tfrac{\vep_{M}}{2L}$ and Definition \ref{def:waveletrgcur}, both smeared operators carry the same prefactor,
}\begin{align}
\DIFadd{\label{eq:smearedweights}
\tilde{\ell}^{(N)}_{\pm}(S^{M}_{N}(X)) }& \DIFadd{= \tfrac{1}{2L_{M}}\!\!\!\sum_{k\in\Gamma_{N,+}}\!\!\!w^{(N)}_{k}\hat{X}_{\per}(k)\!\ \tilde{\ell}^{(N)}_{\pm,k}, }& \DIFadd{\ell_{\pm}(S^{M}_{\infty}(X)) }& \DIFadd{= \tfrac{1}{2L_{M}}\!\!\!\sum_{k\in\Gamma_{\infty,+}}\!\!\!w^{(\infty)}_{k}\hat{X}_{\per}(k)\!\ \ell_{\pm,k},
}\end{align}
\DIFadd{with the }\emph{\DIFadd{loop weights}} \DIFadd{$w^{(N)}_{k}:=\prod_{j=1}^{N-M}m_{0}(\vep_{M+j}k)$ and $w^{(\infty)}_{k}:=\hat{s}(\vep_{M}k)$. The scaling relation \eqref{eq:infprod} gives them in closed form relative to one another,
}\begin{align}
\DIFadd{\label{eq:smearweightid}
w^{(N)}_{k}-w^{(\infty)}_{k} }& \DIFadd{= w^{(N)}_{k}\big(1-\hat{s}(\vep_{N}k)\big),
}\end{align}
\DIFadd{so that the first bracket --- the }\emph{\DIFadd{smearing}} \DIFadd{error --- is governed by the deviation of $\hat{s}$ from $1$ at the }\emph{\DIFadd{fine}} \DIFadd{scale, and is of first order, $|1-\hat{s}(\vep_{N}k)|\leq\|\hat{s}'\|_{\infty}\vep_{N}|k|$. The second bracket --- the }\emph{\DIFadd{generator}} \DIFadd{error --- is of order $\vep_{N}^{\min\{\delta,2\}}$ by Lemma \ref{lem:KSconvc}, the fermions being renormalized with the momentum cutoff. Since $\|\tilde{\ell}^{(N)}_{\pm,k}\|_{h^{1}\rightarrow h^{0}}\leq\sup_{n}|n\mp\tfrac{k}{2}|\langle n\rangle^{-1}\leq C\langle k\rangle$ uniformly in $N$, and since $|w^{(N)}_{k}|\leq C_{K}(1+\vep_{M}|k|)^{-\rho}$ on $\Gamma_{N,+}$ by Corollary \ref{cor:convergence}, we obtain
}\begin{align}
\DIFadd{\label{eq:smearedunifbound}
\sup_{N>M}\big\|\tilde{\ell}^{(N)}_{\pm}(S^{M}_{N}(X))\big\|_{h^{1}\rightarrow h^{0}} }& \DIFadd{\leq C_{X,M} < \infty
}\end{align}
\DIFadd{as soon as $s$ is $\tfrac{3}{2}$-regular, and correspondingly all three statements of the lemma extend from $\cD_{\std}$ to all of $h^{1}(\Gamma_{\infty,{\color{blue}\pm}})$ by density, as in the proof of Lemma \ref{lem:KSconv}. Combining the two brackets,
}\begin{align}
\DIFadd{\label{eq:smearedrate}
\big\|\tilde{\ell}^{(N)}_{\pm}(S^{M}_{N}(X))-\ell_{\pm}(S^{M}_{\infty}(X))\big\|_{h^{1+\delta}\rightarrow h^{0}} }& \DIFadd{\leq C_{X,M,\delta}\!\ \vep_{N}^{\min\{\delta,1\}},
}\end{align}
\DIFadd{for every $\delta>0$ for which $s$ is $\tfrac{5}{2}$-regular, the exponent $1$ coming from \eqref{eq:smearweightid} alone.
}\DIFaddend \end{proof}
\DIFaddbegin \begin{rem}[The smearing error is a phase]
\label{rem:smearphase}
\DIFadd{The first-order term in \eqref{eq:smearweightid} is once more the centre-of-mass offset of Remark \ref{rem:recentring}, and here the structure is particularly transparent. Orthonormality gives $|m_{0}(\xi)|^{2}+|m_{0}(\xi+\pi)|^{2}=1$, while $m_{0}$ has a zero of order $K$ at $\pi$; hence $|m_{0}(\xi)|=1+\cO(\xi^{2K})$ and therefore
}\begin{align}
\DIFadd{\label{eq:shatflat}
|\hat{s}(\xi)| }& \DIFadd{= \prod_{j\geq1}|m_{0}(2^{-j}\xi)| = 1+\cO(\xi^{2K}).
}\end{align}
\DIFadd{Thus $\hat{s}(\xi)=e^{i\varphi(\xi)}(1+\cO(\xi^{2K}))$ with $\varphi$ real and odd, $\varphi(\xi)=-\mu(s)\xi+\cO(\xi^{3})$: }\emph{\DIFadd{the entire smearing error is a phase}}\DIFadd{, and the re-centred comparison of Remark \ref{rem:recentring} reduces \eqref{eq:smearweightid} from first to }\emph{\DIFadd{third}} \DIFadd{order,
}\begin{align*}
\DIFadd{1-e^{i\mu(s)\vep_{N}k}\hat{s}(\vep_{N}k) }& \DIFadd{= \cO((\vep_{N}k)^{3}).
}\end{align*}
\DIFadd{Consequently the exponent in \eqref{eq:smearedrate} improves to $\min\{\delta,2\}$ under re-centring --- the generator error of Lemma \ref{lem:KSconvc} then being the only remaining source --- so that the smeared wavelet route loses nothing against the momentum-cutoff one. Numerically, $|1-\hat{s}(\vep_{N}k)|$ decays at rate $1.00$ and its re-centred counterpart at rate $3.00$ for $_{4}s$ and $_{8}s$ alike, while the measured rates of \eqref{eq:smearedrate} for $_{8}s$ are $0.95$ at $\delta=1$ and $1.86$ at $\delta=2$ after re-centring, approaching $\min\{\delta,2\}$. The flatness \eqref{eq:shatflat} is directly visible for $_{2}s$, where the measured order is $4.00=2K$; for $K\geq4$ it lies below machine precision.
}\end{rem}
\DIFaddend As before, we use the preceding lemma to deduce the following result on the convergence of the KS approximants (cf.~\eqref{eq:noquantconv}).
\begin{thm}[Convergence of smeared KS approximants for $\pi_{{\color{blue}\pm}}$]
\label{thm:KSconvsmeared}
Let \DIFdelbegin \DIFdel{$s\in C^{\alpha}(\R)$ be a sufficiently regular, }\DIFdelend \DIFaddbegin \DIFadd{$s$ be a }\DIFaddend compactly supported orthonormal Daubechies scaling function \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{which is $\tfrac{3}{2}$-regular in the sense of Definition \ref{def:regularity} --- by Remark \ref{rem:regularityvalues} this holds for $K\geq4$ --- }\DIFaddend and \DIFaddbegin \DIFadd{$\tfrac{5}{2}$-regular for the rate \eqref{eq:latviraconvsmearedrate}, i.e.~$K\geq6$; and }\DIFaddend let $X\in\fl(\Lambda_{M},\C)$ for some $M\in\N_{0}$. Then, the smeared KS approximants, $L^{(N)}_{\pm}(S^{M}_{N}(X))$, converge strongly to the continuum Virasoro generators, $L_{\pm}(S^{M}_{\infty}(X))$, on the dense domain $\fF^{\alg}_{\ua}(\cD_{\std})\subset\fF_{\ua}(\fh^{(\pm)}_{\infty,{\color{blue}\pm}})$ spanned by anti-symmetric Fock vectors with finitely many one-particle excitations in $\cD_{\std}$ (finite $\cD_{\std}$-particle number):
\begin{align}
...
\end{align}
for all $n\in\N_{0}$ and $\xi_{1},\dots,\xi_{n}\in\cD_{\std}$. \DIFaddbegin \DIFadd{Moreover, exactly as in Theorem \ref{thm:KSconvc}, the convergence holds on all of $\cD(d\Gamma(\langle\!\ .\!\ \rangle^{1+\delta}))$ with the rate
}\begin{align}
\DIFadd{\label{eq:latviraconvsmearedrate}
}& \DIFadd{\big\|(:\!\pi_{{\color{blue}\pm}}(\alpha^{N}_{\infty}(L^{(N)}_{\pm}(S^{M}_{N}(X)))\!)\!: - L_{\pm}(S^{M}_{\infty}(X)))\Phi\big\| }\\ \DIFadd{\nonumber
}& \DIFadd{\hspace{2cm} \leq \tfrac{L}{\pi}\Big(C_{X,M,\delta}\!\ \vep_{N}^{\min\{\delta,1\}}\big\|d\Gamma(\langle\!\ .\!\ \rangle^{1+\delta})\Phi\big\| + \gamma^{(N)}_{X}\big\|(\Nop+2)\Phi\big\|\Big),
}\end{align}
\DIFadd{where $\gamma^{(N)}_{X}:=\|(\tilde{\ell}^{(N)}_{\pm}(S^{M}_{N}(X))-\ell_{\pm}(S^{M}_{\infty}(X)))_{+-}\|_{2}+\|(\!\ .\!\ )_{-+}\|_{2}\rightarrow0$ by the third statement of Lemma \ref{lem:KSconvsmeared}, and where the exponent improves to $\min\{\delta,2\}$ under the re-centred comparison of Remark \ref{rem:smearphase}.
}\DIFaddend \end{thm}
...
\end{align*}
for $X_{N}\in\fl(\Lambda_{N},\C)$ and $X\in\fl^{\alpha}(S^{1}_{L},\C)$ ($\alpha>0$). Then, assuming we have a $S$-convergent sequence of smearing functions $X_{N}\stackrel{N\rightarrow\infty}{\rightarrow} X$ in the sense of Definition \ref{def:convseq} for a suitable sequence of \DIFdelbegin \DIFdel{Sobolev-type semi-norm $\{p_{N,\delta}\}_{N\in\N_{0}}$}\DIFdelend \DIFaddbegin \DIFadd{Wiener-type semi-norms $\{p_{N,m}\}_{N\in\N_{0}}$ as in \eqref{eq:loopseminorms} below}\DIFaddend ,
\begin{align*}
\lim_{M\rightarrow\infty}\limsup_{N\rightarrow\infty}p_{N,\delta}(X_{N}-S^{M}_{N}(X_{M})\!) & = 0,
\end{align*}
we conclude the convergence of the KS approximants as in \eqref{eq:latviraconvsmeared}. \DIFdelbegin \DIFdel{In principle this allows us to }\DIFdelend \DIFaddbegin \DIFadd{The first and the third term of the splitting can in fact be controlled explicitly, so that this is a statement rather than a hope: we carry it out in Lemma \ref{lem:loopjackson} and Theorem \ref{thm:KSreconstruction}, which }\DIFaddend reconstruct the smeared Virasoro generators for \DIFdelbegin \DIFdel{smooth smearing functions }\DIFdelend \DIFaddbegin \DIFadd{every smooth smearing function }\DIFaddend $X\in\fl^{\infty}(S^{1}_{L},\C)$.
\end{rem}
\DIFaddbegin 

\DIFadd{For the first and the third term of the splitting we use the Wiener-type semi-norms
}\begin{align}
\DIFadd{\label{eq:loopseminorms}
p_{N,m}(Y) }& \DIFadd{:= \tfrac{1}{2L_{N}}\!\!\!\sum_{k\in\Gamma_{N,{\color{blue}+}}}\!\!\!\langle k\rangle^{m}|\hat{Y}_{k}|, }& \DIFadd{p_{\infty,m}(Z) }& \DIFadd{:= \tfrac{1}{2L}\!\!\!\sum_{k\in\Gamma_{\infty,{\color{blue}+}}}\!\!\!\langle k\rangle^{m}|\hat{Z}_{k}|,
}\end{align}
\DIFadd{for $Y\in\fl(\Lambda_{N},\C)$ and $Z\in\fl(S^{1}_{L},\C)$, which are finite for every $m\geq0$ if $Z\in\fl^{\infty}(S^{1}_{L},\C)$. They dominate the three quantities occurring in Lemma \ref{lem:KSconvsmeared}, uniformly in $N$: the matrix elements \eqref{eq:latvirachi1pmommat} give $\|\tilde{\ell}^{(N)}_{\pm,k}\|_{h^{1}\rightarrow h^{0}}\leq C_{L}\langle k\rangle$ and, the off-diagonal block being supported on the $\cO(|k|)$ momenta between $0$ and $\pm k$, also $\|(\tilde{\ell}^{(N)}_{\pm,k})_{\pm\mp}\|_{2}\leq C_{L}\langle k\rangle^{\frac{3}{2}}$, both uniformly in $N$ and both equally for $\ell_{\pm,k}$. Since $\langle k\rangle\leq\langle k\rangle^{\frac{3}{2}}$, one and the same semi-norm controls both; by the triangle inequality,
}\begin{align}
\DIFadd{\label{eq:loopseminormbound}
\big\|\tilde{\ell}^{(N)}_{\pm}(Y)\big\|_{h^{1}\rightarrow h^{0}}+\big\|(\tilde{\ell}^{(N)}_{\pm}(Y))_{\pm\mp}\big\|_{2} }& \DIFadd{\leq C_{L}\!\ p_{N,\frac{3}{2}}(Y),
}\end{align}
\DIFadd{and correspondingly $\|\ell_{\pm}(Z)\|_{h^{1}\rightarrow h^{0}}+\|(\ell_{\pm}(Z))_{\pm\mp}\|_{2}\leq C_{L}\!\ p_{\infty,\frac{3}{2}}(Z)$.
}

\DIFadd{The third term compares the wavelet-smeared loop $S^{M}_{\infty}(X_{M})$ with $X$ itself and is therefore a Jackson-type approximation estimate. Its proof rests on one exact identity: by orthonormality of $s$ the aliasing mass at $\xi$ is $1-|\hat{s}(\xi)|^{2}$, which by \eqref{eq:shatflat} vanishes to order $2K$.
}

\begin{lemma}[Jackson estimate for the loop renormalization group]
\label{lem:loopjackson}
\DIFadd{Let $s$ be a compactly supported orthonormal Daubechies scaling function whose pointwise decay exponent $\rho$ of Remark \ref{rem:regularityvalues} satisfies $\rho>m+1$, let $m\geq0$, and let $\beta\in(0,1-\tfrac{m+1}{\rho})$. Then, for every $X\in\fl^{\infty}(S^{1}_{L},\C)$ with restrictions $X_{M}:=X_{|\Lambda_{M}}$ and every $q>0$,
}\begin{align}
\DIFadd{\label{eq:loopjackson}
p_{\infty,m}(S^{M}_{\infty}(X_{M})-X) }& \DIFadd{\leq C\big(\vep_{M}\!\ p_{\infty,m+1}(X)+\vep_{M}^{K\beta-m}p_{\infty,m+K\beta}(X)+\vep_{M}^{q}\!\ p_{\infty,2m+q}(X)\big),
}\end{align}
\DIFadd{with $C$ depending only on $s$, $m$, $\beta$ and $q$. In particular $p_{\infty,m}(S^{M}_{\infty}(X_{M})-X)\rightarrow0$ as $M\rightarrow\infty$ whenever $K\beta>m$, and the rate is the first-order term $\cO(\vep_{M})$ as soon as $K\beta\geq m+1$. The exponent $K\beta$ is produced by a H\"older split in the proof and is not optimal: every individual aliasing coefficient is $\cO(|\xi|^{K})$ while the sum over the replicas converges by polynomial decay alone, so the true order of \eqref{eq:aliasingmass} is $K$. The resulting conditions on $K$ are therefore sufficient but far from necessary.
}\end{lemma}
\begin{proof}
\DIFadd{By Definition \ref{def:waveletrgcur} the Fourier coefficients of $S^{M}_{\infty}(X_{M})$ are $\hat{s}(\vep_{M}k)\!\ (\vep_{M}\hat{X}_{\per|k})$, and Poisson summation identifies the sampled transform as $\vep_{M}\hat{X}_{\per|k} = \sum_{i\in\Z}\hat{X}_{k+2\pi i/\vep_{M}}$. Adding and subtracting $\hat{s}(\vep_{M}k)\hat{X}_{k}$ therefore gives
}\begin{align*}
\DIFadd{2L\!\ p_{\infty,m}(S^{M}_{\infty}(X_{M})-X) }& \DIFadd{\leq \underbrace{\sum_{k}\langle k\rangle^{m}|\hat{s}(\vep_{M}k)-1||\hat{X}_{k}|}_{=:\ \mathrm{I}}+\underbrace{\sum_{k}\langle k\rangle^{m}|\hat{s}(\vep_{M}k)|\!\!\sum_{i\neq0}\!|\hat{X}_{k+2\pi i/\vep_{M}}|}_{=:\ \mathrm{II}},
}\end{align*}
\DIFadd{the sums running over $\Gamma_{\infty,{\color{blue}+}}$.
}

\emph{\DIFadd{Term}} \DIFadd{$\mathrm{I}$ is the smearing error and is of first order: $\hat{s}$ is smooth with $\|\hat{s}'\|_{\infty}\leq\int_{\R}|x\!\ s(x)|dx<\infty$ by compact support, so $|\hat{s}(\vep_{M}k)-1|\leq\|\hat{s}'\|_{\infty}\vep_{M}|k|$ by \eqref{eq:comdef} and $\mathrm{I}\leq\|\hat{s}'\|_{\infty}\vep_{M}\!\ 2L\!\ p_{\infty,m+1}(X)$. This is the centre-of-mass offset of Remark \ref{rem:recentring} once more, and it is the only first-order contribution.
}

\emph{\DIFadd{Term}} \DIFadd{$\mathrm{II}$ is the aliasing error. Substituting $l = k+2\pi i/\vep_{M}$ and using $\langle k\rangle\leq\langle l\rangle\langle2\pi i/\vep_{M}\rangle\leq C\vep_{M}^{-1}\langle l\rangle\langle i\rangle$,
}\begin{align*}
\DIFadd{\mathrm{II} }& \DIFadd{\leq C\vep_{M}^{-m}\sum_{l}\langle l\rangle^{m}|\hat{X}_{l}|\sum_{i\neq0}\langle i\rangle^{m}|\hat{s}(\vep_{M}l-2\pi i)|.
}\end{align*}
\DIFadd{We claim that
}\begin{align}
\DIFadd{\label{eq:aliasingmass}
\sum_{i\neq0}\langle i\rangle^{m}|\hat{s}(\xi-2\pi i)| }& \DIFadd{\leq C|\xi|^{K\beta}, }& \DIFadd{|\xi| }& \DIFadd{\leq\pi.
}\end{align}
\DIFadd{Indeed, orthonormality of $\{s(\!\ \cdot\!-\!n)\}_{n\in\Z}$ is equivalent to $\sum_{i\in\Z}|\hat{s}(\xi-2\pi i)|^{2}=1$, whence
}\begin{align*}
\DIFadd{\delta(\xi) }& \DIFadd{:= \sum_{i\neq0}|\hat{s}(\xi-2\pi i)|^{2} = 1-|\hat{s}(\xi)|^{2} = \cO(\xi^{2K})
}\end{align*}
\DIFadd{by \eqref{eq:shatflat}. In particular }\emph{\DIFadd{every single}} \DIFadd{aliasing coefficient obeys $|\hat{s}(\xi-2\pi i)|\leq\delta(\xi)^{\frac{1}{2}}$ for $i\neq0$, while $|\hat{s}(\xi-2\pi i)|\leq C\langle i\rangle^{-\rho}$ for $|\xi|\leq\pi$ by \eqref{eq:decaysf}. Splitting each factor accordingly,
}\begin{align*}
\DIFadd{\sum_{i\neq0}\langle i\rangle^{m}|\hat{s}(\xi-2\pi i)| }& \DIFadd{= \sum_{i\neq0}\langle i\rangle^{m}|\hat{s}(\xi-2\pi i)|^{\beta}|\hat{s}(\xi-2\pi i)|^{1-\beta} \leq \delta(\xi)^{\frac{\beta}{2}}C^{1-\beta}\!\!\sum_{i\neq0}\!\langle i\rangle^{m-\rho(1-\beta)},
}\end{align*}
\DIFadd{and the remaining sum converges precisely because $\beta<1-\tfrac{m+1}{\rho}$. With $\delta(\xi)^{\frac{1}{2}}=\cO(|\xi|^{K})$ this is \eqref{eq:aliasingmass}. Applying \eqref{eq:aliasingmass} with $\xi = \vep_{M}l$ to the momenta with $|\vep_{M}l|\leq\pi$ bounds their contribution to $\mathrm{II}$ by $C\vep_{M}^{K\beta-m}2L\!\ p_{\infty,m+K\beta}(X)$. For $|\vep_{M}l|>\pi$ we use instead $\sum_{i}\langle i\rangle^{m}|\hat{s}(\xi-2\pi i)|\leq C\langle\xi\rangle^{m}$, which again follows from \eqref{eq:decaysf} with the term $i\approx\tfrac{\xi}{2\pi}$ dominating, together with $1\leq(\vep_{M}\langle l\rangle/\pi)^{q}$; this contributes $C\vep_{M}^{q}2L\!\ p_{\infty,2m+q}(X)$.
}\end{proof}

\begin{thm}[Reconstruction of the smeared Virasoro generators]
\label{thm:KSreconstruction}
\DIFadd{Let $s$ be a compactly supported orthonormal Daubechies scaling function which is $\tfrac{5}{2}$-regular in the sense of Definition \ref{def:regularity} and whose pointwise decay exponent $\rho$ of Remark \ref{rem:regularityvalues} satisfies $\rho>\tfrac{5}{2}$, and let $\beta\in(0,1-\tfrac{5}{2\rho})$ with $K\beta>\tfrac{3}{2}$; by the values of Remark \ref{rem:regularityvalues} this holds from $K=9$ on, and $K\beta\geq\tfrac{5}{2}$ --- which by Lemma \ref{lem:loopjackson} upgrades the third term below to the first-order rate $\cO(\vep_{M})$ --- from $K=10$ on. Let $X\in\fl^{\infty}(S^{1}_{L},\C)$ with restrictions $X_{M} = X_{|\Lambda_{M}}$, and let $\{X_{N}\}_{N\in\N_{0}}$, $X_{N}\in\fl(\Lambda_{N},\C)$, be $S$-convergent to $X$ in the sense that
}\begin{align}
\DIFadd{\label{eq:Sconvloops}
\lim_{M\rightarrow\infty}\limsup_{N\rightarrow\infty}p_{N,\frac{3}{2}}(X_{N}-S^{M}_{N}(X_{M})) }& \DIFadd{= 0.
}\end{align}
\DIFadd{Then the smeared one-particle KS approximants converge to the smeared one-particle Virasoro generators,
}\begin{align}
\DIFadd{\label{eq:KSreconstruction}
\lim_{N\rightarrow\infty}\big\|\big(\tilde{\ell}^{(N)}_{\pm}(X_{N})-\ell_{\pm}(X)\big)\xi\big\|_{\infty} }& \DIFadd{= 0, }& \DIFadd{\lim_{N\rightarrow\infty}\big\|\big(\tilde{\ell}^{(N)}_{\pm}(X_{N})-\ell_{\pm}(X)\big)_{\pm\mp}\big\|_{2} }& \DIFadd{= 0,
}\end{align}
\DIFadd{for all $\xi\in h^{1}(\Gamma_{\infty,{\color{blue}\pm}})$, and consequently, exactly as in Theorem \ref{thm:KSconvsmeared},
}\begin{align*}
\DIFadd{\lim_{N\rightarrow\infty}\big\|\big(:\!\pi_{{\color{blue}\pm}}(\alpha^{N}_{\infty}(L^{(N)}_{\pm}(X_{N}))\!)\!:-L_{\pm}(X)\big)\Phi\big\| }& \DIFadd{= 0,
}\end{align*}
\DIFadd{for all $\Phi\in\cD(d\Gamma(\langle\!\ \cdot\!\ \rangle))$. Thus every smeared Virasoro generator $L_{\pm}(X)$ with smooth $X$ is a scaling limit of Koo-Saleur approximants, and not only those of the form $L_{\pm}(S^{M}_{\infty}(X))$.
}\end{thm}
\begin{proof}
\DIFadd{Insert the splitting of Remark \ref{rem:KSconvsmeared}. Its first term is bounded, uniformly in $N$, by $C_{L}\!\ p_{N,\frac{3}{2}}(X_{N}-S^{M}_{N}(X_{M}))$ according to \eqref{eq:loopseminormbound}. Its second term tends to $0$ as $N\rightarrow\infty$ for each fixed $M$ by Lemma \ref{lem:KSconvsmeared}, with the rate \eqref{eq:smearedrate}. Its third term is bounded by $C_{L}\!\ p_{\infty,\frac{3}{2}}(S^{M}_{\infty}(X_{M})-X)$, again by \eqref{eq:loopseminormbound}, and therefore tends to $0$ as $M\rightarrow\infty$ by Lemma \ref{lem:loopjackson} applied with $m=\tfrac{3}{2}$, which is admissible because $\rho>\tfrac{5}{2}$ and $K\beta>\tfrac{3}{2}$. Taking $\limsup_{N\rightarrow\infty}$ at fixed $M$ and then letting $M\rightarrow\infty$ gives \eqref{eq:KSreconstruction}; the second-quantized statement follows from \eqref{eq:noquantconv} as in the proof of Theorem \ref{thm:KSconvsmeared}.
}\end{proof}
\begin{rem}[Two-scale rate, and the basic sequences]
\label{rem:reconstructionrate}
\DIFadd{The proof gives more than the limit: for every $M\leq N$,
}\begin{align}
\DIFadd{\label{eq:reconstructionrate}
\big\|\tilde{\ell}^{(N)}_{\pm}(X_{N})-\ell_{\pm}(X)\big\|_{h^{1+\delta}\rightarrow h^{0}} }& \DIFadd{\leq C_{L}\!\ p_{N,\frac{3}{2}}(X_{N}-S^{M}_{N}(X_{M}))+C_{X,M,\delta}\!\ \vep_{N}^{\min\{\delta,1\}}+C_{X}\!\ \vep_{M},
}\end{align}
\DIFadd{by \eqref{eq:loopseminormbound}, \eqref{eq:smearedrate} and \eqref{eq:loopjackson}. The three terms are the price of the sequence, of the generator approximation at the fine scale $N$, and of the smearing at the coarse scale $M$; only the middle one improves to $\min\{\delta,2\}$ under the re-centring of Remark \ref{rem:smearphase}; the last one is first order for the same centre-of-mass reason once $K\geq10$, and is $o(1)$ without a rate for $K=9$. The basic sequences of \mbox{%DIFAUXCMD
\cite{DuffieldMeanFieldDynamical} }\hskip0pt%DIFAUXCMD
are the special case $X_{N} = S^{M}_{N}(X_{M})$ at fixed $M$, where the first term vanishes identically for $N\geq M$ and \eqref{eq:reconstructionrate} reduces to \eqref{eq:smearedrate} plus the fixed smearing offset.
}\end{rem}
\DIFaddend In analogy with the Corollaries \ref{cor:KSconvbog} \& \ref{cor:KSconvbogc}, Lemma \ref{lem:KSconvsmeared} implies the convergence of the Bogoliubov transformation with smeared generators, see \eqref{eq:latviradyn}:
...
Let us begin with a discussion of the approximation error in Lemma \ref{lem:KSconv}. \DIFdelbegin \DIFdel{The structure of the proof, and specifically considering,
}\begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:uniformestimate}%%%
}& \DIFdel{\sum_{m\in\Gamma_{\infty,{\color{blue}\pm}}}|f^{(N)}_{k}(M,m)-f_{k}(M,m)|^{2} }\\ \DIFdel{\nonumber
}& \DIFdel{= \sum_{m\in\Gamma_{\infty,{\color{blue}\pm}}}\left|\hat{s}(\vep_{M}(m\!+\!k))(m\!+\!\tfrac{k}{2})\right|^{2}\left|\cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(m+\frac{k}{2}))}{\vep_{N}(m+\frac{k}{2})}\tfrac{\hat{s}(\vep_{N}m)}{\hat{s}(\vep_{N}(m+k))}-1\right|^{2},
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{Put
}\begin{align*}
\DIFadd{Q_{N}(m) }& \DIFadd{:= \prod_{j=1}^{N-M}m_{0}(\vep_{M+j}(m+k)), }& \DIFadd{c_{N}(m) }& \DIFadd{:= \cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(m+\frac{k}{2}))}{\vep_{N}},
}\end{align*}\DIFaddend 
\DIFdelbegin \DIFdel{suggests a simple strategy to obtain a specific error bound by estimating against the decay of $\hat{s}$:
}\begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:sobolevtrade}%%%
}& \DIFdel{\sum_{m\in\Gamma_{\infty,{\color{blue}\pm}}}\left|\hat{s}(\vep_{M}(m\!+\!k))(m\!+\!\tfrac{k}{2})\right|^{2}\left| \cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(m+\frac{k}{2}))}{\vep_{N}(m+\frac{k}{2})}\tfrac{\hat{s}(\vep_{N}m)}{\hat{s}(\vep_{N}(m+k))}-1\right|^{2} }\\ \DIFdel{\nonumber
}& \DIFdel{\leq\!\!\left(\!\tfrac{2\pi^{2}}{L}\!\!\sup_{m\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!(1\!+\!\tfrac{L}{\pi}|m\!+\!\tfrac{k}{2}|)^{-2\delta}\!\left| \cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(m+\frac{k}{2}))}{\vep_{N}(m+\frac{k}{2})}\tfrac{\hat{s}(\vep_{N}m)}{\hat{s}(\vep_{N}(m+k))}\!-\!1\right|^{2}\right)\|\hat{s}(\vep_{M}(\!\ .\!\ \!+\!\tfrac{k}{2}))\|_{h^{1+\delta}}^{2},
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{so that $f^{(N)}_{k}(M,m) = \hat{s}(\vep_{N}m)c_{N}(m)Q_{N}(m)$ and, by \eqref{eq:infprod}, $f_{k}(M,m) = \hat{s}(\vep_{N}(m+k))Q_{N}(m)(m+\tfrac{k}{2})$. This yields the }\emph{\DIFadd{exact}}\DIFadd{, division-free splitting
}\begin{align}
\DIFadd{\label{eq:uniformestimate}
f^{(N)}_{k}(M,m)-f_{k}(M,m) }& \DIFadd{= \underbrace{\hat{s}(\vep_{M}(m\!+\!k))\big[c_{N}(m)-(m\!+\!\tfrac{k}{2})\big]}_{=:\ \textup{(A)},\ \textup{discretization}}\ +\ \underbrace{Q_{N}(m)\big[\hat{s}(\vep_{N}m)-\hat{s}(\vep_{N}(m\!+\!k))\big]c_{N}(m)}_{=:\ \textup{(B)},\ \textup{centre-of-mass mismatch}} .
}\end{align}
\DIFadd{The two terms are of different order in $\vep_{N}$. For (A) one has, with $x=\vep_{N}(m+\tfrac{k}{2})$ and $|\cos(\tfrac{1}{4}\vep_{N}k)^{2}\sinc(x)-1|\leq|\sinc(x)-1|+\sin(\tfrac{1}{4}\vep_{N}k)^{2}$,
}\begin{align}
\DIFadd{\label{eq:errortermA}
|\textup{(A)}| }& \DIFadd{\leq |\hat{s}(\vep_{M}(m\!+\!k))|\!\ |m\!+\!\tfrac{k}{2}|\Big(\big|\sinc(\vep_{N}(m\!+\!\tfrac{k}{2}))-1\big|+\tfrac{(\vep_{N}k)^{2}}{16}\Big),
}\end{align}\DIFaddend 
\DIFdelbegin \DIFdel{for some $\delta>0$ and $\|\hat{s}(\vep_{M}(\!\ .\!\ +\tfrac{k}{2}))\|_{h^{1+\delta}}^{2}\!=\!\tfrac{1}{2L}\sum_{m\in\Gamma_{\infty,{\color{blue}\pm}}}(1\!+\!\tfrac{L}{\pi}|m|)^{2(1+\delta)}|\hat{s}(\vep_{M}(m\!+\!\tfrac{k}{2}))|^{2}$. The regularizing factor $(1\!+\!\tfrac{L}{\pi}|m\!+\!\tfrac{k}{2}|)^{-2\delta}$ has the effect that,
}\begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:errordecay}%%%
\textup{Error}^{2}(\delta,L, k, N) }&\DIFdel{\!:=\!\tfrac{2\pi^2}{L}\!\!\sup_{m\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!\!\textup{error}^{2}(\delta,L, k, N,m) \stackrel{N\rightarrow\infty}\longrightarrow 0 }\\ \DIFdel{\nonumber
\textup{error}^{2}(\delta,L, k, N,m) }&\DIFdel{\!:=\!(1+\tfrac{L}{\pi}|m+\tfrac{k}{2}|)^{-2\delta}\left| \cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(m+\frac{k}{2}))}{\vep_{N}(m+\frac{k}{2})}\tfrac{\hat{s}(\vep_{N}m)}{\hat{s}(\vep_{N}(m+k))}\!-\!1\right|^{2},
}\end{align*}%DIFAUXCMD
\DIFdel{with a rate determined by $\delta = \delta(L,k)$. To obtain a precise numerical bound we also need to estimate $\|\hat{s}(\vep_{M}(\!\ .\!\ +\tfrac{k}{2}))\|_{h^{1+\delta}}^{2}$, which will decrease with increasing regularity of }\DIFdelend \DIFaddbegin \DIFadd{which is $O(\vep_{N}^{2})$ and, crucially, carries the decaying factor $\hat{s}(\vep_{M}(m+k))$. For (B) we use that $\hat{s}$ is entire with $\|\hat{s}'\|_{\infty}\leq\int_{\R}|x||s(x)|dx<\infty$, }\DIFaddend $s$ \DIFaddbegin \DIFadd{being compactly supported, so that
}\begin{align}
\DIFadd{\label{eq:errortermB}
|\textup{(B)}| }& \DIFadd{\leq \|\hat{s}'\|_{\infty}\!\ \vep_{N}|k|\!\ |m+\tfrac{k}{2}|,
}\end{align}
\DIFadd{which is only $O(\vep_{N})$}\DIFaddend .

\medskip

\DIFaddbegin \noindent\emph{\DIFadd{The case $k=0$.}} \DIFadd{Here (B) vanishes identically and \eqref{eq:uniformestimate} closes completely:
}\begin{align}
\DIFadd{\label{eq:errordecay}
\sum_{m\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!\!|f^{(N)}_{0}(M,m)-f_{0}(M,m)|^{2} }& \DIFadd{= \!\!\!\sum_{m\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!\!|\hat{s}(\vep_{M}m)|^{2}m^{2}\big|\sinc(\vep_{N}m)-1\big|^{2} }\\ \DIFadd{\nonumber
}& \DIFadd{\leq \textup{Error}^{2}(\delta,L,0,N)\!\ \|\hat{s}(\vep_{M}\!\ .\!\ )\|^{2}_{h^{1+\delta}},
}\end{align}
\DIFadd{where, for $\delta\in[0,2]$,
}\begin{align}
\DIFadd{\label{eq:errordecayexp0}
\textup{Error}^{2}(\delta,L,0,N) }& \DIFadd{:= 2L\!\!\sup_{m\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!(1+\tfrac{L}{\pi}|m|)^{-2(1+\delta)}m^{2}|\sinc(\vep_{N}m)-1|^{2} }\\ \DIFadd{\nonumber
}& \DIFadd{\leq 2L(\tfrac{\pi}{L})^{2(1+\delta)}\vep_{N}^{2\delta}\sup_{x\in\R}\tfrac{|\sinc(x)-1|^{2}}{|x|^{2\delta}}\ =\ \tfrac{2L}{36}(\tfrac{\pi}{L})^{6}\vep_{N}^{4}\quad(\delta=2),
}\end{align}
\DIFadd{using $1-\tfrac{x^{2}}{6}\leq\sinc(x)\leq1$, whence $\sup_{x}|x|^{-4}|\sinc(x)-1|^{2}=\tfrac{1}{36}$. The Sobolev norm $\|\hat{s}(\vep_{M}\!\ .\!\ )\|^{2}_{h^{1+\delta}}=\tfrac{1}{2L}\sum_{m}(1+\tfrac{L}{\pi}|m|)^{2(1+\delta)}|\hat{s}(\vep_{M}m)|^{2}$ is finite precisely when $s$ is $(1+\delta)$-regular in the sense of Definition \ref{def:regularity}; by Remark \ref{rem:regularityvalues} the value $\delta=2$ requires $K\geq9$. Thus
}\begin{align}
\DIFadd{\label{eq:errorscaling}
\textup{Error}^{2}(\delta,L,0,N) }& \DIFadd{\lesssim C_{\delta,L}\!\ 2^{-2\delta N},
}\end{align}
\DIFadd{with an explicitly computable proportionality factor $C_{\delta,L}$.
}

\DIFaddend According to the proof of Theorem \ref{thm:KSconv} \DIFdelbegin \DIFdel{, we can estimate }\DIFdelend \DIFaddbegin \DIFadd{this controls }\DIFaddend the Fock-space \DIFdelbegin \DIFdel{norm in \eqref{eq:latviraconv} using \eqref{eq:uniformestimate}: }\begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:uniformestimateFock}%%%
\|(\alpha^{N}_{\infty}(L^{(N)}_{k,\pm})\!-\!L_{k,\pm})a^{\dagger}(\xi_{1})...a^{\dagger}(\xi_{n})\Omega_{0}\| }& \DIFdel{\!\!\leq\!\!\tfrac{nL}{\pi}\textup{Error}(\delta,L, k, N)\|\hat{s}(\vep_{M}(\!\ .\!\ \!\!+\!\tfrac{k}{2}))\|_{h^{1+\delta}}\!\!\prod_{p=1}^{n}\!\|\eta_{p}\|_{M},
}\end{align*}%DIFAUXCMD
\DIFdel{where $\xi_{p} = R^{M}_{\infty}(\eta_{p})$ for }\DIFdelend \DIFaddbegin \DIFadd{error: since $\|(\tilde{\ell}^{(N)}_{\pm,k}-\ell_{\pm,k})R^{M}_{\infty}(\xi)\|^{2}_{\infty}\leq\tfrac{1}{4}\|\xi\|_{\fh_{M}}^{2}\sum_{m}|f^{(N)}_{k}(M,m)-f_{k}(M,m)|^{2}$ (the sum over $l\in\Gamma_{M,{\color{blue}\pm}}$ in the proof of Lemma \ref{lem:KSconv} contains a single non-vanishing term, and $L_{M}\vep_{M}=L$), the bound \eqref{eq:secondquantboundvect} gives, for $\xi_{p}=R^{M}_{\infty}(\eta_{p})$, }\DIFaddend $p=1,\dots,n$\DIFaddbegin \DIFadd{, }\DIFaddend and $M<N$\footnote{Note that the states $a^{\dagger}(\xi_{1})\dots a^{\dagger}(\xi_{n})\Omega_{0}$ have the norm $\|a^{\dagger}(\xi_{1})\dots a^{\dagger}(\xi_{n})\Omega_{0}\| = \prod_{p=1}^{n}\|\xi_{p}\|_{\infty}$, for $\xi_{1},\dots,\xi_{n}$ orthogonal.}\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{:
}\begin{align}
\DIFadd{\label{eq:uniformestimateFock}
\|(\alpha^{N}_{\infty}(L^{(N)}_{0,\pm})\!-\!L_{0,\pm})a^{\dagger}(\xi_{1})...a^{\dagger}(\xi_{n})\Omega_{0}\| }& \DIFadd{\!\!\leq\!\!\tfrac{nL}{2\pi}\textup{Error}(\delta,L,0,N)\|\hat{s}(\vep_{M}\!\ .\!\ )\|_{h^{1+\delta}}\!\!\prod_{p=1}^{n}\!\|\eta_{p}\|_{M}.
}\end{align}
\DIFaddend 

\medskip

\DIFdelbegin \DIFdel{The supremum in \eqref{eq:errordecay} can be estimated for $\delta\in[0,2]$ as follows:
}\begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:errordecayexp}%%%
}& \DIFdel{\textup{Error}^{2}(\delta,L, k, N) }\\ \DIFdel{\nonumber
}& \DIFdel{= \tfrac{2\pi^2}{L}(\tfrac{\pi}{L_{N}})^{2\delta}\!\!\sup_{m\in\Gamma_{\infty}}\!\!(\tfrac{\pi}{L_{N}}\!+\!\vep_{N}|m+\tfrac{k}{2}|)^{-2\delta}\!\left| \cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(\vep_{N}(m+\frac{k}{2}))}{\vep_{N}(m+\frac{k}{2})}\tfrac{\hat{s}(\vep_{N}m)}{\hat{s}(\vep_{N}(m+k))}-1\right|^{2} }\\ \DIFdel{\nonumber
}& \DIFdel{\leq \tfrac{2\pi^2}{L}(\tfrac{\pi}{L_{N}})^{2\delta}\sup_{x\in\R}(\tfrac{\pi}{L_{N}}\!+\!|x+\vep_{N}\tfrac{k}{2}|)^{-2\delta}\!\left| \cos(\tfrac{1}{4}\vep_{N}k)^{2}\tfrac{\sin(x+\vep_{N}\frac{k}{2})}{x+\vep_{N}\frac{k}{2}}\tfrac{\hat{s}(x)}{\hat{s}(x+\vep_{N}k)}-1\right|^{2}.
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \noindent\emph{\DIFadd{The case $k\neq0$: the centre-of-mass mismatch.}} \DIFadd{For $k\neq0$ the term (B) is genuinely present and of first order in $\vep_{N}$. Its origin is elementary: writing $\mu(s):=\int_{\R}x\!\ s(x)dx$ for the first moment of the scaling function, one has $\hat{s}(0)=1$, $\hat{s}'(0)=-i\mu(s)$ and hence, for each fixed $m$,
}\begin{align}
\DIFadd{\label{eq:comphase}
\tfrac{\hat{s}(\vep_{N}m)}{\hat{s}(\vep_{N}(m+k))} }& \DIFadd{= 1+i\mu(s)\vep_{N}k+O(\vep_{N}^{2}) = e^{i\mu(s)\vep_{N}k}+O(\vep_{N}^{2}).
}\end{align}
\DIFadd{The leading deviation from $1$ is thus $m$-independent and is exactly the phase picked up by the $k$-th Virasoro mode under the translation $T_{a}$ by $a=\vep_{N}\mu(s)$, since $T_{a}\ell_{\pm,k}T_{a}^{*}=e^{\pm ika}\ell_{\pm,k}$. In other words, (B) records nothing but the offset between the lattice origin and the centre of mass of $s^{(\vep_{N})}$, which for the Daubechies family is far from zero: $\mu(\!\ _{K}s)=\tfrac{1}{\sqrt{2}}\sum_{n}n\!\ h_{n}$, e.g.~$\mu(\!\ _{4}s)\approx5.995$ and $\mu(\!\ _{10}s)\approx16.869$ (with $\supp(\!\ _{K}s)=[0,2K-1]$). Consequently, the re-centred comparison of Remark \ref{rem:recentring} --- i.e.~comparing $\alpha^{N}_{\infty}(L^{(N)}_{\pm,k})$ not with $L_{\pm,k}$ but with its translate $T_{a_{N}}L_{\pm,k}T_{a_{N}}^{*} = e^{\pm i\mu(s)\vep_{N}k}L_{\pm,k}$, $a_{N}=\vep_{N}\mu(s)$ --- removes (B) to leading order and restores the $O(\vep_{N}^{2})$ behaviour of the $k=0$ case. This affects only the rate, never the limit, since $e^{\pm i\mu(s)\vep_{N}k}\rightarrow1$; and, as explained in Remark \ref{rem:recentring}, it is a comparison convention which we carry along rather than build into Definition \ref{def:waveletrg}.
}

\DIFadd{We can now give the complete estimate for $k\neq0$, splitting $\Gamma_{\infty,{\color{blue}\pm}}$ into the $N$-th Brillouin zone and its complement. Throughout, $\rho=K-\cK_{j}>\tfrac{3}{2}$ and $N\geq N_{0}(k)$.
}

\emph{\DIFadd{(i) The Brillouin zone $|m+k|\leq\pi/\vep_{N}$.}} \DIFadd{Here \eqref{eq:convergencecor} applies to $Q_{N}$ directly, so \eqref{eq:errortermA} and \eqref{eq:errortermB} give, with $C_{1}$ as in \eqref{eq:unifmajP},
}\begin{align}
\DIFadd{\label{eq:errorBZ}
}& \DIFadd{\sum_{|m+k|\leq\frac{\pi}{\vep_{N}}}\!\!\!\!|f^{(N)}_{k}(M,m)-f_{k}(M,m)|^{2} }\\ \DIFadd{\nonumber
}& \DIFadd{\leq 2\|\hat{s}'\|_{\infty}^{2}C_{1}^{2}k^{2}\vep_{N}^{2}\!\!\!\sum_{m\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!\!|m\!+\!\tfrac{k}{2}|^{2}(1\!+\!\vep_{M}|m\!+\!k|)^{-2\rho} }\\ \DIFadd{\nonumber
}& \DIFadd{\hspace{1.5cm} +\tfrac{1}{2}\vep_{N}^{4}\!\!\!\sum_{|m+k|\leq\frac{\pi}{\vep_{N}}}\!\!\!\!|\hat{s}(\vep_{M}(m\!+\!k))|^{2}\big(\tfrac{1}{6}|m\!+\!\tfrac{k}{2}|^{3}+\tfrac{k^{2}}{16}|m\!+\!\tfrac{k}{2}|\big)^{2}\!\!,
}\end{align}\DIFaddend 
\DIFdelbegin \DIFdel{Thus, we find }\DIFdelend \DIFaddbegin \DIFadd{where the first sum converges precisely because $\rho>\tfrac{3}{2}$. The second sum is $O(1)$ if $s$ is $3$-regular and is $O(\vep_{N}^{2\rho-7})$ otherwise, so that the second line of \eqref{eq:errorBZ} is $O(\vep_{N}^{\min\{4,\!\ 2\rho-3\}})$. The first line is the centre-of-mass term and is $O(\vep_{N}^{2})$; it is absent }\DIFaddend for $k=0$ \DIFaddbegin \DIFadd{and, after the re-centring described below, is replaced by a term of the same order as the second.
}

\emph{\DIFadd{(ii) The tail $|m+k|>\pi/\vep_{N}$.}} \DIFadd{Here Corollary \ref{cor:unifmaj} bounds $f^{(N)}_{k}$ and Lemma \ref{lem:decay} bounds $f_{k}$ by the same expression, so with $C_{2}:=C(1+(L|k|)^{K}2^{-M\rho})+C'$,
}\begin{align}
\DIFadd{\label{eq:errortail}
\sum_{|m+k|>\frac{\pi}{\vep_{N}}}\!\!\!\!|f^{(N)}_{k}(M,m)-f_{k}(M,m)|^{2} }& \DIFadd{\leq 2C_{2}^{2}\!\!\!\sum_{|n|>\frac{\pi}{\vep_{N}}}\!\!\!(|n|+\tfrac{|k|}{2})^{2}(1+\vep_{M}|n|)^{-2\rho} }\\ \DIFadd{\nonumber
}& \DIFadd{\leq \tfrac{C_{3}}{2\rho-3}\!\ \vep_{M}^{-2\rho}\Big(\tfrac{\pi}{\vep_{N}}\Big)^{3-2\rho} = \tfrac{C_{3}\pi^{3-2\rho}}{(2\rho-3)\vep_{M}^{3}}\!\ 2^{-(2\rho-3)(N-M)},
}\end{align}
\DIFadd{$n=m+k$ running over the shifted lattice $\Gamma_{\infty,{\color{blue}\pm}}+k$ of spacing $\tfrac{\pi}{L}$, and the sum being compared with the corresponding integral (legitimate since the summand is eventually decreasing, for $\pi/\vep_{N}\geq\max\{1,|k|\}$). Note that \eqref{eq:errortail} is finite, and exponentially small in $N$, for exactly the same reason that $g_{k}(M,\!\ .\!\ )\in\ltwo$, namely $\rho>\tfrac{3}{2}$.
}

\DIFadd{Adding \eqref{eq:errorBZ} and \eqref{eq:errortail}, we obtain for all $k$ and all $N\geq N_{0}(k)$}\DIFaddend :
\DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:errordecayexp0}%%%
\textup{Error}^{2}(\delta,L, 0, N) }& \DIFdel{\leq \tfrac{2\pi^2}{L}(\tfrac{\pi}{L_{N}})^{2\delta}\sup_{x\in\R}(\tfrac{\pi}{L_{N}}\!+\!|x|)^{-2\delta}\!\left|\tfrac{\sin(x)}{x}-1\right|^{2} }\\ \DIFdel{\nonumber
 }& \DIFdel{\leq \tfrac{2\pi^2}{L}(\tfrac{\pi}{L_{N}})^{2\delta}\sup_{x\in\R}|x|^{-2\delta}\!\left|\tfrac{\sin(x)}{x}-1\right|^{2},
}\end{align*}%DIFAUXCMD
\DIFdel{and$\sup_{x\in\R}|x|^{-2\delta}\!\left|\tfrac{\sin(x)}{x}-1\right|^{2}$ is a finite constant of order unity for $\delta\in[0,2]$ since $|\tfrac{\sin(x)}{x}-1|^{2}\approx\tfrac{x^{4}}{36}+\mathcal{O}(x^{6})$. As long as $N$ is sufficiently large, we expect essentially the same behavior for $k\neq 0$ because of the uniform continuity of $\hat{s}$ in \eqref{eq:errordecayexp}. This suggests: }\begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:errorscaling}%%%
\textup{Error}^{2}(\delta,L, k, N) }& \DIFdel{\lesssim C_{\delta,L,k}\!\ 2^{-2\delta N},
}\end{align*}%DIFAUXCMD
\DIFdel{with an explicitly computable proportionality factor $C_{\delta,L,k}$. }\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:errortotal}
\sum_{m\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!\!\!|f^{(N)}_{k}(M,m)-f_{k}(M,m)|^{2} }& \DIFadd{\leq A_{1}\!\ \vep_{N}^{2}+A_{2}\!\ \vep_{N}^{\min\{4,\!\ 2\rho-3\}},
}\end{align}
\DIFadd{with $A_{1},A_{2}$ explicit and $A_{1}=0$ for $k=0$ (and after re-centring). Thus the wavelet renormalization group yields an explicit rate for every $k$, the exponent being limited by the regularity of $s$ through $\rho$.
}\DIFaddend 

\DIFaddbegin \begin{rem}[The rate at $k\neq0$ is not an artefact of the estimate]
\label{rem:errorrates}
\DIFadd{Numerically, $\sum_{m}|f^{(N)}_{k}(M,m)-f_{k}(M,m)|^{2}$ decays like $2^{-2N}$ for $k\neq0$ and like $2^{-4N}$ for $k=0$, and the re-centred comparison of Remark \ref{rem:recentring} restores the rate $2^{-4N}$ for $k\neq0$ while reducing the constant by four to five orders of magnitude, so that neither the order $\vep_{N}^{2}$ of the first term in \eqref{eq:errortotal} nor its removal by re-centring is an artefact of the estimate. Separating the two regions of \eqref{eq:errorBZ} and \eqref{eq:errortail} numerically confirms the structure of \eqref{eq:errortotal}: the tail decays at the rate $2\sigma_{K}-2$ (observed $1.55$, $3.83$, $4.81$ for $_{4}s$, $_{8}s$, $_{10}s$, against $2\sigma_{K}-2 = 1.55$, $3.83$, $4.81$), while the Brillouin-zone part decays at the rate $\min\{4,2\sigma_{K}-2\}$ for $k=0$ and at the rate $2$ for $k\neq0$. In particular the exponent $\min\{4,2\rho-3\}$ in \eqref{eq:errortotal}, which is what the pointwise estimate \eqref{eq:unifmaj} yields, is off by one from the sharp exponent $\min\{4,2\sigma_{K}-2\}$; this is the usual loss incurred by using the pointwise decay \eqref{eq:decaysf} of $\hat{s}$ in place of its $L^{2}$-averaged decay. For $_{4}s$ one therefore observes only $2^{-1.55N}$ and for $_{8}s$ $2^{-3.8N}$, the predicted $2^{-4N}$ being attained from $_{10}s$ on, in agreement with Remark \ref{rem:regularityvalues}.
}\end{rem}

\begin{rem}[Why the sup must be taken with care]
\label{rem:noquotient}
\DIFadd{It is tempting to factor $|\hat{s}(\vep_{M}(m+k))(m+\tfrac{k}{2})|^{2}$ out of $\sum_{m}|f^{(N)}_{k}-f_{k}|^{2}$ and to estimate the remaining supremum of $|\cos(\tfrac{1}{4}\vep_{N}k)^{2}\sinc(\vep_{N}(m+\tfrac{k}{2}))\hat{s}(\vep_{N}m)/\hat{s}(\vep_{N}(m+k))-1|$ against a Sobolev norm of $\hat{s}$, in analogy with \eqref{eq:errordecay}. For $k\neq0$ this is not available: as noted in the proof of Lemma \ref{lem:KSconv}, $\hat{s}$ vanishes on $2\pi\Z\setminus\{0\}$, so in the Ramond sector the quotient has poles at the admissible momenta $m+k\in\tfrac{2\pi}{\vep_{N}}\Z\setminus\{0\}$ and the corresponding supremum is $+\infty$ for every $N$; relaxing $\sup_{m\in\Gamma_{\infty,{\color{blue}\pm}}}$ to $\sup_{x\in\R}$ makes it $+\infty$ in both sectors. This is why we have based the discussion on the division-free splitting \eqref{eq:uniformestimate}. The tail $|m+k|>\pi/\vep_{N}$, where \eqref{eq:convergencecor} is unavailable, is handled by the uniform majorant of Lemma \ref{lem:unifmaj}, see \eqref{eq:errortail}. An unconditional and completely explicit error estimate, valid without any regularity assumption on $s$, is available in the momentum-cutoff setting, see Remark \ref{rem:timeerror} below.
}\end{rem}


\DIFaddend \begin{rem}[CFT simulation]
...
\DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:momrgtimeerrorscaling}%%%
\|((\tilde{\ell}^{(N)}_{\pm,0}\!-\!\ell_{\pm,0})^{(t)}_{\pm\pm}R^{M}_{\infty}(\eta_{p})\|_{\infty}^{2} }& \DIFdel{\!\leq\!\vep_{N}^{2\delta}\underbrace{(\sup_{x\in\R}\tfrac{|\sinc(x)\!-\!1|^{2}}{|x|^{2\delta}})}_{=\frac{1}{36}\textup{ for }\delta=2}\|\eta\|_{M}^{2}(\tfrac{L}{\pi})^{2}\!\!\!\!\sum_{n\in\Gamma_{M,{\color{blue}\pm}}}\!\!\!\!\theta(\pm n)|n|^{2(1+\delta)},
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:momrgtimeerrorscaling}
\|((\tilde{\ell}^{(N)}_{\pm,0}\!-\!\ell_{\pm,0})^{(t)}_{\pm\pm}R^{M}_{\infty}(\eta_{p})\|_{\infty}^{2} }& \DIFadd{\!\leq\!\vep_{N}^{2\delta}\underbrace{(\sup_{x\in\R}\tfrac{|\sinc(x)\!-\!1|^{2}}{|x|^{2\delta}})}_{=\frac{1}{36}\textup{ for }\delta=2}\|\eta\|_{M}^{2}\!\!\!\!\sum_{n\in\Gamma_{M,{\color{blue}\pm}}}\!\!\!\!\theta(\pm n)|n|^{2(1+\delta)},
}\end{align}\DIFaddend 
...
\DIFdelbegin \begin{align*}
\DIFdel{\|((\tilde{r}^{(N)}_{\pm,k})^{j}-r_{\pm,k}^{j})e_{m}\|_{\infty} }& \DIFdel{\leq 4L(\tfrac{L}{2\pi})^{j}\binom{2j}{j}^{\frac{1}{2}}|k|^{j}\tfrac{\Gamma(|\frac{m}{k}|+\frac{1}{2}+j)}{\Gamma(|\frac{m}{k}|+\frac{1}{2})}.
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align*}
\DIFadd{\|((\tilde{r}^{(N)}_{\pm,k})^{j}-r_{\pm,k}^{j})e_{m}\|_{\infty} }& \DIFadd{\leq 4L(\tfrac{1}{2})^{j}\binom{2j}{j}^{\frac{1}{2}}|k|^{j}\tfrac{\Gamma(|\frac{m}{k}|+\frac{1}{2}+j)}{\Gamma(|\frac{m}{k}|+\frac{1}{2})}.
}\end{align*}\DIFaddend 
This immediately implies (for sufficiently small $t>0$ if $k\neq 0$):
\DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:conformalerror}%%%
\|(e^{it\tilde{r}^{(N)}_{\pm,k}}-e^{itr_{\pm,k}})e_{m}\|_{\infty} }& \DIFdel{\leq t \|(\tilde{r}^{(N)}_{\pm,k}-r_{\pm,k})e_{m}\|_{\infty} + 4L\frac{(\tfrac{L}{\pi}(|m|+\frac{1}{2}|k|)t)^{2}}{1-\tfrac{L}{\pi}(|m|+\frac{1}{2}|k|)t} ,
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:conformalerror}
\|(e^{it\tilde{r}^{(N)}_{\pm,k}}-e^{itr_{\pm,k}})e_{m}\|_{\infty} }& \DIFadd{\leq t \|(\tilde{r}^{(N)}_{\pm,k}-r_{\pm,k})e_{m}\|_{\infty} + 4L\frac{(2(|m|+\frac{1}{2}|k|)t)^{2}}{1-2(|m|+\frac{1}{2}|k|)t} ,
}\end{align}\DIFaddend 
because \DIFdelbegin \DIFdel{$\binom{2j}{j}^{\frac{1}{2}}|k|^{j}\tfrac{\Gamma(|\frac{m}{k}|+\frac{1}{2}+j)}{\Gamma(|\frac{m}{k}|+\frac{1}{2})} \leq (2(|m|+\tfrac{1}{2}|k|))^{j}j!$}\DIFdelend \DIFaddbegin \DIFadd{$\binom{2j}{j}^{\frac{1}{2}}|k|^{j}\tfrac{\Gamma(|\frac{m}{k}|+\frac{1}{2}+j)}{\Gamma(|\frac{m}{k}|+\frac{1}{2})} \leq (4(|m|+\tfrac{1}{2}|k|))^{j}j!$}\DIFaddend . An analogous bound holds for the approximation of the unitary group associated with $\iota_{\pm,k}$.

\DIFaddbegin \DIFadd{This last step is lossy, and it is worth separating the two mechanisms. The factor $\binom{2j}{j}^{\frac{1}{2}}\leq2^{j}$ is sharp up to $(\pi j)^{-\frac{1}{4}}$ and costs nothing, being exactly compensated by the $(\tfrac{1}{2})^{j}$ in \eqref{eq:analyticvector}. What is lossy is the replacement of
}\begin{align*}
\DIFadd{|k|^{j}\tfrac{\Gamma(a+j)}{\Gamma(a)} }& \DIFadd{= \prod_{i=0}^{j-1}\big(|m|+\tfrac{1}{2}|k|+i|k|\big), }& \DIFadd{a }& \DIFadd{= |\tfrac{m}{k}|+\tfrac{1}{2},
}\end{align*}
\DIFadd{by $(2(|m|+\tfrac{1}{2}|k|))^{j}j!$, which is crude precisely in the regime $|k|\ll|m|$ relevant for the conformal limit, where the product stays close to $(|m|+\tfrac{1}{2}|k|)^{j}$. Retaining the Gamma-quotient and summing the binomial series as in \eqref{eq:analyticradius} instead of estimating it termwise gives
}\begin{align}
\DIFadd{\label{eq:conformalerrorsharp}
\|(e^{it\tilde{r}^{(N)}_{\pm,k}}-e^{itr_{\pm,k}})e_{m}\|_{\infty} }& \DIFadd{\leq t\|(\tilde{r}^{(N)}_{\pm,k}-r_{\pm,k})e_{m}\|_{\infty}+4L\big[(1-|k|t)^{-a}-1-a|k|t\big],
}\end{align}
\DIFadd{valid on the whole interval $t<\tfrac{1}{|k|}$ of \eqref{eq:analyticradiusvalue}, whose length does not depend on $m$, whereas \eqref{eq:conformalerror} is confined to $t<\tfrac{1}{2|m|+|k|}$ and thus degenerates for high momenta. For small $t$ the second term of \eqref{eq:conformalerrorsharp} is $2L\!\ a(a+1)(|k|t)^{2}+\cO(t^{3})\approx2L(|m|t)^{2}$, smaller than the $16L(|m|t)^{2}$ of \eqref{eq:conformalerror} by a factor of $8$.
}
...
\end{align*}
of the single-component fermion\DIFaddbegin \DIFadd{, }\DIFaddend which is related to the Hamiltonian density of the two-component fermion by the half-shift $x\mapsto x+\vep_{N+1}$:
\begin{align}
\label{eq:hdstag1c}
h^{(N)}_{x} & = \tilde{h}^{(N)}_{x} + \tilde{h}^{(N)}_{x+\vep_{N+1}}, & x & \in\Lambda_{N}.
\end{align}
\DIFaddbegin \DIFadd{This is more than a caveat: \eqref{eq:hdstag1c} determines the discrepancy exactly. Splitting the sum \eqref{eq:hdfourier} over $\Lambda_{N}$ according to \eqref{eq:hdstag1c}, using $\Lambda_{N}\!\ \dot\cup\!\ (\Lambda_{N}+\vep_{N+1}) = \Lambda_{N+1}$ and $\vep_{N}=2\vep_{N+1}$, and writing the resulting two-valued weight as
}\begin{align*}
\DIFadd{\begin{cases} 1, & y\in\Lambda_{N} \\ e^{-ik\vep_{N+1}}, & y\in\Lambda_{N+1}\!\setminus\!\Lambda_{N}\end{cases} }& \DIFadd{= e^{-\frac{i}{4}\vep_{N}k}\big(\cos(\tfrac{1}{4}\vep_{N}k)+i\sin(\tfrac{1}{4}\vep_{N}k)\!\ e^{i\pi y/\vep_{N+1}}\big),
}\end{align*}
\DIFadd{where $e^{i\pi y/\vep_{N+1}} = \pm1$ is the staggering sign on $\Lambda_{N+1}$, we obtain
}\begin{align}
\DIFadd{\label{eq:halfshiftmodes}
H^{(N)}_{k} }& \DIFadd{= 2\!\ e^{-\frac{i}{4}\vep_{N}k}\Big(\cos(\tfrac{1}{4}\vep_{N}k)\!\ \tilde{H}^{(N)}_{k}+i\sin(\tfrac{1}{4}\vep_{N}k)\!\ \tilde{H}^{(N)}_{k+\pi/\vep_{N+1}}\Big),
}\end{align}
\DIFadd{with $\tilde{H}^{(N)}_{q} := \tfrac{L}{\pi}\vep_{N+1}\sum_{y\in\Lambda_{N+1}}e^{iqy}\tilde{h}^{(N)}_{y}$ the modes of $\tilde{h}^{(N)}$ on the doubled lattice. Thus the two-component mode at $k$ is a combination of the single-component mode at $k$ }\emph{\DIFadd{and}} \DIFadd{of the umklapp mode at the zone-boundary momentum $k+\pi/\vep_{N+1}$, with mixing amplitude $\sin(\tfrac{1}{4}\vep_{N}k) = \cO(\vep_{N}k)$. This is the precise sense in which \eqref{eq:latvirastag} and \eqref{eq:latvirastag1c} are not directly comparable: they agree to leading order in $\vep_{N}$, and differ at first order by an umklapp contribution. It also accounts for the prefactor of \eqref{eq:latvirastag}: the phase $e^{\mp\frac{i}{4}\vep_{N}k}$ there is exactly the one appearing in \eqref{eq:halfshiftmodes}, so it originates in the half-shift and not in the Koo-Saleur combination \eqref{eq:latviracpt} --- note that the latter contributes no further phase, since $\tilde{H}^{(N)}_{0}$ enters \eqref{eq:halfshiftmodes} with $\cos0=1$. The same trigonometric factor $\cos(\tfrac{1}{4}\vep_{N}k)$ pervades the chiral symbols \eqref{eq:latvirasubchi}, as one expects given that both it and \eqref{eq:halfshiftmodes} arise from the sublattice structure; we do not track the precise power through the chiral projection here, as \eqref{eq:latviraDchi} computes it directly.
}\DIFaddend 
...
and,
\DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:U1curadjoint}%%%
j^{(N)}_{k}{}^{*} }& \DIFdel{= j^{(N)}_{-k}, }& \DIFdel{j_{k}^{*} }& \DIFdel{= j_{k},
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:U1curadjoint}
j^{(N)}_{k}{}^{*} }& \DIFadd{= j^{(N)}_{-k}, }& \DIFadd{j_{k}^{*} }& \DIFadd{= j_{-k},
}\end{align}\DIFaddend 
...
Comparing the matrix elements \eqref{eq:U1cur1pmatrix} with those of the one-particle operators of the Virasoro generators and the KS approximants \eqref{eq:latvirachi1pmommat}, we \DIFdelbegin \DIFdel{deduce that all }\DIFdelend \DIFaddbegin \DIFadd{see that the }\DIFaddend convergence results of Section \ref{sec:KSconv} \DIFdelbegin \DIFdel{equally }\DIFdelend apply to the approximation of the $U(1)$-current by \eqref{eq:U1curfinite} in the scaling limit. \DIFdelbegin \DIFdel{Notably, some of the proof of the results for the $U(1)$-current simplify slightly because the }\DIFdelend \DIFaddbegin \DIFadd{In fact the situation is considerably better than for the Virasoro generators, because $j_{k}$ is a }\emph{\DIFadd{unitary}} \DIFadd{momentum translation and no discretisation of a derivative is involved. Two points nevertheless require care, and we treat them first.
}

\emph{\DIFadd{The modification of Section \ref{sec:KSmod} is needed here as well.}} \DIFadd{As it stands, $j^{(N)}_{k}$ of \eqref{eq:U1cur1p} is the }\emph{\DIFadd{cyclic}} \DIFadd{shift on $\Gamma_{N,{\color{blue}-}}$: for the $|k|\tfrac{L}{\pi}$ momenta $n\in\Gamma_{N,{\color{blue}-}}$ with $n+k\notin\Gamma_{N,{\color{blue}-}}$ it produces $n+k-\tfrac{2\pi}{\vep_{N}}$ instead of $n+k$. For $k>0$ these are the momenta at the upper edge of the Brillouin zone, and the wrap-around sends them to the }\emph{\DIFadd{lower}} \DIFadd{edge --- that is, across the Fermi point $n=0$. Consequently $\tilde{j}^{(N)}_{k}$ acquires off-diagonal matrix elements of modulus one relative to the Hardy decomposition, and
}\begin{align}
\DIFadd{\label{eq:U1curwrap}
\big\|(\tilde{j}^{(N)}_{k}-j_{k})_{{\color{blue}-}{\color{blue}+}}\big\|_{2} }& \DIFadd{= \big(|k|\tfrac{L}{\pi}\big)^{\frac{1}{2}} \qquad\textup{for all }N,
}\end{align}
\DIFadd{which does }\emph{\DIFadd{not}} \DIFadd{tend to $0$. The third hypothesis of Lemma \ref{lem:KSconvsmeared}, and with it the estimate \eqref{eq:noquantconv} in the form used in Theorem \ref{thm:KSconvsmeared}, is therefore unavailable for the unmodified currents. We accordingly replace $j^{(N)}_{k}$ throughout by
}\begin{align}
\DIFadd{\label{eq:U1curmod}
\overline{j}^{(N)}_{k} }& \DIFadd{:= \tfrac{1}{2L_{N}}\sum_{n\in\Gamma_{N,{\color{blue}-}}}\chi_{\Gamma_{N}}(n+k)\!\ e_{n+k}\otimes\overline{e}_{n},
}\end{align}
\DIFadd{exactly as in \eqref{eq:latvirasubchi}; $\overline{j}^{(N)}_{k}$ is a partial isometry, it still satisfies $\overline{j}^{(N)}_{k}{}^{*} = \overline{j}^{(N)}_{-k}$ and the implementability condition \eqref{eq:sdcconstraints}, and it agrees with $j^{(N)}_{k}$ on momenta of order $\vep_{M}^{-1}$ once $N\gg M$, so that none of the asymptotic statements is affected.
}

\emph{\DIFadd{What is gained.}} \DIFadd{With the modification, the }\DIFaddend one-particle \DIFdelbegin \DIFdel{operators $j_{k}$, $k\in\Gamma_{\infty,{\color{blue}+}}$, are bounded. As an illustration we state the analogue of Theorem \ref{thm:KSconvsmeared}:
}\DIFdelend \DIFaddbegin \DIFadd{analysis becomes exact rather than asymptotic.
}

\begin{lemma}[Convergence of the one-particle currents]
\label{lem:U1cur1pconv}
\DIFadd{Let $R^{N}_{\infty}$ be the momentum-cutoff renormalization group and $\tilde{\overline{j}}^{(N)}_{k}:=R^{N}_{\infty}\overline{j}^{(N)}_{k}R^{N\!\ *}_{\infty}$. Then, for every $k\in\Gamma_{\infty,{\color{blue}+}}$ and every $N$ with $\tfrac{\pi}{\vep_{N}}>2|k|$:
}\begin{enumerate}
\item[\DIFadd{(i)}] \DIFadd{$\tilde{\overline{j}}^{(N)}_{k}$ and $j_{k}$ }\emph{\DIFadd{agree exactly}} \DIFadd{on the span of $\{e_{n}\!\ :\!\ n,n+k\in\Gamma_{N,{\color{blue}-}}\}$, and the difference is supported on momenta of modulus $\geq\tfrac{\pi}{\vep_{N}}-|k|$;
}\item[\DIFadd{(ii)}] \DIFadd{the off-diagonal blocks vanish identically, $(\tilde{\overline{j}}^{(N)}_{k}-j_{k})_{\pm\mp} = 0$, so that there is no Schwinger-cocycle error at any finite scale;
}\item[\DIFadd{(iii)}] \DIFadd{$\|\tilde{\overline{j}}^{(N)}_{k}-j_{k}\| = 1$ for all $N$, while for every $\delta>0$
}\begin{align}
\DIFadd{\label{eq:U1curate}
\big\|\tilde{\overline{j}}^{(N)}_{k}-j_{k}\big\|_{h^{\delta}\rightarrow h^{0}} }& \DIFadd{\asymp \vep_{N}^{\delta};
}\end{align}
\item[\DIFadd{(iv)}] \DIFadd{$\lim_{N\rightarrow\infty}\|(\tilde{\overline{j}}^{(N)}_{k}-j_{k})\xi\|_{\infty} = 0$ for }\emph{\DIFadd{every}} \DIFadd{$\xi\in\fh^{(+)}_{\infty,{\color{blue}-}}$.
}\end{enumerate}
\DIFadd{No regularity of the scaling function is assumed.
}\end{lemma}
\begin{proof}
\DIFadd{Since $R^{N}_{\infty}$ is the sharp cutoff, $\tilde{\overline{j}}^{(N)}_{k}$ restricts to $\Gamma_{N,{\color{blue}-}}$, shifts by $k$, deletes what leaves the Brillouin zone, and embeds; on the set in (i) this is precisely $e_{n}\mapsto e_{n+k}$, i.e.~$j_{k}$, whence (i). On the remaining momenta --- those with $n\notin\Gamma_{N,{\color{blue}-}}$, and the $|k|\tfrac{L}{\pi}$ edge momenta --- the difference equals $-j_{k}$, and there $n$ and $n+k$ have the same sign because $|n|\geq\tfrac{\pi}{\vep_{N}}-|k|>|k|$; hence the difference is block-diagonal relative to the Hardy decomposition and (ii) follows. For (iii), the difference is a partial isometry composed with a projection, so its norm is $1$; being supported on columns with $\langle n\rangle\geq\tfrac{L}{\pi}(\tfrac{\pi}{\vep_{N}}-|k|)$, its norm from $h^{\delta}$ to $h^{0}$ is at most $(\tfrac{L}{\pi}(\tfrac{\pi}{\vep_{N}}-|k|))^{-\delta}\leq C_{k}\vep_{N}^{\delta}$, and the matching lower bound follows from $\xi=e_{n}$ with $|n|$ just above $\tfrac{\pi}{\vep_{N}}$. Finally (iv) states that $\sum_{|n|\geq\frac{\pi}{\vep_{N}}-|k|}|\hat{\xi}_{n}|^{2}\rightarrow0$, which holds for every $\hat{\xi}\in\ltwo$.
}\end{proof}
\DIFadd{Note the contrast with Lemma \ref{lem:KSconvc}: the exponent in \eqref{eq:U1curate} is $\delta$ }\emph{\DIFadd{uncapped}}\DIFadd{, whereas for the KS approximants it saturates at $\min\{\delta,2\}$. The reason is (i): for the currents there is no bulk error whatsoever, the sole source of error being the truncation to the Brillouin zone, whereas the KS approximants additionally replace $n\mp\tfrac{k}{2}$ by $\vep_{N}^{-1}\sin(\vep_{N}(n\mp\tfrac{k}{2}))$.
}

\DIFaddend \begin{thm}[Convergence of smeared lattice currents for $\pi_{{\color{blue}-}}$]
\label{thm:U1cursmearedconv}
Let \DIFdelbegin \DIFdel{$s\in C^{\alpha}(\R)$ be a sufficiently regular, }\DIFdelend \DIFaddbegin \DIFadd{$s$ be a }\DIFaddend compactly supported orthonormal Daubechies scaling function \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{--- }\emph{\DIFadd{no regularity is required}} \DIFadd{--- }\DIFaddend and let $X\in\fl(\Lambda_{M},\C)$ for some $M\in\N_{0}$. Then \DIFdelbegin \DIFdel{, }\DIFdelend the smeared lattice currents \DIFdelbegin \DIFdel{, }\DIFdelend $J^{(N)}(S^{M}_{N}(X))$, \DIFaddbegin \DIFadd{built from the modified approximants \eqref{eq:U1curmod}, }\DIFaddend converge strongly to the $U(1)$-current \DIFdelbegin \DIFdel{, }\DIFdelend $J_{{\color{blue}-}}(S^{M}_{\infty}(X))$\DIFdelbegin \DIFdel{, on the dense domain $\fF^{\alg}_{\ua}(\cD_{\std})\subset\fF_{\ua}(\fh^{(\pm)}_{\infty,{\color{blue}\pm}})$ spanned by anti-symmetric Fock vectors with finitely many one-particle excitations in $\cD_{\std}$ (finite $\cD_{\std}$-particle number) }\DIFdelend :
\DIFdelbegin \begin{align*}
\DIFdel{%DIFDELCMD < \label{eq:U1curconvsmeared}%%%
\lim_{N\rightarrow\infty}\|(:\!\pi_{{\color{blue}-}}(\alpha^{N}_{\infty}(J^{(N)}(S^{M}_{N}(X)))\!)\!: - J_{{\color{blue}-}}(S^{M}_{\infty}(X)))a^{\dagger}(\xi_{1})\dots a^{\dagger}(\xi_{n})\Omega_{0}\| }& \DIFdel{= 0,
}\end{align*}%DIFAUXCMD
\DIFdel{for all $n\in\N_{0}$ and $\xi_{1},\dots,\xi_{n}\in\cD_{\std}$. }\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{\label{eq:U1curconvsmeared}
\lim_{N\rightarrow\infty}\|(:\!\pi_{{\color{blue}-}}(\alpha^{N}_{\infty}(J^{(N)}(S^{M}_{N}(X)))\!)\!: - J_{{\color{blue}-}}(S^{M}_{\infty}(X)))\Phi\| }& \DIFadd{= 0
}\end{align}
\DIFadd{for every $\Phi\in\cD(\Nop)$, $\Nop$ the number operator --- in particular on all of $\fF^{\alg}_{\ua}(\cD_{\std})$ --- and with the rate
}\begin{align}
\DIFadd{\label{eq:U1curconvrate}
\big\|(:\!\pi_{{\color{blue}-}}(\alpha^{N}_{\infty}(J^{(N)}(S^{M}_{N}(X)))\!)\!: - J_{{\color{blue}-}}(S^{M}_{\infty}(X)))\Phi\big\| }& \DIFadd{\leq C_{X,\delta}\!\ \vep_{N}^{\delta}\big\|d\Gamma(\langle\!\ .\!\ \rangle^{\delta})\Phi\big\|
}\end{align}
\DIFadd{for every $\delta>0$ for which $s$ is $(\delta+\tfrac{1}{2})$-regular in the sense of Definition \ref{def:regularity}.
}\DIFaddend \end{thm}
\begin{proof}
\DIFdelbegin \DIFdel{The proof is identical to that Theorem \ref{thm:KSconvsmeared}using the }\DIFdelend \DIFaddbegin \DIFadd{By Lemma \ref{lem:U1cur1pconv}\!\ (ii) the two pair terms of \eqref{eq:secondquantarakino} are }\emph{\DIFadd{absent}} \DIFadd{from $dF_{{\color{blue}-}}(\tilde{\overline{j}}^{(N)}(S^{M}_{N}(X))-j(S^{M}_{\infty}(X)))$ as soon as $\tfrac{\pi}{\vep_{N}}>2|k|$ for every $k$ in the support of $\hat{X}_{\per}$ within $\Gamma_{N,{\color{blue}+}}$, so that only the two diagonal terms of \eqref{eq:noquantconv} remain and no Hilbert-Schmidt estimate is required. These are bounded slot by slot by $\|(\tilde{\overline{j}}^{(N)}(S^{M}_{N}(X))-j(S^{M}_{\infty}(X)))\xi_{p}\|_{\infty}$, which tends to $0$ for every $\xi_{p}\in\fh^{(+)}_{\infty,{\color{blue}-}}$ by Lemma \ref{lem:U1cur1pconv}\!\ (iv), after splitting off the smearing error exactly as in the proof of Lemma \ref{lem:KSconvsmeared} --- which here needs no Sobolev weight at all, since $\|j_{k}\|=1$ makes $\tfrac{1}{2L}\sum_{k}|\hat{X}_{k}|$ a sufficient majorant. Together with the uniform bound $\|dF_{{\color{blue}-}}(\tilde{\overline{j}}^{(N)}(S^{M}_{N}(X))-j(S^{M}_{\infty}(X)))P_{\leq n}\|\leq \tfrac{n}{L}\sum_{k}|\hat{X}_{k}|$, which follows from \eqref{eq:secondquantbound}, a density argument gives \eqref{eq:U1curconvsmeared} on all of $\cD(\Nop)$. For \eqref{eq:U1curconvrate} one inserts \eqref{eq:U1curate} instead and sums over $k$ by means of Corollary \ref{cor:convergence}, which converges under $(\delta+\tfrac{1}{2})$-regularity as in \eqref{eq:relbound1psmeared}.
}\end{proof}
\begin{rem}[Currents versus Virasoro generators]
\label{rem:U1curvsvira}
\DIFadd{It is worth collecting how much simpler the current is. }\emph{\DIFadd{(a)}} \DIFadd{No regularity of $s$ is needed for \eqref{eq:U1curconvsmeared}, in contrast with Lemma \ref{lem:KSconv} and Theorem \ref{thm:KSconvsmeared}. }\emph{\DIFadd{(b)}} \DIFadd{The convergence holds on $\cD(\Nop)$ rather than on a core of finite-particle vectors, and the relevant bound involves the number operator instead of the conformal Hamiltonian --- no }\DIFaddend analogue of Lemma \DIFdelbegin \DIFdel{\ref{lem:KSconvsmeared} with the additional simplification that we have uniform bounds:
}\begin{align*}
\DIFdel{\|\tilde{j}^{(N)}_{k}R^{K}_{\infty}(e_{l})\|_{\infty} }& \DIFdel{= 1, }& \DIFdel{\|j_{k}R^{K}_{\infty}(e_{l})\|_{\infty} }& \DIFdel{= 1.
}\end{align*}%DIFAUXCMD
%DIFDELCMD < \end{proof}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{\ref{lem:energybound1k} is required, $j_{k}$ being bounded. }\emph{\DIFadd{(c)}} \DIFadd{The Schwinger cocycle contributes no error at any finite scale, by Lemma \ref{lem:U1cur1pconv}\!\ (ii); the central charge $c=1$ of \eqref{eq:U1curcom} is reproduced exactly for $\tfrac{\pi}{\vep_{N}}>2|k|$, not merely in the limit. }\emph{\DIFadd{(d)}} \DIFadd{The rate is $\vep_{N}^{\delta}$ with no cap, the only error being the Brillouin-zone truncation. Numerically (with $L=\pi$ and $k=3$) the measured rates of $\|\tilde{\overline{j}}^{(N)}_{k}-j_{k}\|_{h^{\delta}\rightarrow h^{0}}$ are $0.00, 0.51, 1.02, 2.03, 3.05$ at $\delta = 0,\tfrac{1}{2},1,2,3$, the value at $\delta=0$ being exactly $1$; and the off-diagonal Hilbert-Schmidt norms are exactly $0$, against $(|k|\tfrac{L}{\pi})^{\frac{1}{2}}$ for the unmodified currents, cf.~\eqref{eq:U1curwrap}.
}\end{rem}
\DIFaddend 
...
\DIFaddbegin \DIFadd{We can now make Theorem \ref{thm:corapprox} quantitative for the case relevant to a simulation, namely the chiral time evolution. Two features of the momentum-cutoff renormalization group make this possible without any intermediate scale: the one-particle generator $\ell^{(N)}_{\pm,0}$ is a Fourier multiplier, so the lattice dynamics does not move momenta at all, and $\alpha^{M}_{N}$ maps into the span of $\{e_{l}\}_{l\in\Gamma_{M,{\color{blue}\pm}}}$. The renormalized observable therefore stays supported on momenta $|l|\leq\tfrac{\pi}{\vep_{M}}$ for all times, and only the deviation of the vacuum symbols }\emph{\DIFadd{on those momenta}} \DIFadd{enters.
}

\begin{prop}[Explicit error bound for dynamical correlation functions]
\label{prop:corapproxrate}
\DIFadd{Let $R^{N}_{\infty}$ be the momentum-cutoff renormalization group, let $M\in\N_{0}$, and let $A,B$ be monomials of degrees $d_{A}$, $d_{B}$ in the standard generators $a^{(j)}_{x}$, $x\in\Lambda_{M}$, of \eqref{eq:rsfermi}. Let $\sigma^{(N)}_{t}$ and $\sigma_{t}$ be generated by the chiral conformal Hamiltonian, i.e.~by $L^{(N)}_{\pm,0}$ and $L_{\pm,0}$. Then, for all $N>M$ and all $t\in\R$,
}\begin{align}
\DIFadd{\label{eq:corapproxrate}
\big|C^{(N)}_{t}(\alpha^{M}_{N}(A),\alpha^{M}_{N}(B))-C_{t}(\alpha^{M}_{\infty}(A),\alpha^{M}_{\infty}(B))\big| }& \DIFadd{\leq \underbrace{(d_{A}\!+\!d_{B})\!\ \eta_{M}(N)}_{\textup{ground state}} \; + \; \underbrace{\tfrac{1}{6}d_{B}\!\ \Theta_{M}\!\ |t|\!\ \vep_{N}^{2}}_{\textup{dynamics}},
}\end{align}
\DIFadd{where $\Theta_{M}:=\big(\sum_{n\in\Gamma_{M,{\color{blue}\pm}}}\theta(\pm n)|n|^{6}\big)^{\frac{1}{2}}$ and
}\begin{align}
\DIFadd{\label{eq:vacerrororders}
\eta_{M}(N) }& \DIFadd{:= \!\!\!\max_{l\in\Gamma_{M,{\color{blue}\pm}}}\!\!\big\|S^{(N)}_{0}(l)-S(l)\big\| = \begin{cases} \big|\!\sin(\tfrac{\pi}{4}2^{-(N-M)})\big| \leq \tfrac{\pi}{4}2^{-(N-M)} & \textup{on }\fA_{M,{\color{blue}\pm}}, \\[0.1cm] \sin(\tfrac{\pi}{4}2^{-(N-M)})^{2} \leq \tfrac{\pi^{2}}{16}2^{-2(N-M)} & \textup{on }\fA^{(\pm)}_{M,{\color{blue}\pm}}. \end{cases}
}\end{align}
\end{prop}
\begin{proof}
\DIFadd{Following the proof of Theorem \ref{thm:corapprox} we split
}\begin{align*}
\DIFadd{\big|C^{(N)}_{t}-C_{t}\big| }& \DIFadd{\leq \big|(\omega^{(N)}_{0,{\color{blue}\pm}}-\omega^{(N)}_{{\color{blue}\pm}})(O_{N})\big| + \|A\|\!\ \big\|(\tilde{\sigma}^{(N)}_{t}-\sigma_{t})(\pi_{{\color{blue}\pm}}(\alpha^{M}_{\infty}(B)))\big\|,
}\end{align*}
\DIFadd{with $O_{N}=\alpha^{M}_{N}(A)\Ad_{U_{N}(t)}(\alpha^{M}_{N}(B))$ and $\|A\|\leq1$, the generators $a^{(j)}_{x}$ being of norm one.
}

\emph{\DIFadd{Ground state.}} \DIFadd{Both $\omega^{(N)}_{0,{\color{blue}\pm}}$ and $\omega^{(N)}_{{\color{blue}\pm}}=\omega_{{\color{blue}\pm}}\circ\alpha^{N}_{\infty}$ are quasi-free with momentum-diagonal symbols, the latter with $\hat{S}^{(N)}_{\infty}(l)=S(l)$ exactly, by Remark \ref{rem:stateconv}. Since $\alpha^{M}_{N}$ maps into $\operatorname{span}\{e_{l}\}_{l\in\Gamma_{M,{\color{blue}\pm}}}$ and $\Ad_{U_{N}(t)}(a(\xi))=a(e^{it\ell^{(N)}_{\pm,0}}\xi)$ with $\ell^{(N)}_{\pm,0}$ a Fourier multiplier, $O_{N}$ is a monomial of degree $d_{A}+d_{B}$ in generators $a(\xi)$ with $\|\xi\|\leq1$ and $\hat{\xi}$ supported in $\Gamma_{M,{\color{blue}\pm}}$, for every $t$. The determinant estimate \eqref{eq:2pconvmono} of Lemma \ref{lem:stateconv} therefore applies with the symbol deviation taken over $\Gamma_{M,{\color{blue}\pm}}$ only, and gives the first term of \eqref{eq:corapproxrate}. The two cases in \eqref{eq:vacerrororders} are \eqref{eq:vacsymbolerror} and, for the chiral subalgebras, the corresponding restriction: by \eqref{eq:chiralentanglement} and \eqref{eq:masslesslimproj},
}\begin{align*}
\DIFadd{\big|\tfrac{1}{2}(1\pm\sign(l)\cos(\tfrac{1}{2}\vep_{N}l))-\tfrac{1}{2}(1\pm\sign(l))\big| }& \DIFadd{= \sin(\tfrac{1}{4}\vep_{N}l)^{2},
}\end{align*}
\DIFadd{and $\vep_{N}|l|\leq\vep_{N}\tfrac{\pi}{\vep_{M}}=\pi2^{-(N-M)}$ on $\Gamma_{M,{\color{blue}\pm}}$ in both cases.
}

\emph{\DIFadd{Dynamics.}} \DIFadd{By \eqref{eq:KSmultiestimate} and $\|R^{M}_{\infty}(\delta^{(M)}_{x,j})\|_{\infty}=1$,
}\begin{align*}
\DIFadd{\big\|(\tilde{\sigma}^{(N)}_{t}-\sigma_{t})(\pi_{{\color{blue}\pm}}(\alpha^{M}_{\infty}(B)))\big\| }& \DIFadd{\leq \sum_{p=1}^{d_{B}}\big\|(e^{it\tilde{\ell}^{(N)}_{\pm,0}}-e^{it\ell_{\pm,0}})R^{M}_{\infty}(\delta^{(M)}_{x_{p},j_{p}})\big\|_{\infty} \leq |t|\sum_{p=1}^{d_{B}}\big\|(\tilde{\ell}^{(N)}_{\pm,0}-\ell_{\pm,0})R^{M}_{\infty}(\delta^{(M)}_{x_{p},j_{p}})\big\|_{\infty},
}\end{align*}
\DIFadd{the second step being \eqref{eq:momrgestimatetime}; note that for $k=0$ the off-diagonal blocks vanish, $\ell_{\pm,0}$ being diagonal, so no Hilbert-Schmidt term occurs and the bound is linear in $t$ for all $t$. Inserting \eqref{eq:momrgtimeerrorscaling} with $\delta=2$ gives the second term of \eqref{eq:corapproxrate}.
}\end{proof}
\begin{rem}[Quantum simulation: the error budget]
\label{rem:simulationbudget}
\DIFadd{Collecting the pieces, we can now state what a simulation of a chiral free-fermion CFT costs. Let $\eta>0$ be the target accuracy for the dynamical correlation functions of Proposition \ref{prop:corapproxrate} on a time interval $|t|\leq T$, and write $d:=d_{A}+d_{B}$.
}

\emph{\DIFadd{(1) The two error sources scale in $N-M$, not in $N$.}} \DIFadd{By \eqref{eq:corapproxrate} and $\vep_{N}=2^{-N}L$, both terms are governed by the ratio $\vep_{N}/\vep_{M}=2^{-(N-M)}$ --- the dynamics term because $\Theta_{M}\vep_{N}^{2}\asymp\vep_{M}^{-\frac{3}{2}}2^{-2(N-M)}$. This is as it should be: $M$ fixes the physical resolution at which the observables are prepared and read out, and $N-M$ is the number of additional refinement steps.
}

\emph{\DIFadd{(2) Chiral observables are quadratically cheaper.}} \DIFadd{On the full two-component algebra $\fA_{M,{\color{blue}\pm}}$ the ground-state term of \eqref{eq:vacerrororders} is of }\emph{\DIFadd{first}} \DIFadd{order, and it then dominates the second-order dynamics term. The obstruction is precisely the chiral entanglement \eqref{eq:chiralentanglement} of the massless lattice vacuum: the off-diagonal block $p_{\pm}P^{(+)}_{\lambda_{N}=0}p_{\mp}$ is of order $\vep_{N}k$, whereas its continuum counterpart vanishes. Restricted to a chiral subalgebra $\fA^{(\pm)}_{M,{\color{blue}\pm}}$ --- which is where the conformal structure lives, cf.~Section \ref{sec:formallim} --- the deviation is $\sin(\tfrac{1}{4}\vep_{N}l)^{2}$ and hence of }\emph{\DIFadd{second}} \DIFadd{order, matching the dynamics term.
}
...
\DIFadd{\label{eq:qubitcount}
N-M }& \DIFadd{\geq \tfrac{1}{2}\log_{2}\big(C\!\ d\!\ (1+T)/\eta\big), \qquad\textup{hence}\qquad \#\textup{qubits} = \cO\big(2^{M}\sqrt{d\!\ (1+T)/\eta}\big).
}\end{align}
\DIFadd{The prefactor $2^{M}$ is fixed by the two constraints of Remark \ref{rem:stateapprox}: $M<N$ (Nyquist-Shannon) and $\vep_{M}(2K-1)\lesssim L$, i.e.~$2^{M}\gtrsim2K-1$, so that $s^{(\vep_{M})}$ is well localized in $S^{1}_{L}$. Since the rate $\vep_{N}^{2}$ used above requires $2$-regularity, Remark \ref{rem:regularityvalues} gives $K\geq5$ and hence $M\geq4$; the value $\delta=2$ in \eqref{eq:momrgtimeerrorscaling} itself is unconditional, no scaling function entering the momentum-cutoff estimates.
}

\emph{\DIFadd{(4) What one loses along the wavelet route.}} \DIFadd{With the wavelet renormalization group the generator error is $\cO(\vep_{N})$ rather than $\cO(\vep_{N}^{2})$, by Remark \ref{rem:KSconvrate}, so \eqref{eq:qubitcount} degrades to $\#\textup{qubits}=\cO(2^{M}d(1+T)/\eta)$ --- a }\emph{\DIFadd{quadratic}} \DIFadd{loss. The re-centred comparison of Remark \ref{rem:recentring} restores the exponent $\tfrac{1}{2}$. Localization in real space, which is what the wavelet group buys, is therefore paid for in qubits unless one re-centres.
}

\emph{\DIFadd{(5) Currents are cheaper still.}} \DIFadd{For the $U(1)$- and WZW-currents, Lemma \ref{lem:U1cur1pconv} gives a generator error $\cO(\vep_{N}^{\delta})$ with $\delta$ }\emph{\DIFadd{arbitrary}}\DIFadd{, subject only to $(\delta+\tfrac{1}{2})$-regularity of $s$, and no Schwinger-cocycle error at any finite scale. The ground-state term of \eqref{eq:vacerrororders} then becomes the sole bottleneck, and $\#\textup{qubits}=\cO(2^{M}\sqrt{d(1+T)/\eta})$ is attained already for chiral observables of the current algebra without any condition beyond $2$-regularity.
}\end{rem}

\DIFaddend \paragraph{Virasoro correlation functions}
...
\end{align}
In the case of correlation functions of Virasoro generators, \DIFdelbegin \DIFdel{we need two additional observation: First, $\|\prod_{p=1}^{n}\!:\!\!(\pi_{{\color{blue}\pm}}(\alpha^{N}_{\infty}(L^{(N)}_{\pm}(X_{N,p})\!)\!)\!\!:\!\Omega_{0}\|$ can be bounded uniformly in }\DIFdelend \DIFaddbegin \DIFadd{the two estimates \eqref{eq:corapproxvirestimate} \& \eqref{eq:corapproxvirestimateunitary} do not yet finish the argument, because Theorem \ref{thm:KSconvsmeared} provides strong convergence only on $\fF^{\alg}_{\ua}(\cD_{\std})$, whereas the vectors $\prod_{p=1}^{q-1}L_{\pm}(X_{p})\Omega_{0}$ appearing there do not lie in that domain: already $L_{\pm}(X)\Omega_{0} = a^{\dagger}(\ell_{\pm}(S^{M}_{\infty}(X))_{+-})a^{\dagger}\Omega_{0}$ is an infinite two-particle superposition, since after smearing the off-diagonal block $(\ell_{\pm}(S^{M}_{\infty}(X)))_{+-} = \tfrac{1}{2L}\sum_{k}\hat{X}_{k}(\ell_{\pm,k})_{+-}$ is Hilbert-Schmidt but no longer of finite rank --- in contrast with the unsmeared case, where $(\ell_{\pm,k})_{\pm\mp}$ is supported on the finite strip $\{n:\pm n>0,\ \mp(n\mp k)>0\}$, cf.~\eqref{eq:momrgestimates}. What is needed to close the gap is therefore the following two ingredients, which we isolate because they are of independent interest and are not implied by Theorem \ref{thm:KSconvsmeared} alone.
}

\begin{prop}[Uniform energy bound, smeared]
\label{hyp:energybound}
\DIFadd{Let $s$ be $1$-regular in the sense of Definition \ref{def:regularity}, let $M\in\N_{0}$ and $X\in\fl(\Lambda_{M},\C)$. Then there is $C_{X}>0$ such that
}\begin{align}
\DIFadd{\label{eq:energybound}
\big\|:\!\pi_{{\color{blue}\pm}}(\alpha^{N}_{\infty}(L^{(N)}_{\pm}(S^{M}_{N}(X))))\!:\Psi\big\| }& \DIFadd{\leq C_{X}\big\|(\1+L_{\pm,0})\Psi\big\|, }& \DIFadd{\Psi }& \DIFadd{\in\cD(L_{\pm,0}),
}\end{align}
\DIFadd{for all $N>M$: the smeared KS approximants obey the linear energy bounds of the smeared Virasoro fields \mbox{%DIFAUXCMD
\cite{BuchholzHaagDualityIn, CarpiOnTheUniqueness} }\hskip0pt%DIFAUXCMD
}\emph{\DIFadd{uniformly in the scale $N$}}\DIFadd{.
}\end{prop}
\begin{proof}
\DIFadd{By \eqref{eq:KShermiteanparts}, $:\!\pi_{{\color{blue}\pm}}(\alpha^{N}_{\infty}(L^{(N)}_{\pm}(S^{M}_{N}(X))))\!: = \tfrac{L}{\pi}dF_{{\color{blue}\pm}}(G^{(N)})$ with
}\begin{align*}
\DIFadd{G^{(N)} }& \DIFadd{:= \tilde{\ell}^{(N)}_{\pm}(S^{M}_{N}(X)) = \tfrac{1}{2L_{N}}\!\!\!\sum_{k\in\Gamma_{N,+}}\!\!\!S^{M}_{N}(\hat{X})_{k}\!\ \tilde{\ell}^{(N)}_{\pm,k},
}\end{align*}
\DIFadd{so it suffices to verify the two hypotheses \eqref{eq:relbound1k} of Lemma \ref{lem:energybound1k} for $G^{(N)}$ in place of $\tilde{\ell}^{(N)}_{\pm,k}$, uniformly in }\DIFaddend $N$\DIFdelbegin \DIFdel{in terms of $\|\prod_{p=1}^{n}L_{\pm}(X_{p})\Omega_{0}\|$ for sufficiently regular }\DIFdelend \DIFaddbegin \DIFadd{; the remainder of that proof then applies verbatim.
}

\DIFadd{For the first, insert the fixed-$k$ bound of \eqref{eq:relbound1k} and use $\tfrac{1}{2L_{N}}2^{N-M}=\tfrac{1}{2L_{M}}$ together with Definition \ref{def:waveletrgcur}:
}\begin{align}
\DIFadd{\label{eq:relbound1psmeared}
\big\|A^{-\frac{1}{2}}G^{(N)}A^{-\frac{1}{2}}\big\| }& \DIFadd{\leq \tfrac{C}{2L_{M}}\!\!\!\sum_{k\in\Gamma_{N,+}}\!\!\Big|\prod_{j=1}^{N-M}m_{0}(\vep_{M+j}k)\Big|\!\ |\hat{X}_{\per}(k)|\!\ \langle k\rangle^{\frac{1}{2}} }\\ \DIFadd{\nonumber
}& \DIFadd{\leq \tfrac{C\!\ C_{K}\|\hat{X}\|_{\infty}}{2L_{M}}\!\!\!\sum_{k\in\Gamma_{\infty,{\color{blue}\pm}}}\!\!\!(1+\vep_{M}|k|)^{-\rho}\langle k\rangle^{\frac{1}{2}} \ =:\ c_{X} \ <\ \infty,
}\end{align}
\DIFadd{where the second step used Corollary \ref{cor:convergence}, whose hypothesis $|\vep_{M}k|\leq2^{N-M}\pi$ is satisfied }\emph{\DIFadd{precisely because $k$ ranges over the Brillouin zone}} \DIFadd{$\Gamma_{N,+}$, and where the last sum is finite --- and independent of $N$ --- if and only if $\rho-\tfrac{1}{2}>1$, i.e.~exactly under the standing hypothesis of $1$-regularity. This is the only point at which the regularity of the scaling function enters.
}

\DIFadd{For the second, Lemma \ref{lem:KSconvsmeared} shows that the Hilbert-Schmidt norms $\|(G^{(N)})_{\pm\mp}\|_{2}$ }\emph{\DIFadd{converge}} \DIFadd{as $N\rightarrow\infty$, so that $\gamma_{X}:=\sup_{N>M}(\|(G^{(N)})_{+-}\|_{2}+\|(G^{(N)})_{-+}\|_{2})<\infty$. Lemma \ref{lem:energybound1k} now gives \eqref{eq:energybound} with $C_{X}=\tfrac{L}{\pi}(12\!\ c_{X}+6\!\ \gamma_{X})$, say.
}\end{proof}

\DIFadd{Granting Proposition \ref{hyp:energybound}, the proof of Theorem \ref{thm:corapproxvir} is completed as follows. First, iterating \eqref{eq:energybound} and using $L_{\pm,0}\Omega_{0}=h\Omega_{0}$, with $h=0$ in the Neveu-Schwarz and $h=\tfrac{1}{8}$ (resp.~$\tfrac{1}{16}$) in the Ramond sector, bounds $\|\prod_{p=q+1}^{n}\!:\!\pi_{{\color{blue}\pm}}(\alpha^{N}_{\infty}(L^{(N)}_{\pm}(X_{N,p})))\!:\!\Omega_{0}\|$ uniformly in $N$, which is what \eqref{eq:corapproxvirestimate} requires. Second, $\prod_{p=1}^{q-1}L_{\pm}(X_{p})\Omega_{0}$ lies in $\cD(L_{\pm,0}^{n})$ and is, for sufficiently regular }\DIFaddend $X_{p}$\DIFdelbegin \DIFdel{as powers of the approximants $:\!(\pi_{{\color{blue}\pm}}\circ\alpha^{N}_{\infty})(L^{(N)}_{\pm}(X_{N,p}))\!:$ are uniformly bounded (in }\DIFdelend \DIFaddbegin \DIFadd{, the norm limit of its finite-energy truncations $P_{\leq E}\prod_{p=1}^{q-1}L_{\pm}(X_{p})\Omega_{0}\in\fF^{\alg}_{\ua}(\cD_{\std})$; combining this with \eqref{eq:energybound} and Theorem \ref{thm:KSconvsmeared}, applied to the truncations, gives
}\begin{align*}
\DIFadd{\lim_{N\rightarrow\infty}\Big\|\Big(\!:\!\pi_{{\color{blue}\pm}}(\alpha^{N}_{\infty}(L^{(N)}_{\pm}(X_{N,q})))\!:\!-L_{\pm}(X_{q})\Big)\Big(\prod_{p=1}^{q-1}L_{\pm}(X_{p})\Big)\Omega_{0}\Big\| }& \DIFadd{= 0,
}\end{align*}
\DIFadd{by an $\tfrac{\vep}{3}$-argument, and similarly for \eqref{eq:corapproxvirestimateunitary}. We stress that Proposition \ref{hyp:energybound} --- the lattice counterpart of the Buchholz--Schulz-Mirbach/Carpi--Weiner energy bounds --- is exactly the ingredient that this argument requires, and that it is used here uniformly in the scale }\DIFaddend $N$\DIFdelbegin \DIFdel{) by $L_{\pm}(X_{p})$.
Second, $\prod_{p=1}^{n}L_{\pm}(X_{p}))\Omega_{0}$ is given by a convergent sum (depending on the regularity of $X_{p}$) of Fock-space vectors with finite particle number in momentum space}\DIFdelend \DIFaddbegin \DIFadd{; and that, by \eqref{eq:relbound1psmeared}, the regularity of the scaling function enters this section at one point only, namely through the summation over the smearing modes}\DIFaddend .
\end{proof}
...
\newblock URL \url{https://doi.org/10.1007/bf02096738}\DIFaddbegin \DIFadd{.
}

\bibitem[Buchholz and Schulz-Mirbach(1990)]{BuchholzHaagDualityIn}
\DIFadd{D.~Buchholz and H.~Schulz-Mirbach.
}\newblock {\DIFadd{Haag duality in conformal quantum field theory}}\DIFadd{.
}\newblock \emph{\DIFadd{Reviews in Mathematical Physics}}\DIFadd{, 2}\penalty0 \DIFadd{(1):}\penalty0
  \DIFadd{105--125, 1990.
}\newblock \doi{10.1142/S0129055X90000053}\DIFadd{.
}\newblock \DIFadd{URL }\url{https://doi.org/10.1142/S0129055X90000053}\DIFaddend .
